...
```{r precision-beta-api, eval=FALSE}
fit <- qdesn_fit_mcmc(
  y = y,
  p0 = 0.5,
  mcmc_args = list(
...
* **Synthesis helper** `quantileSynthesis()` to combine posterior quantile-draw objects.
...
* **Transfer-function helpers** `exdqlmTransferLDVB()` and `exdqlmTransferMCMC()`, with legacy `exdqlmTransferISVB()` retained for backward compatibility, plus expanded static object generics (`exal_mcmc`, `exal_ldvb`).
* **Compatibility note:** this validation branch still exports the lower-snake-case
  `exal_static_*`, `transfn_exdqlm*`, and `exdqlm_synthesize_from_draws()`
  names as aliases, but package-facing documentation now follows the upstream
  `0.4.0` naming.
...
into a single non-crossing predictive object.
