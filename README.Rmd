...
* **Dynamic Bayesian quantile state-space modeling** is the core use case:
  `exdqlmISVB()`, `exdqlmLDVB()`, and `exdqlmMCMC()` all fit univariate
  time-series quantile models with a familiar DLM/state-space specification.
...
* **Multiple inference engines** are available depending on the use case:
  fast approximate dynamic VB (`ISVB`), deterministic Laplace-Delta VB
  (`LDVB`), and posterior simulation (`MCMC`).
