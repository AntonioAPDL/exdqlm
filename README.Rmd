...
In **v0.4.0**, this CRAN update consolidates the internal 0.4/0.5/0.6 feature line: dynamic **LDVB** (`exdqlmLDVB()`), synthesis (`exdqlm_synthesize_from_draws()`), and static regression/exAL routines (`regMod()`, `exal_static_LDVB()`, `exal_static_mcmc()`), while keeping backend defaults conservative for CRAN stability.
