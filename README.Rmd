...
* **Static Bayesian quantile regression** is now part of the same package via
  `exal_static_LDVB()` and `exal_static_mcmc()`, rather than living in a
...
* **Post hoc multi-quantile synthesis** is built in through
  `exdqlm_synthesize_from_draws()`, which can enforce isotonicity and apply
...
* **Transfer-function helpers** `transfn_exdqlmLDVB()` and `transfn_exdqlmMCMC()`, with legacy `transfn_exdqlmISVB()` retained for backward compatibility, plus expanded static object generics (`exal_mcmc`, `exal_ldvb`).
