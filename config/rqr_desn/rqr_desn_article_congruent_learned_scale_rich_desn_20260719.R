rqr_desn_article_congruent_simulation_config <- list(
  config_id = "rqr_desn_article_congruent_learned_scale_rich_desn_20260719",
  status = "frozen_guarded_no_full_launch_learned_scale",
  ...
  interpretation = list(
    ...
    learned_scale_interpretation = "global inverse RQR loss scale on the standardized-analysis scale; not a response variance and not an uncertainty-calibration guarantee",
  ...
  scientific_contract = list(
    ...
    rqr_learned_scale_prior = list(shape = 4, rate = 4, power = 1),
  ...
  priors = list(
    rqr_rhs_ns = list(type = "rhs_ns", tau0 = 1e-4, a_zeta = 2, b_zeta = 1, s2 = 1, n_inner = 1L, shrink_intercept = FALSE),
    qdesn_rhs_ns = list(type = "rhs_ns", tau0 = 1e-4, a_zeta = 2, b_zeta = 1, s2 = 1, n_inner = 1L, shrink_intercept = FALSE)
  ),
  stages = list(
    list(
    ...
    list(
      ...
      purpose = "article-congruent rolling one-step interval forecasting with richer fixed DESN features and matched quantile-pair competitors",
      replicates = 30L,
      calibration_n = 500L,
      final_fit_n = 500L,
      test_n = 1000L,
      oracle_endpoints = TRUE,
      design = list(
        design_id = "rich_desn_D2_n100_m50_alpha0p1_rho0p95",
  ...
  competitors = list(
    ...
    list(
      ...
      chain_count = 3L,
      adapter_note = "Fixed-rate continuity benchmark using the previous nominal grid on the standardized RQR loss scale."
    ),
    list(
