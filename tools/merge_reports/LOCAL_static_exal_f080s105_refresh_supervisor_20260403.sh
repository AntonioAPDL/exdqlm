...
launch_script="$out_dir/LOCAL_static_exal_f080s105_refresh_launch_20260403.sh"
...
for ((pass=1; pass<=max_passes; pass++)); do
  ...
  echo "=== static refresh supervisor pass ${pass}/${max_passes} at ${ts} ==="
  ...
  if [[ -n "$missing_now" && "$missing_now" == "0" ]]; then
    echo "All static refresh rows completed."
    exit 0
  fi
