...
resolved_tag="${REFRESHED288_RUN_TAG:-20260419_numcrash_thetafreeze_v1}"
resolved_tag="${resolved_tag//[^A-Za-z0-9_-]/_}"

prepare_script="$script_dir/LOCAL_refreshed288_prepare_numerical_failures_20260419.R"
...
status_path="$repo_root/tools/merge_reports/LOCAL_refreshed288_numerical_runtime_failure_manifest_status_${resolved_tag}.csv"
phase_path="$repo_root/tools/merge_reports/LOCAL_refreshed288_numerical_runtime_failure_phase_summary_${resolved_tag}.csv"
method_path="$repo_root/tools/merge_reports/LOCAL_refreshed288_numerical_runtime_failure_method_summary_${resolved_tag}.csv"
report_path="$repo_root/reports/static_exal_tuning_${resolved_tag:0:8}/refreshed288_numerical_runtime_failure_status_${resolved_tag}.md"
...
worker_count_for_phase() {
  case "$1" in
    numerical_vb_primary) echo "$workers_vb" ;;
    numerical_exdqlm_mcmc) echo "$workers_exdqlm_mcmc" ;;
    numerical_dqlm_mcmc) echo "$workers_dqlm_mcmc" ;;
    *) echo "1" ;;
  esac
}
...
case "$action" in
  ...
  dry-run)
    ...
    run_eval_and_report
    for phase in numerical_vb_primary numerical_exdqlm_mcmc numerical_dqlm_mcmc; do
      launch_phase "$phase"
    done
    ;;
  launch)
