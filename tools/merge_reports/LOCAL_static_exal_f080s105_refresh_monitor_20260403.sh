...
session="static-exal-f080s105-refresh-20260403"
interval="120"
max_checks="360"
...
latest_row_log_heartbeat() {
  local latest_line
  latest_line="$(find "$out_dir" -maxdepth 1 -type f -name 'LOCAL_static_exal_f080s105_refresh_*.log' -printf '%T@ %p\n' 2>/dev/null | sort -nr | head -n1 || true)"
...
while true; do
  ...
  echo "=== static refresh monitor ${ts} (check ${check}) ==="

  session_state="NOT_RUNNING"
  if tmux has-session -t "$session" 2>/dev/null; then
    session_state="RUNNING"
  fi
  runner_count="$( { pgrep -af 'LOCAL_static_exal_case_runner_20260323.R.*static_exal_f080_sub2_s105_(rhsns_current|rhs_legacy)_20260403' 2>/dev/null || true; } | wc -l | tr -d ' ' )"
  ...
  if [[ -n "$missing_now" && "$missing_now" == "0" ]]; then
    echo "All static refresh rows completed. Exiting monitor."
    exit 0
  fi
