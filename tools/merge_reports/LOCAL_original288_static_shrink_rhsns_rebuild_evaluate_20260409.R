...
status <- read_original288_static_shrink_rhsns_rebuild_status(manifest_path = manifest_path, run_tag = tag)
...
summarize_status_rhsns <- function(df, group_cols) {
...
phase_summary <- summarize_status_rhsns(status, "phase")
phase_summary$phase_order <- unname(phase_order_original288_static_shrink_rhsns_rebuild[phase_summary$phase])
...
block_summary <- summarize_status_rhsns(status, c("inference", "model"))
block_summary <- block_summary[order(block_summary$inference, block_summary$model), , drop = FALSE]
utils::write.csv(block_summary, paths$block_summary, row.names = FALSE)

accepted_compare <- summarize_status_rhsns(status, "accepted_compare")
