...
source("tools/merge_reports/LOCAL_original288_syncedbase_rerun_helpers_20260406.R")

repo_root <- normalizePath(".", winslash = "/", mustWork = TRUE)
setwd(repo_root)

args <- commandArgs(trailingOnly = TRUE)
manifest_path <- paths_original288_syncedbase_rerun()$manifest
...
status <- read_original288_syncedbase_rerun_status(manifest_path = manifest_path, run_tag = tag)
...
phase_summary$phase_order <- unname(phase_order_original288_syncedbase_rerun[phase_summary$phase])
...
block_summary <- summarize_status(status, c("root_kind", "inference"))
block_summary <- block_summary[order(block_summary$root_kind, block_summary$inference), , drop = FALSE]
