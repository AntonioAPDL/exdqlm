...
carry_path <- "tools/merge_reports/LOCAL_original288_carryforward_selection_v1_20260405.csv"
...
best_rows$selection_source_type <- ifelse(
  best_rows$best_phase == "archive_rescore_existing",
  "dynamic_archive_rescore_20260405",
  "dynamic_residual_20260405"
)
...
best_rows$selection_reason <- ifelse(
  ...
  sprintf(
    "Promote dynamic residual candidate %s from phase %s after improving original baseline from %s to %s.",
  ...
  sprintf(
    "Retain unresolved original baseline because dynamic residual best candidate remained %s.",
    best_rows$best_gate_overall
  )
)
...
if (nrow(selection_update)) {
  ...
  if (length(hit)) {
    ...
    carry2$selection_mode[idx[hit]] <- "promoted_dynamic_residual"
