...
prepare_script="$out_dir/LOCAL_static_exal_failband_wave3_prepare_20260404.R"
...
rows_tsv="$out_dir/LOCAL_static_exal_failband_wave3_rows_20260404.tsv"
...
if [[ "$mode" == "evaluate" ]]; then
  Rscript "$evaluate_script" ${stage_filter:+--stage="$stage_filter"} ${candidate_filter:+--candidate="$candidate_filter"}
  exit 0
fi
...
if [[ -z "$stage_filter" ]]; then
  echo "launch requires --stage=residual18 or --stage=confirm30" >&2
  exit 2
fi

launch_rows_tsv="$(mktemp "/tmp/exdqlm_static_failband_wave3_rows_XXXX.tsv")"
trap 'rm -f "$launch_rows_tsv"' EXIT

awk -F'\t' -v stage_filter="$stage_filter" -v scope_filter="$scope_filter" -v candidate_filter="$candidate_filter" -v row_filter="$row_filter" '
  BEGIN {
    split(candidate_filter, tmp_c, ",")
    for (i in tmp_c) if (tmp_c[i] != "") wanted_candidate[tmp_c[i]] = 1
    split(row_filter, tmp_r, ",")
    for (i in tmp_r) if (tmp_r[i] != "") wanted_row[tmp_r[i]] = 1
  }
  {
    keep = 1
    if (stage_filter != "" && $1 != stage_filter) keep = 0
    if (scope_filter != "" && $3 != scope_filter) keep = 0
    if (candidate_filter != "" && !($2 in wanted_candidate)) keep = 0
...
manifest="$out_dir/LOCAL_static_exal_failband_wave3_manifest_$(date '+%Y%m%d_%H%M%S')_${RANDOM}_$$.csv"
fail_log="$out_dir/LOCAL_static_exal_failband_wave3_failures_$(date '+%Y%m%d_%H%M%S')_${RANDOM}_$$.log"
...
run_one() {
  ...
  local log_path="$out_dir/LOCAL_static_exal_failband_wave3_${stage}_${candidate_id}_${scope_label}_row${row_id}.log"
  ...
  if [[ "$rc" -ne 0 ]]; then
    append_fail_log "$(date '+%Y-%m-%d %H:%M:%S'),${stage},${candidate_id},${scope_label},${row_id},${variant_tag},rc=${rc},log=${log_path}"
...
if [[ "$xargs_rc" -ne 0 && "$keep_going" == "0" ]]; then
  echo "failband wave-3 launch aborted due to non-zero exit (rc=$xargs_rc)"
  exit "$xargs_rc"
fi
