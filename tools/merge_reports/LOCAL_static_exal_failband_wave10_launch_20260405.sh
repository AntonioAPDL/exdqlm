...
prepare_script="$out_dir/LOCAL_static_exal_failband_wave10_prepare_20260405.R"
...
rows_tsv="$out_dir/LOCAL_static_exal_failband_wave10_rows_20260405.tsv"
...
launch_rows_tsv="$(mktemp "/tmp/exdqlm_static_failband_wave10_rows_XXXX.tsv")"
...
manifest="$out_dir/LOCAL_static_exal_failband_wave10_manifest_$(date '+%Y%m%d_%H%M%S')_${RANDOM}_$$.csv"
fail_log="$out_dir/LOCAL_static_exal_failband_wave10_failures_$(date '+%Y%m%d_%H%M%S')_${RANDOM}_$$.log"
...
run_one() {
  ...
  local log_path="$out_dir/LOCAL_static_exal_failband_wave10_${candidate_id}_${scope_label}_row${row_id}.log"
...
if [[ "$xargs_rc" -ne 0 && "$keep_going" == "0" ]]; then
  echo "failband wave-10 launch aborted due to non-zero exit (rc=$xargs_rc)"
  exit "$xargs_rc"
fi
...
if [[ -z "${missing_now:-}" ]]; then
  echo "unable to parse evaluator summary for static failband wave-10" >&2
  exit 4
fi

if [[ "${missing_now}" != "0" ]]; then
  echo "static failband wave-10 stage is incomplete: ${missing_now} rows remain MISSING after launch" >&2
  exit 5
fi
