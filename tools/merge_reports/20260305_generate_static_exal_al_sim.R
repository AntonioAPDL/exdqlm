...
p_grid_default <- c(0.01, seq(0.05, 0.95, by = 0.05), 0.99)

TT <- safe_int(Sys.getenv("EXDQLM_STATIC_SIM_TT", "5000"), 5000L)
seed <- safe_int(Sys.getenv("EXDQLM_STATIC_SIM_SEED", "20260305"), 20260305L)
period <- safe_num(Sys.getenv("EXDQLM_STATIC_SIM_PERIOD", "50"), 50)
p0_gen <- safe_num(Sys.getenv("EXDQLM_STATIC_SIM_P0", "0.5"), 0.5)
sigma_true <- safe_num(Sys.getenv("EXDQLM_STATIC_SIM_SIGMA", "3.0"), 3.0)
gamma_true <- safe_num(Sys.getenv("EXDQLM_STATIC_SIM_GAMMA", "0.35"), 0.35)
out_root <- Sys.getenv(
...
if (!is.finite(period) || period <= 2) stop("period must be > 2")

L <- L.fn(p0_gen)
U <- U.fn(p0_gen)
if (!is.finite(gamma_true) || gamma_true <= L || gamma_true >= U) {
  stop(sprintf("gamma_true=%.4f must be in (L,U)=(%.4f, %.4f)", gamma_true, L, U))
}

p_grid <- sort(unique(as.numeric(p_grid_default)))
...
set.seed(seed)
t_idx <- seq_len(TT)
x1 <- sin(2 * pi * t_idx / period)
x2 <- cos(2 * pi * t_idx / period)
x3 <- as.numeric(scale(t_idx, center = TRUE, scale = TRUE))
X <- cbind(1, x1, x2, x3)
colnames(X) <- c("intercept", "x_sin", "x_cos", "x_trend")

beta_true <- c(0.0, 2.0, -1.4, 0.6)
mu <- as.numeric(drop(X %*% beta_true))
y <- rexal(n = TT, p0 = p0_gen, mu = mu, sigma = sigma_true, gamma = gamma_true)

# exAL is location-shifted in mu, so we can compute baseline quantiles once.
q0_exal <- suppressWarnings(as.numeric(qexal(p_grid, p0 = p0_gen, mu = 0, sigma = sigma_true, gamma = gamma_true)))
q0_al <- suppressWarnings(as.numeric(qexal(p_grid, p0 = p0_gen, mu = 0, sigma = sigma_true, gamma = 0)))

fallback_q <- function(p_vec, gamma_val, n_draw = 300000L) {
  draws <- rexal(n = as.integer(n_draw), p0 = p0_gen, mu = 0, sigma = sigma_true, gamma = gamma_val)
  as.numeric(stats::quantile(draws, probs = p_vec, names = FALSE, type = 8))
}

if (any(!is.finite(q0_exal))) {
  bad <- which(!is.finite(q0_exal))
  q0_exal[bad] <- fallback_q(p_grid[bad], gamma_true)
}
if (any(!is.finite(q0_al))) {
  bad <- which(!is.finite(q0_al))
  q0_al[bad] <- fallback_q(p_grid[bad], 0)
}
q_exal <- outer(mu, q0_exal, `+`)
q_al <- outer(mu, q0_al, `+`)

q_exal <- as.matrix(q_exal)
q_al <- as.matrix(q_al)
...
cat("-------------------------\n")
cat("scenario: static_exal_mildskew\n")
cat(sprintf("seed: %d\n", seed))
cat(sprintf("TT: %d\n", TT))
