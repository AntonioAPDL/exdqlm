...
dynamic_tail8_phase_order_original288 <- c(
  anchor8_slice_sync = 1L,
  tau05_long6_slice_sync = 2L
)

dynamic_tail8_force_original288 <- c(
  anchor8_slice_sync = 1L,
  tau05_long6_slice_sync = 1L
)

gate_rank_dynamic_tail8_original288 <- function(x) {
...
candidate_preference_rank_dynamic_tail8_original288 <- function(phase) {
  ranks <- c(
    tau05_long6_slice_sync = 2L,
    anchor8_slice_sync = 1L
  )
...
rbind_fill_dynamic_tail8_original288 <- function(parts) {
...
paths_dynamic_tail8_original288 <- function() {
  list(
    manifest = "tools/merge_reports/LOCAL_original288_dynamic_tail8_manifest_20260405.csv",
    stage_counts = "tools/merge_reports/LOCAL_original288_dynamic_tail8_stage_counts_20260405.csv",
    manifest_status = "tools/merge_reports/LOCAL_original288_dynamic_tail8_manifest_status_20260405.csv",
    phase_summary = "tools/merge_reports/LOCAL_original288_dynamic_tail8_phase_summary_20260405.csv",
    config_summary = "tools/merge_reports/LOCAL_original288_dynamic_tail8_config_summary_20260405.csv",
    case_best = "tools/merge_reports/LOCAL_original288_dynamic_tail8_case_best_20260405.csv",
    unresolved_after_run = "tools/merge_reports/LOCAL_original288_dynamic_tail8_unresolved_after_run_20260405.csv",
    selection_update = "tools/merge_reports/LOCAL_original288_dynamic_tail8_selection_update_20260405.csv",
    selection_delta = "tools/merge_reports/LOCAL_original288_dynamic_tail8_selection_delta_20260405.csv",
    carryforward_preview = "tools/merge_reports/LOCAL_original288_carryforward_selection_dynamic_tail8_preview_20260405.csv",
    health_summary_preview = "tools/merge_reports/LOCAL_original288_health_summary_dynamic_tail8_preview_20260405.csv",
    tracker = "tools/merge_reports/LOCAL_ORIGINAL288_DYNAMIC_RECOVERY_TRACKER_20260405.md",
    plan_doc = "reports/static_exal_tuning_20260405/original_288_dynamic_tail8_closure_program_20260405.md",
    execution_doc = "reports/static_exal_tuning_20260405/original_288_dynamic_tail8_closure_execution_20260405.md"
  )
}

run_tag_dynamic_tail8_original288 <- function() {
  "original288_dynamic_tail8_20260405"
}

build_candidate_fit_path_dynamic_tail8_original288 <- function(baseline_fit_path, variant_tag) {
  sub("\\.rds$", paste0("_", variant_tag, ".rds"), normalize_path_original288(baseline_fit_path))
}

full288_seed_map_dynamic_tail8_original288 <- function() {
  ...
  out <- rbind_fill_dynamic_tail8_original288(Filter(Negate(is.null), rows))
...
read_unresolved_dynamic_original288_dynamic_tail8 <- function() {
  path <- "tools/merge_reports/LOCAL_original288_unresolved_dynamic_inventory_v2_20260405.csv"
...
read_dynamic_vb_selection_original288_dynamic_tail8 <- function() {
  path <- "tools/merge_reports/LOCAL_original288_carryforward_selection_v2_20260405.csv"
  x <- read.csv(path, stringsAsFactors = FALSE, check.names = FALSE)
  x <- subset(
    x,
    block == "dynamic" &
...
config_note_dynamic_tail8_original288 <- function(config_id) {
  switch(
    config_id,
    mcmc_exdqlm_slice_sync = paste(
      "Exact dynamic exdqlm slice corridor carried forward from the successful",
      "historical rescues: slice proposal, no adaptation, 1200 burn, 4000 kept,",
      "and explicit healthy VB warm start."
    ),
    mcmc_exdqlm_slice_sync_long = paste(
      "Low-tail escalation only for tau=0p05 unresolved cases: keep the same",
      "successful slice geometry and healthy VB warm start, but extend burn and",
      "kept draws for more stable tail exploration."
    ),
    "Unspecified dynamic tail-8 config."
  )
}

apply_dynamic_tail8_config_original288 <- function(cfg, config_id) {
  cfg <- cfg %||% list()
  cfg$mcmc <- cfg$mcmc %||% list()
  cfg$mcmc$mh <- cfg$mcmc$mh %||% list()

  if (identical(config_id, "mcmc_exdqlm_slice_sync")) {
    cfg$mcmc$burn <- 1200L
    cfg$mcmc$n <- 4000L
  ...
  if (identical(config_id, "mcmc_exdqlm_slice_sync_long")) {
    cfg$mcmc$burn <- 2000L
    cfg$mcmc$n <- 8000L
    cfg$mcmc$trace_every <- 50L
    cfg$mcmc$init_from_vb <- TRUE
...
read_dynamic_tail8_status_original288 <- function(manifest_path = paths_dynamic_tail8_original288()$manifest,
                                                  run_tag = run_tag_dynamic_tail8_original288()) {
  ...
  rows <- rbind_fill_dynamic_tail8_original288(parts)
  if (nrow(rows)) {
