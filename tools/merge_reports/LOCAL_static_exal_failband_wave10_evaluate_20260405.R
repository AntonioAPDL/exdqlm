...
if (!nrow(schedule)) stop("wave-10 schedule subset is empty")
...
schedule$scale_dist <- abs(schedule$global_eta_jump_scale - 1.025)
...
row_best <- do.call(rbind, lapply(split(schedule, paste(schedule$scope_label, schedule$row_id, sep = "\r")), function(df) {
  ...
  data.frame(
    ...
    best_geometry_candidate = best$geometry_candidate,
...
remaining <- schedule[schedule$gate_overall %in% c("MISSING", "FAIL"),
  c("stage", "candidate_id", "geometry_candidate", "scope_label", "row_id", "family", "tt", "tau", "gate_overall", "healthy", "unhealthy_reason"),
  drop = FALSE
]
