...
status <- read_dynamic_tail7_rw_status_original288()

if (!nrow(status)) {
  stop("dynamic tail7-rw manifest status is empty")
}

status$gate_overall[is.na(status$gate_overall) | !nzchar(status$gate_overall)] <- "MISSING"
status$gate_rank <- gate_rank_dynamic_tail7_rw_original288(status$gate_overall)
status$baseline_gate_rank <- gate_rank_dynamic_tail7_rw_original288(status$baseline_gate_overall)
status$candidate_preference_rank <- candidate_preference_rank_dynamic_tail7_rw_original288(status$phase)
...
phase_summary <- phase_summary[order(dynamic_tail7_rw_phase_order_original288[phase_summary$phase]), , drop = FALSE]
...
config_summary <- config_summary[order(dynamic_tail7_rw_phase_order_original288[config_summary$phase], config_summary$config_id), , drop = FALSE]
...
case_best <- case_best[order(case_best$fit_size, case_best$family, case_best$tau), , drop = FALSE]
...
if (!nrow(view)) {
  stop("dynamic tail7-rw evaluate filter matched no rows")
}
