...
status <- read_dynamic_tail8_status_original288()

if (!nrow(status)) {
  stop("dynamic tail-8 manifest status is empty")
}

status$gate_overall[is.na(status$gate_overall) | !nzchar(status$gate_overall)] <- "MISSING"
status$gate_rank <- gate_rank_dynamic_tail8_original288(status$gate_overall)
status$baseline_gate_rank <- gate_rank_dynamic_tail8_original288(status$baseline_gate_overall)
status$candidate_preference_rank <- candidate_preference_rank_dynamic_tail8_original288(status$phase)
...
phase_summary <- phase_summary[order(dynamic_tail8_phase_order_original288[phase_summary$phase]), , drop = FALSE]
...
config_summary <- config_summary[order(dynamic_tail8_phase_order_original288[config_summary$phase], config_summary$config_id), , drop = FALSE]
...
if (!nrow(view)) {
  stop("dynamic tail-8 evaluate filter matched no rows")
}
