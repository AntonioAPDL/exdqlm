...
source("tools/merge_reports/LOCAL_original288_syncedbase_dynamic_tail6_localmix_helpers_20260408.R")

repo_root <- normalizePath(".", winslash = "/", mustWork = TRUE)
setwd(repo_root)
paths <- paths_original288_syncedbase_dynamic_tail6_localmix()
...
source_status <- read_source_status_original288_syncedbase_dynamic_tail6_localmix()
queue <- subset(source_status, in_scope)
deferred <- read_deferred_inventory_original288_syncedbase_dynamic_tail6_localmix()
...
spec <- schedule_spec_original288_syncedbase_dynamic_tail6_localmix()
...
for (i in seq_len(nrow(resolved))) {
  ...
  reference_fit_path <- reference_fit_path_original288_syncedbase_dynamic_tail6_localmix(
    target_row,
    reference_candidate_label = spec_row$reference_candidate_label[1],
    reference_path = spec_row$reference_path[1],
    reference_basename = spec_row$reference_basename[1]
  )
  ...
  cfg <- apply_overrides_original288_syncedbase_dynamic_tail6_localmix(cfg, spec_row)
  ...
  row_id <- i
  cfg_path <- config_path_original288_syncedbase_dynamic_tail6_localmix(row_id)
  ensure_dir_original288_syncedbase_rerun(dirname(cfg_path))
  saveRDS(cfg, cfg_path)

  candidate_fit_path <- candidate_fit_path_original288_syncedbase_dynamic_tail6_localmix(
    target_row$run_root,
  ...
  vb_candidate_fit_path <- vb_candidate_fit_path_original288_syncedbase_dynamic_tail6_localmix(
    target_row$run_root,
    target_row$model,
    target_row$tau_label,
    spec_row$candidate_label[1]
  ...
  rows[[i]] <- data.frame(
    ...
    phase_order = unname(phase_order_original288_syncedbase_dynamic_tail6_localmix[spec_row$phase[1]]),
    ...
    planned_reference_path = safe_chr_original288_syncedbase_rerun(spec_row$reference_path[1], NA_character_),
  ...
  schedule_rows[[i]] <- data.frame(
    ...
    reference_path = safe_chr_original288_syncedbase_rerun(spec_row$reference_path[1], NA_character_),
    reference_basename = safe_chr_original288_syncedbase_rerun(spec_row$reference_basename[1], basename(reference_fit_path)),
...
stage_counts$phase_order <- unname(phase_order_original288_syncedbase_dynamic_tail6_localmix[stage_counts$phase])
