...
source("tools/merge_reports/LOCAL_original288_dynamic_tail7_rw_helpers_20260406.R")

paths <- paths_dynamic_tail7_rw_original288()
dir.create(dirname(paths$manifest), recursive = TRUE, showWarnings = FALSE)

config_dir <- file.path("tools", "merge_reports", "original288_dynamic_tail7_rw_configs_20260406")
dir.create(config_dir, recursive = TRUE, showWarnings = FALSE)

unresolved <- read_unresolved_dynamic_original288_dynamic_tail7_rw()
vb_sel <- read_dynamic_vb_selection_original288_dynamic_tail7_rw()
seed_map <- full288_seed_map_dynamic_tail7_rw_original288()

if (nrow(unresolved) != 7L) {
  stop(sprintf("expected exactly 7 unresolved dynamic tail cases, found %d", nrow(unresolved)))
}

tt500_subset <- subset(unresolved, fit_size == 500L)
if (nrow(tt500_subset) != 4L) {
  stop(sprintf("expected exactly 4 TT500 unresolved tail cases, found %d", nrow(tt500_subset)))
}

tt5000_subset <- subset(unresolved, fit_size == 5000L)
if (nrow(tt5000_subset) != 3L) {
  stop(sprintf("expected exactly 3 TT5000 unresolved tail cases, found %d", nrow(tt5000_subset)))
}
...
variant_tag_for_config <- function(config_id) {
  switch(
    config_id,
    mcmc_exdqlm_rw_joint_anchor = "orig288_dyn_tail7_rw_joint_20260406",
    mcmc_exdqlm_rw_refresh_tt500 = "orig288_dyn_tail7_rw_refresh4_20260406",
    mcmc_exdqlm_rw_joint_tt5000_long = "orig288_dyn_tail7_rw_joint_long3_20260406",
...
build_rows <- function(df, phase, config_id, seed_offset) {
  ...
  for (i in seq_len(nrow(df))) {
    ...
    cfg_out <- apply_dynamic_tail7_rw_config_original288(base_cfg, config_id, r$fit_size[1])
    ...
    rows[[i]] <- data.frame(
      ...
      block = "dynamic",
      phase = phase,
      phase_order = dynamic_tail7_rw_phase_order_original288[phase],
      config_id = config_id,
      ...
      candidate_fit_path = build_candidate_fit_path_dynamic_tail7_rw_original288(baseline_fit_path, variant_tag),
      ...
      rationale = config_note_dynamic_tail7_rw_original288(config_id),
...
tt500_subset <- tt500_subset[order(tt500_subset$family, tt500_subset$tau), , drop = FALSE]
tt5000_subset <- tt5000_subset[order(tt5000_subset$family, tt5000_subset$tau), , drop = FALSE]

rows <- c(
  build_rows(unresolved, "anchor7_rw_joint", "mcmc_exdqlm_rw_joint_anchor", 2026049110L),
  build_rows(tt500_subset, "tt500_rw_refresh4", "mcmc_exdqlm_rw_refresh_tt500", 2026050110L),
  build_rows(tt5000_subset, "tt5000_rw_joint_long3", "mcmc_exdqlm_rw_joint_tt5000_long", 2026051110L)
)

manifest <- rbind_fill_dynamic_tail7_rw_original288(rows)
if (!nrow(manifest)) {
  stop("dynamic tail7-rw manifest would be empty")
}
...
stage_counts <- stage_counts[order(dynamic_tail7_rw_phase_order_original288[stage_counts$phase]), , drop = FALSE]
