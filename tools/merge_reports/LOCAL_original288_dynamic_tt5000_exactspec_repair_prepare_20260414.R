...
current_selection <- read_current_selection_targets_original288_dynamic_tt5000_exactspec_repair()
baseline_selection <- read_baseline_selection_original288_dynamic_tt5000_exactspec_repair()
...
vb_lookup <- baseline_dynamic_vb_lookup_original288_dynamic_tt5000_exactspec_repair(baseline_selection)
...
for (i in seq_len(nrow(current_selection))) {
  ...
  vb_reference_fit_path <- if (nrow(vb_ref)) {
    resolve_existing_path_original288_dynamic_tt5000_exactspec_repair(vb_ref$selected_fit_path[1])
  } else {
    NA_character_
  }
  ...
  base_seed <- base_seed_from_source_original288_dynamic_tt5000_exactspec_repair(
    source_row = resolved,
    fallback_fit_path = baseline_row$selected_fit_path,
    original_case_key = baseline_row$original_case_key
  )
  seedbank <- seedbank_for_base_row_original288_dynamic_tt5000_exactspec_repair(
    base_seed = base_seed,
  ...
  phase1_rows[[i]] <- build_manifest_rows_original288_dynamic_tt5000_exactspec_repair(
    seedbank = seedbank,
    source_rows = src,
...
utils::write.csv(seedbank_all, file.path(dirname(paths$phase1_source_audit), "LOCAL_original288_dynamic_tt5000_exactspec_repair_seedbank_20260414.csv"), row.names = FALSE)
write_stage_counts_original288_dynamic_tt5000_exactspec_repair(phase1_manifest, paths$phase1_stage_counts)

phase2_inventory <- read_phase2_candidate_pool_original288_dynamic_tt5000_exactspec_repair(
...
write_stage_counts_original288_dynamic_tt5000_exactspec_repair(empty_phase2_manifest, paths$phase2_stage_counts)
