...
best_rows$selection_source_type <- "dynamic_tail7_20260406"
...
best_rows$selection_reason <- ifelse(
  ...
  sprintf(
    "Promote dynamic tail-7 candidate %s from phase %s after improving original baseline from %s to %s.",
  ...
  sprintf(
    "Retain unresolved original baseline because dynamic tail-7 best candidate remained %s.",
    best_rows$best_gate_overall
  )
)
...
if (nrow(selection_update)) {
  ...
  if (length(hit)) {
    ...
    carry2$selection_mode[idx[hit]] <- "promoted_dynamic_tail7"
