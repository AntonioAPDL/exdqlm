...
parse_args_original288_dynamic_tt5000_postfix_repair <- function(args) {
  out <- list()
  for (x in args) {
    if (grepl("^--[^=]+=.*$", x)) {
      key <- sub("^--([^=]+)=.*$", "\\1", x)
...
safe_num_vec_original288_dynamic_tt5000_postfix_repair <- function(x, default) {
  v <- suppressWarnings(as.numeric(x))
  if (!length(v) || any(!is.finite(v))) return(as.numeric(default))
  v
}

compact_fit_original288_dynamic_tt5000_postfix_repair <- function(fit, inference) {
  out <- fit
...
collect_vb_health_original288_dynamic_tt5000_postfix_repair <- function(wrapped,
                                                                           case_id,
                                                                           variant,
                                                                           candidate_path,
  ...
  data.frame(
    ...
    run_time_sec = safe_num_original288_dynamic_tt5000_postfix_repair(wrapped$meta$runtime_sec %||% fit$run.time, NA_real_),
    candidate_path = candidate_path,
    stringsAsFactors = FALSE
  )
}

set_dynamic_ld_options_original288_dynamic_tt5000_postfix_repair <- function(ld_list) {
...
write_row_failure_original288_dynamic_tt5000_postfix_repair <- function(row, row_id, reason) {
  health_row <- data.frame(
    case_id = row$original_case_key,
    variant = run_tag_original288_dynamic_tt5000_postfix_repair(),
    gate_overall = "FAIL",
    healthy = FALSE,
...
args <- parse_args_original288_dynamic_tt5000_postfix_repair(commandArgs(trailingOnly = TRUE))
repo_root <- normalizePath(".", winslash = "/", mustWork = TRUE)
setwd(repo_root)

source("tools/merge_reports/LOCAL_validation_health_gate_common_20260321.R")

manifest_path <- safe_chr_original288_dynamic_tt5000_postfix_repair(
  args$manifest,
  paths_original288_dynamic_tt5000_postfix_repair()$full_manifest
)
row_id <- safe_int_original288_dynamic_tt5000_postfix_repair(args$row_id, NA_integer_)
tag <- safe_chr_original288_dynamic_tt5000_postfix_repair(
  args$tag,
  run_tag_original288_dynamic_tt5000_postfix_repair()
)
force <- as_flag_original288_dynamic_tt5000_postfix_repair(args$force, FALSE)
...
if (isTRUE(row$missing_inputs)) {
  write_row_failure_original288_dynamic_tt5000_postfix_repair(row, row_id, "missing_inputs flag is TRUE in manifest")
...
resolve_fit_original288_dynamic_tt5000_postfix_repair <- function(obj) obj$fit %||% obj

run_dynamic_original288_dynamic_tt5000_postfix_repair <- function() {
  sim_obj <- readRDS(row$sim_output_path)
  baseline_path <- safe_chr_original288_dynamic_tt5000_postfix_repair(row$baseline_fit_path, NA_character_)
  if (is.na(baseline_path) || !nzchar(baseline_path) || !file.exists(baseline_path)) {
    ...
    synth_row <- list(
      selected_fit_path = safe_chr_original288_dynamic_tt5000_postfix_repair(row$source_reference_fit_path, row$baseline_fit_path),
      family = row$family,
      tau = row$tau_label,
      tau_label = row$tau_label,
      fit_size = row$fit_size,
  ...
  bf <- resolve_fit_original288_dynamic_tt5000_postfix_repair(baseline)
  ...
  tryCatch({
    if (file.exists(cfg$fit_path) && !force) {
      ...
      if (identical(row$inference, "vb")) {
        vb_cfg <- cfg$vb %||% list()
        old_ld <- set_dynamic_ld_options_original288_dynamic_tt5000_postfix_repair(vb_cfg$ld %||% list())
        on.exit(if (length(old_ld)) options(old_ld), add = TRUE)
        old_opt <- options(list(
          exdqlm.max_iter = safe_int_original288_dynamic_tt5000_postfix_repair(vb_cfg$max_iter %||% 300L, 300L),
          exdqlm.tol_sigma = safe_num_original288_dynamic_tt5000_postfix_repair(vb_cfg$tol_sigma %||% vb_cfg$tol %||% 0.03, 0.03),
          exdqlm.tol_gamma = safe_num_original288_dynamic_tt5000_postfix_repair(vb_cfg$tol_gamma %||% vb_cfg$tol %||% 0.03, 0.03),
          exdqlm.tol_elbo = safe_num_original288_dynamic_tt5000_postfix_repair(vb_cfg$tol_elbo %||% 1e-6, 1e-6),
          exdqlm.vb.min_iter = safe_int_original288_dynamic_tt5000_postfix_repair(vb_cfg$min_iter %||% 10L, 10L),
          exdqlm.vb.patience = safe_int_original288_dynamic_tt5000_postfix_repair(vb_cfg$patience %||% 3L, 3L),
          exdqlm.vb.allow_elbo_drop = safe_num_original288_dynamic_tt5000_postfix_repair(vb_cfg$allow_elbo_drop %||% 1e-5, 1e-5)
        ))
        ...
        runtime_obj <- system.time({
          fit_obj <- exdqlmLDVB(
            ...
            dim.df = bf$dim.df,
            fix.sigma = FALSE,
            sig.init = safe_num_original288_dynamic_tt5000_postfix_repair(bf$sig.init %||% NA_real_, NA_real_),
            dqlm.ind = identical(row$model, "dqlm"),
            tol = safe_num_original288_dynamic_tt5000_postfix_repair(vb_cfg$tol %||% 0.03, 0.03),
            n.samp = safe_int_original288_dynamic_tt5000_postfix_repair(vb_cfg$n_samp %||% 1000L, 1000L),
            verbose = FALSE
          )
        })
      } else {
        ...
        if (!is.na(row$vb_reference_fit_path) && nzchar(row$vb_reference_fit_path) && file.exists(row$vb_reference_fit_path)) {
          vb_obj <- resolve_fit_original288_dynamic_tt5000_postfix_repair(readRDS(row$vb_reference_fit_path))
        }

        refresh_opts <- list()
        refresh_interval <- safe_int_original288_dynamic_tt5000_postfix_repair(
          mh$laplace_refresh_interval %||% accepted_mh$laplace_refresh$interval,
          NA_integer_
        )
        refresh_start <- safe_int_original288_dynamic_tt5000_postfix_repair(
          mh$laplace_refresh_start %||% accepted_mh$laplace_refresh$start,
          NA_integer_
        )
        refresh_weight <- safe_num_original288_dynamic_tt5000_postfix_repair(
        ...
        default_isvb <- legacy_isvb_default ||
          identical(
            tolower(safe_chr_original288_dynamic_tt5000_postfix_repair(bf$vb.init.method, "")),
            "isvb"
          )
        ...
        call_args <- list(
          ...
          dim.df = bf$dim.df,
          dqlm.ind = identical(row$model, "dqlm"),
          n.burn = safe_int_original288_dynamic_tt5000_postfix_repair(mc_cfg$burn %||% 5000L, 5000L),
          n.mcmc = safe_int_original288_dynamic_tt5000_postfix_repair(mc_cfg$n %||% 20000L, 20000L),
          init.from.isvb = as_flag_original288_dynamic_tt5000_postfix_repair(
            if (source_has_init_from_isvb) mc_cfg[["init_from_isvb"]] else default_isvb,
            default_isvb
          ),
          joint.sample = as_flag_original288_dynamic_tt5000_postfix_repair(mh$joint_sample %||% mh$primary_joint_sample %||% accepted_mh$joint_sample, FALSE),
          mh.proposal = safe_chr_original288_dynamic_tt5000_postfix_repair(mh$proposal %||% mh$primary_proposal %||% accepted_mh$proposal %||% "laplace_rw", "laplace_rw"),
          mh.adapt.interval = safe_int_original288_dynamic_tt5000_postfix_repair(mh$adapt_interval %||% accepted_mh$adapt_interval %||% 50L, 50L),
          mh.target.accept = safe_num_vec_original288_dynamic_tt5000_postfix_repair(mh$target_accept %||% accepted_mh$target_accept %||% c(0.20, 0.45), c(0.20, 0.45)),
          mh.scale.bounds = safe_num_vec_original288_dynamic_tt5000_postfix_repair(mh$scale_bounds %||% accepted_mh$scale_bounds %||% c(0.1, 10), c(0.1, 10)),
          mh.max_scale.step = safe_num_original288_dynamic_tt5000_postfix_repair(mh$max_scale_step %||% accepted_mh$max_scale_step %||% 0.35, 0.35),
          mh.min_burn_adapt = safe_int_original288_dynamic_tt5000_postfix_repair(mh$min_burn_adapt %||% accepted_mh$min_burn_adapt %||% 50L, 50L),
          trace.diagnostics = TRUE,
          trace.every = safe_int_original288_dynamic_tt5000_postfix_repair(mh$trace_every %||% mc_cfg$trace_every %||% accepted_mh$trace_every %||% 50L, 50L),
          verbose = FALSE
        )
        if (source_has_init_from_vb) {
          call_args$init.from.vb <- as_flag_original288_dynamic_tt5000_postfix_repair(mc_cfg[["init_from_vb"]], !is.null(vb_obj))
        }
        vb_init_controls <- list(
          tol = safe_num_original288_dynamic_tt5000_postfix_repair(vb_cfg$tol %||% 0.03, 0.03),
          n.IS = safe_int_original288_dynamic_tt5000_postfix_repair(vb_cfg$n_IS %||% vb_cfg$n_is %||% 200L, 200L),
          n.samp = safe_int_original288_dynamic_tt5000_postfix_repair(vb_cfg$n_samp %||% 1000L, 1000L),
          max_iter = safe_int_original288_dynamic_tt5000_postfix_repair(vb_cfg$max_iter %||% 300L, 300L),
          verbose = FALSE
        )
        if (!legacy_isvb_default) {
          vb_init_controls$method <- safe_chr_original288_dynamic_tt5000_postfix_repair(
        ...
        if ("adapt" %in% names(mh) && !all(is.na(mh[["adapt"]]))) {
          call_args$mh.adapt <- as_flag_original288_dynamic_tt5000_postfix_repair(mh[["adapt"]], TRUE)
        } else if (!is.null(accepted_mh$adapt) && length(accepted_mh$adapt) && !all(is.na(accepted_mh$adapt))) {
          call_args$mh.adapt <- as_flag_original288_dynamic_tt5000_postfix_repair(accepted_mh$adapt, TRUE)
        }
        slice_width <- safe_num_original288_dynamic_tt5000_postfix_repair(mh$slice_width %||% accepted_mh$slice_width, NA_real_)
        slice_max_steps <- safe_int_original288_dynamic_tt5000_postfix_repair(mh$slice_max_steps %||% accepted_mh$slice_max_steps, NA_integer_)
      ...
      wrapped <- list(
        fit = compact_fit_original288_dynamic_tt5000_postfix_repair(fit_obj, row$inference),
        meta = list(
    ...
    case_id <- safe_chr_original288_dynamic_tt5000_postfix_repair(cfg$original_case_key, sprintf("row_%04d", row_id))
    ...
    if (identical(row$inference, "mcmc")) {
      ...
      health_row <- collect_vb_health_original288_dynamic_tt5000_postfix_repair(
        wrapped,
        case_id = case_id,
        variant = tag,
    ...
    draw_idx <- select_draw_indices_original288_dynamic_tt5000_postfix_repair(ncol(draw_mat), 20000L, cfg$fit_seed)
    draw_keep <- draw_mat[, draw_idx, drop = FALSE]
    metric_core <- dynamic_metrics_original288_dynamic_tt5000_postfix_repair(row, sim_obj, draw_keep)
    ...
    metrics_row <- data.frame(
      ...
      inference = row$inference,
      candidate_label = row$candidate_label,
      gate_overall = safe_chr_original288_dynamic_tt5000_postfix_repair(health_row$gate_overall[1], "FAIL"),
      healthy = isTRUE(health_row$healthy[1]),
      runtime_sec = safe_num_original288_dynamic_tt5000_postfix_repair(health_row$run_time_sec[1], safe_num_original288_dynamic_tt5000_postfix_repair(wrapped$meta$runtime_sec, NA_real_)),
    ...
    write_row_failure_original288_dynamic_tt5000_postfix_repair(row, row_id, conditionMessage(e))
  })
}

run_dynamic_original288_dynamic_tt5000_postfix_repair()
