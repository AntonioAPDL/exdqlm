...
source("tools/merge_reports/LOCAL_original288_syncedbase_dynamic_closure_helpers_20260407.R")

repo_root <- normalizePath(".", winslash = "/", mustWork = TRUE)
setwd(repo_root)
paths <- paths_original288_syncedbase_dynamic_closure()
...
source_status <- read_source_status_original288_syncedbase_dynamic_closure()
queue <- subset(source_status, in_scope)
deferred <- read_deferred_inventory_original288_syncedbase_dynamic_closure()
...
spec <- schedule_spec_original288_syncedbase_dynamic_closure()
...
for (i in seq_len(nrow(resolved))) {
  ...
  reference_fit_path <- reference_fit_path_original288_syncedbase_dynamic_closure(
    target_row,
  ...
  cfg <- apply_overrides_original288_syncedbase_dynamic_closure(cfg, spec_row)
  ...
  row_id <- i
  cfg_path <- config_path_original288_syncedbase_dynamic_closure(row_id)
  ensure_dir_original288_syncedbase_rerun(dirname(cfg_path))
  saveRDS(cfg, cfg_path)

  candidate_fit_path <- candidate_fit_path_original288_syncedbase_dynamic_closure(
    target_row$run_root,
  ...
  vb_candidate_fit_path <- vb_candidate_fit_path_original288_syncedbase_dynamic_closure(
    target_row$run_root,
    target_row$model,
    target_row$tau_label,
    spec_row$candidate_label[1]
  ...
  rows[[i]] <- data.frame(
    ...
    phase_order = unname(phase_order_original288_syncedbase_dynamic_closure[spec_row$phase[1]]),
...
stage_counts$phase_order <- unname(phase_order_original288_syncedbase_dynamic_closure[stage_counts$phase])
