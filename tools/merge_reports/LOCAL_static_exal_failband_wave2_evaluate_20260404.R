...
if (!nrow(schedule)) stop("wave-2 schedule subset is empty")
...
stage_summary <- do.call(rbind, lapply(split(schedule, schedule$stage), aggregate_stage))
stage_summary <- stage_summary[order(match(stage_summary$stage, c("sentinel12", "expand20", "full30"))), , drop = FALSE]

candidate_summary <- do.call(rbind, lapply(split(schedule, paste(schedule$stage, schedule$candidate_id, sep = "\r")), aggregate_candidate))
candidate_summary <- candidate_summary[order(
  match(candidate_summary$stage, c("sentinel12", "expand20", "full30")),
...
row_coverage <- do.call(rbind, lapply(split(schedule, paste(schedule$stage, schedule$scope_label, schedule$row_id, sep = "\r")), function(df) {
  data.frame(
    stage = df$stage[1],
...
row_coverage <- row_coverage[order(
  match(row_coverage$stage, c("sentinel12", "expand20", "full30")),
...
remaining <- remaining[order(
  match(remaining$stage, c("sentinel12", "expand20", "full30")),
