...
prepare_script="$out_dir/LOCAL_static_exal_failband_wave4_prepare_20260404.R"
...
rows_tsv="$out_dir/LOCAL_static_exal_failband_wave4_rows_20260404.tsv"
...
if [[ "$mode" == "evaluate" ]]; then
  Rscript "$evaluate_script" ${candidate_filter:+--candidate="$candidate_filter"} ${row_filter:+--row-ids="$row_filter"}
  exit 0
fi
...
launch_rows_tsv="$(mktemp "/tmp/exdqlm_static_failband_wave4_rows_XXXX.tsv")"
trap 'rm -f "$launch_rows_tsv"' EXIT

awk -F'\t' -v candidate_filter="$candidate_filter" -v row_filter="$row_filter" '
  BEGIN {
...
manifest="$out_dir/LOCAL_static_exal_failband_wave4_manifest_$(date '+%Y%m%d_%H%M%S')_${RANDOM}_$$.csv"
fail_log="$out_dir/LOCAL_static_exal_failband_wave4_failures_$(date '+%Y%m%d_%H%M%S')_${RANDOM}_$$.log"
...
run_one() {
  ...
  local log_path="$out_dir/LOCAL_static_exal_failband_wave4_${candidate_id}_${scope_label}_row${row_id}.log"
...
if [[ "$xargs_rc" -ne 0 && "$keep_going" == "0" ]]; then
  echo "failband wave-4 launch aborted due to non-zero exit (rc=$xargs_rc)"
  exit "$xargs_rc"
fi
