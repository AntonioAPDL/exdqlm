...
export REFRESHED288_RUN_TAG="${REFRESHED288_RUN_TAG:-20260430_p90_dynamic72_qdesn_comparable_v3_repair}"
export REFRESHED288_VARIANT_TAG="${REFRESHED288_VARIANT_TAG:-p90_dynamic72_qdesn_comparable_v3_repair}"

# Keep every R worker single-threaded; parallelism comes from one process per
# selected row so each worker maps cleanly to one CPU core.
...
case "$action" in
  ...
  verify-windows|health-smoke|health-full)
    ...
    ;;
  smoke-repair)
    # Rerun only the three repaired smoke rows from the completed v2 failure
    # class: dynamic exDQLM MCMC TT500 at tau 0.50.
    manifest="$repo_root/tools/merge_reports/LOCAL_refreshed288_smoke_manifest_${REFRESHED288_RUN_TAG}.csv"
    if [[ ! -f "$manifest" ]]; then
      echo "Missing v3 smoke manifest: $manifest" >&2
...
Actions:
  prepare        Prepare v3 manifests, apply the TT500 exDQLM MCMC repair overlay, and refresh smoke health.
  apply-overlay  Re-apply the repair overlay to existing v3 smoke/full configs.
  verify-windows Verify canonical dynamic windows against Q-DESN effective tails.
  smoke-repair   Force-run only repaired smoke_dynamic_mcmc FAIL-class rows.
  smoke          Launch all dynamic smoke phases for this v3 tag.
  full           Launch full dynamic phases for this v3 tag.
