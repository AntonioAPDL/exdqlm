...
prepare_script="$out_dir/LOCAL_static_exal_failband_wave7_prepare_20260404.R"
...
rows_tsv="$out_dir/LOCAL_static_exal_failband_wave7_rows_20260404.tsv"
...
launch_rows_tsv="$(mktemp "/tmp/exdqlm_static_failband_wave7_rows_XXXX.tsv")"
...
manifest="$out_dir/LOCAL_static_exal_failband_wave7_manifest_$(date '+%Y%m%d_%H%M%S')_${RANDOM}_$$.csv"
fail_log="$out_dir/LOCAL_static_exal_failband_wave7_failures_$(date '+%Y%m%d_%H%M%S')_${RANDOM}_$$.log"
...
run_one() {
  ...
  local log_path="$out_dir/LOCAL_static_exal_failband_wave7_${candidate_id}_${scope_label}_row${row_id}.log"
...
if [[ "$xargs_rc" -ne 0 && "$keep_going" == "0" ]]; then
  echo "failband wave-7 launch aborted due to non-zero exit (rc=$xargs_rc)"
  exit "$xargs_rc"
fi
