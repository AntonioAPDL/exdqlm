...
session="static-exal-failband-wave3-20260404"
interval="120"
max_checks="360"
...
while true; do
  ...
  echo "=== static failband wave3 monitor ${ts} (check ${check}) ==="

  session_state="NOT_RUNNING"
  if tmux has-session -t "$session" 2>/dev/null; then
    session_state="RUNNING"
  fi
  runner_count="$( { pgrep -af 'LOCAL_static_exal_case_runner_20260323.R.*failband3_' 2>/dev/null || true; } | wc -l | tr -d ' ' )"
