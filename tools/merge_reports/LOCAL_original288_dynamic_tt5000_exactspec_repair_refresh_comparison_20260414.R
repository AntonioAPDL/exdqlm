...
for (i in seq_len(nrow(selected))) {
  ...
  selection$selected_source_type[idx] <- "dynamic_tt5000_exactspec_repair_20260414"
  selection$selected_source_subtype[idx] <- "selected_seed"
  selection$selected_candidate[idx] <- selected$candidate_label[i]
  selection$selected_variant_tag[idx] <- run_tag_original288_dynamic_tt5000_exactspec_repair()
  ...
  selection$improved_over_baseline[idx] <-
    gate_rank_original288_dynamic_tt5000_exactspec_repair(selected$gate_current[i]) <
    gate_rank_original288_dynamic_tt5000_exactspec_repair(old_gate)
  selection$selection_mode[idx] <- "dynamic_tt5000_exactspec_repair_selected"
