...
session="static-exal-failband-wave7-20260404"
interval="120"
max_checks="240"
...
latest_row_log_heartbeat() {
  local latest_line
  latest_line="$(find "$out_dir" -maxdepth 1 -type f -name 'LOCAL_static_exal_failband_wave7_*_row*.log' -printf '%T@ %p\n' 2>/dev/null | sort -nr | head -n1 || true)"
...
while true; do
  ...
  echo "=== static failband wave7 monitor ${ts} (check ${check}) ==="

  session_state="NOT_RUNNING"
  if tmux has-session -t "$session" 2>/dev/null; then
    session_state="RUNNING"
  fi
  runner_count="$( { pgrep -af 'LOCAL_static_exal_case_runner_20260323.R.*(repairmap7_|rowfix7_)' 2>/dev/null || true; } | wc -l | tr -d ' ' )"
  ...
  if [[ -n "$missing_now" && "$missing_now" == "0" && "$session_state" != "RUNNING" && "${runner_count:-0}" == "0" ]]; then
    echo "All failband wave7 rows completed. Exiting monitor."
    exit 0
  fi
