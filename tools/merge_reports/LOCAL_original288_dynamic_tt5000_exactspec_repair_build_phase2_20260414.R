...
if (!nrow(unresolved) || !nrow(phase2_inventory)) {
  ...
  write_stage_counts_original288_dynamic_tt5000_exactspec_repair(empty_manifest, paths$phase2_stage_counts)
  cat("phase2_rows=0\n")
...
for (i in seq_len(nrow(phase2_inventory))) {
  ...
  src <- data.frame(
    ...
    run_root = map_to_current_repo_root_original288_dynamic_tt5000_exactspec_repair(sub("/fits/.*$", "", cand$selected_fit_path)),
  ...
  candidate_label <- sprintf(
    ...
    sanitize_candidate_label_original288_dynamic_tt5000_exactspec_repair(cand$selected_variant_tag)
  )
  phase2_rows[[i]] <- build_manifest_rows_original288_dynamic_tt5000_exactspec_repair(
    seedbank = seedbank,
    source_rows = src,
...
write_stage_counts_original288_dynamic_tt5000_exactspec_repair(phase2_manifest, paths$phase2_stage_counts)
