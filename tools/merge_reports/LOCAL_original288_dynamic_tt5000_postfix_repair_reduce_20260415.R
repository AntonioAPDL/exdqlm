...
parse_args_original288_dynamic_tt5000_postfix_repair_reduce <- function(args) {
  out <- list()
  for (x in args) {
    if (grepl("^--[^=]+=.*$", x)) {
      key <- sub("^--([^=]+)=.*$", "\\1", x)
...
args <- parse_args_original288_dynamic_tt5000_postfix_repair_reduce(commandArgs(trailingOnly = TRUE))
repo_root <- normalizePath(".", winslash = "/", mustWork = TRUE)
setwd(repo_root)

paths <- paths_original288_dynamic_tt5000_postfix_repair()
status_path <- safe_chr_original288_dynamic_tt5000_postfix_repair(args$status, paths$full_manifest_status)
ranking_out <- safe_chr_original288_dynamic_tt5000_postfix_repair(args$ranking_out, paths$full_seed_ranking)
selected_out <- safe_chr_original288_dynamic_tt5000_postfix_repair(args$selected_out, paths$full_selected)
...
done_df$gate_rank <- gate_rank_original288_dynamic_tt5000_postfix_repair(done_df$gate_current)
