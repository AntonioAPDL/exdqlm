...
TAG="original288_dynamic_tt5000_postfix_repair_20260415"
PREPARE_ONLY="0"
DRY_RUN="0"
SKIP_PREPARE="0"
FORCE="0"
...
PATHS_R='source("tools/merge_reports/LOCAL_original288_dynamic_tt5000_postfix_repair_helpers_20260415.R"); x<-paths_original288_dynamic_tt5000_postfix_repair(); cat(x$phase1_manifest,"\n",x$phase2_manifest,"\n",x$full_manifest,"\n",x$phase1_manifest_status,"\n",x$phase2_manifest_status,"\n",x$full_manifest_status,"\n",x$phase1_phase_summary,"\n",x$phase2_phase_summary,"\n",x$full_phase_summary,"\n",x$full_seed_ranking,"\n",x$full_selected,"\n",x$comparison_report,"\n", sep="")'
...
CONSOLE_LOG="$OUT_DIR/LOCAL_original288_dynamic_tt5000_postfix_repair_launcher_console_20260415.log"
mkdir -p "$LOG_DIR"
...
evaluate_manifest() {
  ...
  Rscript "$OUT_DIR/LOCAL_original288_dynamic_tt5000_postfix_repair_evaluate_20260415.R" \
    --manifest="$manifest" \
    --status_out="$status" \
...
reduce_manifest() {
  ...
  local ranking="$2"
  local selected="$3"
  Rscript "$OUT_DIR/LOCAL_original288_dynamic_tt5000_postfix_repair_reduce_20260415.R" \
    --status="$status" \
    --ranking_out="$ranking" \
...
run_manifest_rows() {
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    xargs -a "$ids_file" -P "$max_parallel" -I{} bash -lc '
      ...
      cd "$repo_root"
      Rscript tools/merge_reports/LOCAL_original288_dynamic_tt5000_postfix_repair_run_row_20260415.R \
        --manifest="$manifest" \
        --row_id="$id" \
        --tag="$tag" \
        --force="$force" > "$log" 2>&1
...
if [[ "$SKIP_PREPARE" != "1" ]]; then
  ...
  Rscript "$OUT_DIR/LOCAL_original288_dynamic_tt5000_postfix_repair_prepare_20260415.R" | tee -a "$CONSOLE_LOG"
else
...
Rscript "$OUT_DIR/LOCAL_original288_dynamic_tt5000_postfix_repair_build_phase2_20260415.R" | tee -a "$CONSOLE_LOG"
...
if [[ "$DRY_RUN" != "1" ]]; then
  ...
  Rscript "$OUT_DIR/LOCAL_original288_dynamic_tt5000_postfix_repair_refresh_comparison_20260415.R" | tee -a "$CONSOLE_LOG"
fi
