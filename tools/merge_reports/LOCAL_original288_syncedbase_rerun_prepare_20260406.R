...
for (i in seq_len(nrow(merged))) {
  ...
  selected_obj <- if (selected_fit_needed_original288_syncedbase_rerun(row)) {
    readRDS(row$selected_fit_path)
  } else {
    NULL
  }
  seed <- if (is.null(selected_obj)) {
    2026040600L + i
  } else {
    extract_selected_seed_original288_syncedbase_rerun(selected_obj, 2026040600L + i)
  }
  ...
  rows[[i]] <- data.frame(
    ...
    missing_inputs = !file.exists(source_ctx$baseline_signoff_path) ||
      ...
      !file.exists(row$selected_fit_path),
    block = row$block,
    ...
    baseline_fit_path = row$selected_fit_path,
...
manifest$seed[is.na(manifest$seed)] <- 2026040600L + manifest$row_id[is.na(manifest$seed)]
