...
config_dir <- file.path("tools", "merge_reports", "original288_dynamic_tail8_configs_20260405")
dir.create(config_dir, recursive = TRUE, showWarnings = FALSE)

unresolved <- read_unresolved_dynamic_original288_dynamic_tail8()
vb_sel <- read_dynamic_vb_selection_original288_dynamic_tail8()
seed_map <- full288_seed_map_dynamic_tail8_original288()

if (nrow(unresolved) != 8L) {
  stop(sprintf("expected exactly 8 unresolved dynamic tail cases, found %d", nrow(unresolved)))
}
...
variant_tag_for_config <- function(config_id) {
  switch(
    config_id,
    mcmc_exdqlm_slice_sync = "orig288_dyn_tail8_slice_sync_20260405",
    mcmc_exdqlm_slice_sync_long = "orig288_dyn_tail8_slice_sync_long_20260405",
...
build_rows <- function(df, phase, config_id, seed_offset) {
  ...
  for (i in seq_len(nrow(df))) {
    ...
    cfg_out <- apply_dynamic_tail8_config_original288(base_cfg, config_id)
    ...
    rows[[i]] <- data.frame(
      ...
      block = "dynamic",
      phase = phase,
      phase_order = dynamic_tail8_phase_order_original288[phase],
      config_id = config_id,
      ...
      candidate_fit_path = build_candidate_fit_path_dynamic_tail8_original288(baseline_fit_path, variant_tag),
      ...
      rationale = config_note_dynamic_tail8_original288(config_id),
...
rows <- c(
  build_rows(unresolved, "anchor8_slice_sync", "mcmc_exdqlm_slice_sync", 2026042500L),
  build_rows(tau05_subset, "tau05_long6_slice_sync", "mcmc_exdqlm_slice_sync_long", 2026043500L)
)

manifest <- rbind_fill_dynamic_tail8_original288(rows)
if (!nrow(manifest)) {
  stop("dynamic tail-8 manifest would be empty")
}
...
stage_counts <- stage_counts[order(dynamic_tail8_phase_order_original288[stage_counts$phase]), , drop = FALSE]
