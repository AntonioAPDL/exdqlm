...
parse_args_rhsns_rebuild <- function(args) {
  out <- list()
  for (x in args) {
    if (grepl("^--[^=]+=.*$", x)) {
      key <- sub("^--([^=]+)=.*$", "\\1", x)
...
safe_chr_rhsns_rebuild <- function(x, default = NA_character_) {
  if (is.null(x) || !length(x) || is.na(x[1]) || !nzchar(trimws(as.character(x[1])))) return(default)
  as.character(x[1])
}

safe_num_rhsns_rebuild <- function(x, default = NA_real_) {
  v <- suppressWarnings(as.numeric(x)[1])
  if (is.finite(v)) v else default
}

safe_int_rhsns_rebuild <- function(x, default = NA_integer_) {
  v <- suppressWarnings(as.integer(x)[1])
  if (is.finite(v)) v else default
}

as_flag_rhsns_rebuild <- function(x, default = FALSE) {
  if (is.null(x) || !length(x)) return(default)
  if (is.logical(x)) return(isTRUE(x[1]))
  tolower(as.character(x)[1]) %in% c("1", "true", "yes", "y", "t")
}

safe_rmvnorm_rhsns_rebuild <- function(n, mean, sigma) {
...
collect_vb_health_rhsns_rebuild <- function(wrapped, case_id, variant, candidate_path, vhg_extract_rhs_collapse) {
  ...
  data.frame(
    ...
    run_time_sec = safe_num_rhsns_rebuild(wrapped$meta$runtime_sec %||% fit$run.time, NA_real_),
    candidate_path = candidate_path,
    stringsAsFactors = FALSE
  )
}

compact_fit_rhsns_rebuild <- function(fit, inference) {
  out <- fit
...
compute_static_metrics_rhsns_rebuild <- function(cfg, fit_obj, series_wide, coef_truth, vb_draws = 1000L) {
  ...
  if (identical(cfg$inference, "mcmc")) {
    ...
    beta_draws <- safe_rmvnorm_rhsns_rebuild(vb_draws, beta_mean, as.matrix(fit_obj$qbeta$V))
  }
...
args <- parse_args_rhsns_rebuild(commandArgs(trailingOnly = TRUE))
...
manifest_path <- safe_chr_rhsns_rebuild(args$manifest, NA_character_)
row_id <- safe_int_rhsns_rebuild(args$row_id, NA_integer_)
tag <- safe_chr_rhsns_rebuild(args$tag, run_tag_original288_static_shrink_rhsns_rebuild())
force <- as_flag_rhsns_rebuild(args$force, FALSE)
...
write_failure_row_rhsns_rebuild <- function(reason) {
  out <- data.frame(
    row_id = row_id,
    ts_start = start_ts,
...
if (isTRUE(row$missing_inputs[1])) {
  write_failure_row_rhsns_rebuild("missing_inputs flag is TRUE in manifest")
...
case_id <- safe_chr_rhsns_rebuild(cfg$target_original_case_key, sprintf("static_shrink_rhsns::row_%04d", row_id))

tryCatch({
  if (file.exists(cfg$fit_path) && !force) {
    wrapped <- readRDS(cfg$fit_path)
    fit_obj <- wrapped$fit %||% wrapped
    runtime_sec <- safe_num_rhsns_rebuild(wrapped$meta$runtime_sec %||% fit_obj$run.time, NA_real_)
    ...
    if (identical(cfg$inference, "vb")) {
      runtime_obj <- system.time({
        fit_obj <- exal_static_LDVB(
          ...
          X = X,
          p0 = cfg$tau,
          max_iter = safe_int_rhsns_rebuild(cfg$max_iter, 1000L),
          tol = safe_num_rhsns_rebuild(cfg$tol, 1e-4),
          beta_prior = cfg$beta_prior,
          beta_prior_controls = NULL,
          dqlm.ind = isTRUE(cfg$dqlm_ind),
          n_samp_xi = safe_int_rhsns_rebuild(cfg$n_samp_xi, 200L),
      ...
      options(
        exdqlm.static.mcmc.laplace_refresh_interval = safe_int_rhsns_rebuild(cfg$laplace_refresh_interval, 50L),
        exdqlm.static.mcmc.laplace_refresh_start = safe_int_rhsns_rebuild(cfg$laplace_refresh_start, 333L),
        exdqlm.static.mcmc.laplace_refresh_weight = safe_num_rhsns_rebuild(cfg$laplace_refresh_weight, 0.60)
      )
      on.exit(
        options(
      ...
      runtime_obj <- system.time({
        fit_obj <- exal_static_mcmc(
          ...
          beta_prior_controls = NULL,
          dqlm.ind = isTRUE(cfg$dqlm_ind),
          n.burn = safe_int_rhsns_rebuild(cfg$n_burn, 2000L),
          n.mcmc = safe_int_rhsns_rebuild(cfg$n_mcmc, 1000L),
          thin = safe_int_rhsns_rebuild(cfg$thin, 1L),
          ...
          mh.proposal = safe_chr_rhsns_rebuild(cfg$mh_proposal, "laplace_rw"),
          mh.adapt = as_flag_rhsns_rebuild(cfg$mh_adapt, TRUE),
          slice.width = safe_num_rhsns_rebuild(cfg$slice_width, 0.12),
          slice.max.steps = safe_num_rhsns_rebuild(cfg$slice_max_steps, 80),
          gamma.substeps = safe_int_rhsns_rebuild(cfg$gamma_substeps, 1L),
          p.global.eta.jump = safe_num_rhsns_rebuild(cfg$p_global_eta_jump, 0),
          global.eta.jump.scale = safe_num_rhsns_rebuild(cfg$global_eta_jump_scale, 1),
          trace.diagnostics = TRUE,
          trace.every = safe_int_rhsns_rebuild(cfg$trace_every, 50L),
          verbose = FALSE
        )
      })
    }
    ...
    wrapped <- list(
      fit = compact_fit_rhsns_rebuild(fit_obj, cfg$inference),
      meta = list(
  ...
  metrics_row <- compute_static_metrics_rhsns_rebuild(cfg, fit_for_metrics, series_wide, coef_truth)
  metrics_row <- cbind(
    data.frame(
      row_id = row_id,
      case_id = case_id,
  ...
  if (identical(cfg$inference, "mcmc")) {
    ...
    health_row <- collect_vb_health_rhsns_rebuild(
      wrapped,
      case_id = case_id,
      variant = tag,
      candidate_path = cfg$fit_path,
...
row_out <- data.frame(
  ...
  status = status,
  error = error_msg,
  gate_overall = safe_chr_rhsns_rebuild(health_row$gate_overall[1], "FAIL"),
...
cat(sprintf(
  "[row %d] status=%s gate=%s healthy=%s phase=%s family=%s tau=%s tt=%d model=%s inference=%s profile=%s\n",
  row_id,
  status,
