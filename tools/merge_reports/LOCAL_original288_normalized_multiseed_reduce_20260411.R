...
parse_args_original288_normalized_multiseed_reduce <- function(args) {
  out <- list()
  for (x in args) {
    if (grepl("^--[^=]+=.*$", x)) {
      key <- sub("^--([^=]+)=.*$", "\\1", x)
...
args <- parse_args_original288_normalized_multiseed_reduce(commandArgs(trailingOnly = TRUE))
repo_root <- normalizePath(".", winslash = "/", mustWork = TRUE)
setwd(repo_root)

paths <- paths_original288_normalized_multiseed()
status_path <- safe_chr_original288_normalized_multiseed(args$status, paths$pilot_manifest_status)
ranking_out <- safe_chr_original288_normalized_multiseed(args$ranking_out, paths$pilot_seed_ranking)
selected_out <- safe_chr_original288_normalized_multiseed(args$selected_out, paths$pilot_selected)
...
done_df$gate_rank <- gate_rank_original288_normalized_multiseed(done_df$gate_current)
...
ranked_parts <- lapply(split_rows, function(d) {
  d <- d[order(d$gate_rank, d$crps_rank, d$primary_accuracy_rank, d$runtime_rank, d$seed), , drop = FALSE]
