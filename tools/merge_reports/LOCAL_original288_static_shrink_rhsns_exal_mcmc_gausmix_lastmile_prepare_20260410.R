...
source("tools/merge_reports/LOCAL_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile_helpers_20260410.R")

repo_root <- normalizePath(".", winslash = "/", mustWork = TRUE)
setwd(repo_root)
paths <- paths_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile()
...
schedule <- materialize_schedule_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile()
...
for (i in seq_len(nrow(schedule))) {
  ...
  cfg <- build_row_config_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile(row, repo_root)
  cfg_path <- config_path_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile(cfg$row_id)
...
for (i in seq_len(nrow(manifest))) {
  ...
  if (!identical(cfg$row_id, manifest$row_id[i])) {
    ...
    cfg$config_path <- config_path_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile(manifest$row_id[i])
    cfg$row_status_path <- row_status_path_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile(manifest$row_id[i])
    cfg$health_path <- health_path_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile(manifest$row_id[i])
    cfg$metrics_path <- metrics_path_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile(manifest$row_id[i])
...
stage_counts$phase_order <- unname(phase_order_original288_static_shrink_rhsns_exal_mcmc_gausmix_lastmile[stage_counts$phase])
...
cat(sprintf("gausmix_lastmile_schedule_rows: %d\n", nrow(schedule)))
