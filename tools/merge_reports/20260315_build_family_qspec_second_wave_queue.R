...
state_dir <- if (length(args) >= 2L) normalizePath(args[[2L]], mustWork = FALSE) else '/home/jaguir26/local/state/exdqlm/family_qspec_second_wave_v1'

targets_path <- file.path(repo_root, 'tools', 'merge_reports', '20260315_family_qspec_second_wave_deeper_chain_targets.tsv')
source(file.path(repo_root, 'tools', 'merge_reports', '20260312_family_qspec_v2_common.R'))

targets <- fq_read_tsv(targets_path)
if (!nrow(targets)) stop('No second-wave deeper-chain targets found.', call. = FALSE)
if (!all(tolower(as.character(targets$inference)) == 'mcmc')) {
  stop('Second-wave deeper-chain queue expects only MCMC targets.', call. = FALSE)
}
...
scalar_chr <- function(x, default = NA_character_) {
  ...
  val <- x[[1]]
  if (is.null(val) || !length(val) || is.na(val) || !nzchar(as.character(val))) return(default)
  as.character(val)
}
...
wave_task_state <- function(task_id) {
  if (task_id %in% lock_task_ids) return('running')
  idx <- match(task_id, event_status$task_id)
  if (is.na(idx)) return('not_started')
  last_event <- event_status$last_event[[idx]]
  if (identical(last_event, 'DONE')) return('wave_complete')
  if (identical(last_event, 'FAILED')) return('failed')
  if (identical(last_event, 'START')) return('interrupted')
  'not_started'
}
...
dependency_ready <- function(parent_task_id, targeted_child_ids) {
  ...
  if (!length(child_ids)) return(TRUE)
  targeted <- child_ids[child_ids %in% targeted_child_ids]
  if (!length(targeted)) return(TRUE)
...
if (!nrow(task_from_target)) stop('No model_path manifest rows matched second-wave targets.', call. = FALSE)
...
ensure_unique_task_ids(model_plan, 'second_wave_model_plan')
...
post_plan <- build_wave_rows(
  ...
  note_prefix = 'upstream_second_wave_model_repair'
)
...
signoff_plan <- build_wave_rows(
  ...
  note_prefix = 'upstream_second_wave_postprocess'
)
...
review_plan <- build_wave_rows(
  ...
  note_prefix = 'upstream_second_wave_signoff'
)
...
prior_plan <- build_wave_rows(
  ...
  note_prefix = 'upstream_second_wave_review'
)
...
campaign_plan <- build_wave_rows(
  ...
  note_prefix = 'upstream_second_wave_compare'
)
...
global_plan <- build_wave_rows(
  ...
  note_prefix = 'upstream_second_wave_campaign'
)
...
rownames(queue) <- NULL
ensure_unique_task_ids(queue, 'second_wave_queue')
...
plan_summary <- data.frame(
  ...
  second_wave_target_rows = nrow(targets),
...
queue_path <- file.path(out_dir, '20260315_family_qspec_second_wave_queue.tsv')
queue_summary_path <- file.path(out_dir, '20260315_family_qspec_second_wave_queue_summary.tsv')
plan_summary_path <- file.path(out_dir, '20260315_family_qspec_second_wave_plan_summary.tsv')
fq_write_tsv(queue, queue_path)
fq_write_tsv(summary_df, queue_summary_path)
fq_write_tsv(plan_summary, plan_summary_path)
