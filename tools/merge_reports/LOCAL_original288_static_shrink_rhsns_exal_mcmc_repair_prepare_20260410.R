...
setwd(repo_root)
paths <- paths_original288_static_shrink_rhsns_exal_mcmc_repair()
...
schedule <- materialize_schedule_original288_static_shrink_rhsns_exal_mcmc_repair()
...
for (i in seq_len(nrow(schedule))) {
  ...
  cfg <- build_row_config_original288_static_shrink_rhsns_exal_mcmc_repair(row, repo_root)
  cfg_path <- config_path_original288_static_shrink_rhsns_exal_mcmc_repair(cfg$row_id)
...
for (i in seq_len(nrow(manifest))) {
  ...
  if (!identical(cfg$row_id, manifest$row_id[i])) {
    ...
    cfg$config_path <- config_path_original288_static_shrink_rhsns_exal_mcmc_repair(manifest$row_id[i])
    cfg$row_status_path <- row_status_path_original288_static_shrink_rhsns_exal_mcmc_repair(manifest$row_id[i])
    cfg$health_path <- health_path_original288_static_shrink_rhsns_exal_mcmc_repair(manifest$row_id[i])
    cfg$metrics_path <- metrics_path_original288_static_shrink_rhsns_exal_mcmc_repair(manifest$row_id[i])
...
stage_counts$phase_order <- unname(phase_order_original288_static_shrink_rhsns_exal_mcmc_repair[stage_counts$phase])
...
cat(sprintf("repair_schedule_rows: %d\n", nrow(schedule)))
