...
status <- read_original288_static_shrink_rhsns_exal_mcmc_repair_status(manifest_path = manifest_path, run_tag = tag)
...
phase_summary$phase_order <- unname(phase_order_original288_static_shrink_rhsns_exal_mcmc_repair[phase_summary$phase])
...
utils::write.csv(compare_rebuild, paths$compare_rebuild, row.names = FALSE)
