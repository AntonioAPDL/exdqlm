...
quantiles** across time while keeping a standard state-space setup
(design/evolution matrices and a state vector).

**What’s new in v0.4.0.** We introduce `exdqlmLDVB`, a **Laplace–Delta
variational Bayes** routine for **fast** quantile state-space fitting.
LDVB provides an accurate, low-variance approximation to the posterior
updates with **ELBO monitoring**. The existing IS-VB routine remains
available and unchanged. Optional C++ bridges can still be enabled, but
examples keep the **pure-R path** for portability.
...
#> LDVB converged: 2 iterations, 0.454 seconds
...
- **exAL errors**: governed by **scale** and **skewness**; fixing them
  stabilizes small examples.
...
- **Laplace–Delta VB**: `exdqlmLDVB` for fast quantile state-space
  fitting with exAL errors.
- **ELBO diagnostics** for LDVB (mirrors IS-VB monitoring).
- **Optional C++ bridges** remain available; pure R paths are preserved.
- **Documentation and tests** updated; CRAN checks green across
  platforms.

### LDVB vs IS-VB (when to use)

- **Use LDVB (`exdqlmLDVB`)** when you want **speed** and **stable**
  updates on standard models; monitor ELBO for convergence.
- **Keep IS-VB (`exdqlmISVB`)** as a **baseline** or for cross-checks;
  it may be slower but is a useful reference.
...
#> LDVB converged: 2 iterations, 0.381 seconds
...
This compares the **ELBO tail values** on the same small example. Values
need not match exactly but should be similar.
