...
manifest_path <- resolve_path(get_arg("--manifest", file.path("config", "validation", "qdesn_static_exdqlm_crossstudy_fit_fail_closure_wave_manifest.yaml")), must_work = TRUE)
...
local_baseline_map <- read_csv_safe(file.path(outer_report_root, "tables", "local_baseline_map.csv"))
completed_manifest <- if (file.exists(file.path(outer_report_root, "manifest", "fit_fail_closure_completed.json"))) {
  jsonlite::fromJSON(file.path(outer_report_root, "manifest", "fit_fail_closure_completed.json"))
} else {
  list()
}
...
cat(sprintf("Shared default baseline: %s\n", as.character(completed_manifest$shared_default_baseline %||% "IN_PROGRESS")))
cat("\nStage status:\n")
...
cat("\nLocal baseline map:\n")
if (nrow(local_baseline_map)) {
  utils::write.table(local_baseline_map, row.names = FALSE, quote = FALSE)
} else {
  cat("(no local baseline map yet)\n")
}
