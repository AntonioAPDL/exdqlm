...
al_v3_output_path <- resolve_path(
  get_arg(
    "--al-v3-output",
    file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_tau050_remaining_failed_mcmc_al_v3_grid.csv")
  ),
  must_work = FALSE
)
exal_v3_output_path <- resolve_path(
  get_arg(
    "--exal-v3-output",
    file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_tau050_remaining_failed_mcmc_exal_v3_grid.csv")
  ),
  must_work = FALSE
)
al_canary_output_path <- resolve_path(
  get_arg(
    "--al-canary-output",
    file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_tau050_remaining_failed_mcmc_al_v3_canary_grid.csv")
  ),
  must_work = FALSE
)
exal_canary_output_path <- resolve_path(
  get_arg(
    "--exal-canary-output",
    file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_tau050_remaining_failed_mcmc_exal_v3_canary_grid.csv")
  ),
  must_work = FALSE
)
al_residual_output_path <- resolve_path(
  get_arg(
    "--al-residual-output",
    file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_tau050_remaining_failed_mcmc_al_v3_residual_grid.csv")
  ),
  must_work = FALSE
)
exal_residual_output_path <- resolve_path(
  get_arg(
    ...
    file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_tau050_remaining_failed_mcmc_exal_v3_residual_grid.csv")
  ),
  must_work = FALSE
)
...
exal_v3_root_ids <- c(
  ...
  "root__dynamic__dlm_constV_smallW__gausmix__tau_0p50__lasttt_5000__qdesn_rhs_ns",
  "root__dynamic__dlm_constV_smallW__gausmix__tau_0p50__lasttt_5000__qdesn_ridge",
  ...
  "root__dynamic__dlm_constV_smallW__normal__tau_0p50__lasttt_5000__qdesn_rhs_ns"
)
...
al_v3_grid <- subset_and_write(al_grid, al_v3_root_ids, al_v3_output_path, "AL v3")
exal_v3_grid <- subset_and_write(exal_grid, exal_v3_root_ids, exal_v3_output_path, "EXAL v3")
...
al_residual_grid <- subset_and_write(
  al_v3_grid,
  setdiff(as.character(al_v3_grid$root_id), al_canary_root_ids),
  al_residual_output_path,
  "AL residual"
)
exal_residual_grid <- subset_and_write(
  exal_v3_grid,
  setdiff(as.character(exal_v3_grid$root_id), exal_canary_root_ids),
...
cat(sprintf("al_v3_output=%s\n", al_v3_output_path))
cat(sprintf("al_v3_roots=%d\n", nrow(al_v3_grid)))
...
cat(sprintf("exal_v3_output=%s\n", exal_v3_output_path))
cat(sprintf("exal_v3_roots=%d\n", nrow(exal_v3_grid)))
