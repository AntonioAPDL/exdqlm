...
manifest_rel <- get_arg("--manifest", file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_candidate_last5000_last500_audit_manifest.yaml"))
prepare_only <- has_flag("--prepare-only")
workers_override <- suppressWarnings(as.integer(get_arg("--max-workers", NA)))

manifest <- exdqlm:::qdesn_dynamic_candidate_5000_500_audit_load_manifest(manifest_rel, repo_root = repo_root)
...
run_tag_prefix <- as.character((manifest$meta %||% list())$run_tag_prefix %||% "qdesn-dynamic-candidate-last5000-last500-audit")[1L]
...
if (prepare_only) {
  exdqlm:::qdesn_dynamic_candidate_5000_500_audit_write_outputs(
    inventory = inventory,
...
render_status <- exdqlm:::qdesn_dynamic_candidate_5000_500_audit_render_plots(
  inventory = inventory,
  output_root = output_root,
  manifest = manifest,
  max_workers = max_workers
)
inventory_out <- exdqlm:::qdesn_dynamic_candidate_5000_500_audit_write_outputs(
  inventory = inventory,
...
if (any(!png_ok) || any(nzchar(render_error))) {
  stop(sprintf("Candidate 5000/500 audit completed with rendering problems. See %s", file.path(output_root, "000__dataset_index.csv")), call. = FALSE)
}
cat(sprintf("Candidate 5000/500 audit pack complete: %s\n", output_root))
