...
if (full && !launch_approved) {
  stop("Full repair launch requires --full --launch-approved.", call. = FALSE)
}
...
orchestration_tag <- sprintf(
  "qdesn-tt500-mcmc-metricgap-v3-tau0-repair-orch-%s__git-%s",
  stamp,
  git_short
)
...
if (!skip_materialize) {
  steps[[length(steps) + 1L]] <- run_step(
    "00_materialize",
    "Rscript",
    c("scripts/materialize_qdesn_tt500_mcmc_metricgap_v3_tau0_repair.R")
  )
}
...
if (nrow(targets) != 25L ||
    any(abs(as.numeric(targets$rhs_tau0) - 3e-5) > 1e-12) ||
    anyDuplicated(targets$spec_id)) {
  stop("Repair orchestrator requires 25 unique rhs_tau0=3e-5 target specs.", call. = FALSE)
}
...
if (nrow(smoke_target) != 1L) {
  stop("Could not resolve exactly one repair smoke spec.", call. = FALSE)
}
...
if (!skip_prepare) {
  prepare_tag <- sprintf(
    "qdesn-tt500-mcmc-metricgap-v3-tau0-repair-prepare-%s__git-%s",
    stamp,
    git_short
  )
  steps[[length(steps) + 1L]] <- run_step(
...
if (run_smoke && !skip_smoke) {
  smoke_tag <- sprintf(
    "qdesn-tt500-mcmc-metricgap-v3-tau0-repair-smoke-%s__git-%s",
    stamp,
    git_short
  )
  steps[[length(steps) + 1L]] <- run_step(
...
if (full) {
  run_tag <- as.character(get_arg(
    "--run-tag",
    sprintf(
      "qdesn-tt500-mcmc-metricgap-v3-tau0-repair-full-%s__git-%s",
  ...
  tmux_session <- as.character(get_arg(
    "--tmux-session",
    sprintf("qdesn_tt500_mcmc_metricgap_v3_tau0_repair_%s", format(Sys.time(), "%Y%m%d_%H%M%S"))
  ))[1L]
...
manifest <- list(
  ...
  exact_rhs_tau0 = unique(as.numeric(targets$rhs_tau0)),
...
manifest_path <- file.path(
  orchestration_root,
  "manifest",
  "qdesn_tt500_mcmc_metricgap_v3_tau0_repair_orchestration.json"
)
jsonlite::write_json(
  manifest,
  manifest_path,
