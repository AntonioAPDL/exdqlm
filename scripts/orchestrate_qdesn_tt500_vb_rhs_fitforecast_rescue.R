...
max_p_over_n <- as.character(get_arg("--max-p-over-n", "0.35"))[1L]
git_sha <- trimws(system("git rev-parse --short HEAD", intern = TRUE))
stamp <- format(Sys.time(), "%Y%m%d-%H%M%S")
run_tag <- as.character(get_arg("--run-tag", sprintf("qdesn-vb-rhs-fitforecast-rescue-%s__git-%s", stamp, git_sha)))[1L]
orchestrator_tag <- as.character(get_arg("--orchestrator-tag", sprintf("qdesn-vb-rhs-fitforecast-rescue-orchestrator-%s__git-%s", stamp, git_sha)))[1L]
...
if (isTRUE(do_full) && !isTRUE(launch_approved)) {
  stop("Full RHS fit+forecast rescue launch requires both --full and --launch-approved.", call. = FALSE)
}
...
orchestrator_root <- file.path(
  ...
  "qdesn_tt500_vb_rhs_fitforecast_rescue",
  orchestrator_tag
)
...
run_cmd <- function(label, cmd, args) {
  ...
  cat(sprintf("[rhs-fitforecast-rescue] %s start: %s\n", label, Sys.time()))
  status <- system2(cmd, args = args, stdout = log_path, stderr = log_path)
  cat(sprintf("[rhs-fitforecast-rescue] %s status=%d end: %s\n", label, as.integer(status), Sys.time()))
  as.integer(status)
}
...
materialize_status <- if (isTRUE(skip_materialize)) {
  if (!file.exists(materialization_manifest)) {
    stop("Cannot --skip-materialize because the RHS fit+forecast rescue materialization manifest does not exist.", call. = FALSE)
  }
  cat(sprintf("[rhs-fitforecast-rescue] materialize skipped; using manifest: %s\n", materialization_manifest))
  0L
} else {
  run_cmd(
    label = "materialize",
    cmd = "Rscript",
    args = c(
      file.path("scripts", "materialize_qdesn_tt500_vb_rhs_fitforecast_rescue.R"),
...
if (!identical(materialize_status, 0L)) {
  stop("Q-DESN RHS fit+forecast rescue materialization failed. Inspect orchestrator logs.", call. = FALSE)
}
if (!file.exists(materialization_manifest)) {
...
if (!isTRUE(materialize_only) && !isTRUE(skip_prepare) && (isTRUE(prepare_only) || isTRUE(do_smoke) || isTRUE(do_full))) {
  ...
  if (!identical(prepare_status, 0L)) {
    stop("Q-DESN RHS fit+forecast rescue prepare preflight failed. Inspect orchestrator logs.", call. = FALSE)
  }
}
...
if (isTRUE(do_smoke) && !isTRUE(materialize_only)) {
  ...
  if (!identical(smoke_status, 0L)) {
    stop("Q-DESN RHS fit+forecast rescue smoke failed. Inspect orchestrator logs.", call. = FALSE)
  }
}
...
manifest <- list(
  ...
  stage = "qdesn_vb_rhs_fitforecast_rescue",
  ...
  expected_vb_fits_if_al_exal = expected_roots * 2L,
