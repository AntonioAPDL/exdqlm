...
output_path <- resolve_path(
  get_arg("--output", file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_grid.csv")),
  must_work = FALSE
)
...
cat(sprintf("Scenarios: %s\n", paste(validation$scenarios, collapse = ", ")))
cat(sprintf("Families: %s\n", paste(validation$families, collapse = ", ")))
cat(sprintf("Taus: %s\n", paste(validation$taus, collapse = ", ")))
cat(sprintf("Fit sizes: %s\n", paste(validation$fit_sizes, collapse = ", ")))
