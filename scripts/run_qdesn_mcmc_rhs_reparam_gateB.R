...
results_root <- get_arg(
  "--results-root",
  file.path("results", "qdesn_mcmc_validation", "rhs_reparam_gateB_tauwarm25", run_stub)
)
report_root <- get_arg(
  "--report-root",
  file.path("reports", "qdesn_mcmc_validation", "rhs_reparam_gateB_tauwarm25", run_stub)
)
...
tmp_defaults <- tempfile(pattern = "gateB-defaults-", fileext = ".yaml")
...
if (file.exists(file.path(report_root, "tables", "campaign_root_confirmation.csv"))) {
  ...
  gate_summary <- data.frame(
    ...
    gateB_pass = all(confirm$confirmation_grade != "FAIL"),
...
utils::write.csv(gate_summary, file.path(report_root, "tables", "gateB_summary.csv"), row.names = FALSE)
exdqlm:::.qdesn_validation_write_json(file.path(report_root, "manifest", "gateB_manifest.json"), list(
  ...
  gateB_summary_path = normalizePath(file.path(report_root, "tables", "gateB_summary.csv"), winslash = "/", mustWork = FALSE)
))

cat(sprintf("Gate B report root: %s\n", normalizePath(report_root, winslash = "/", mustWork = FALSE)))
cat(sprintf("Gate B results root: %s\n", normalizePath(results_root, winslash = "/", mustWork = FALSE)))
if (nrow(gate_summary)) {
  cat(sprintf("Gate B pass: %s (fail=%s, warn=%s, pass=%s, max_split_rhat=%.4f)\n",
              as.character(gate_summary$gateB_pass[[1L]]),
