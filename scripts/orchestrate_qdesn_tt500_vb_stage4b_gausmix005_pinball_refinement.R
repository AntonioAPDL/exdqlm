...
run_tag <- as.character(get_arg(
  "--run-tag",
  sprintf("qdesn-tt500-vb-stage4b-gausmix005-pinball-%s__git-%s", stamp, git_sha)
))[1L]
orchestrator_tag <- as.character(get_arg(
  "--orchestrator-tag",
  sprintf("qdesn-tt500-vb-stage4b-gausmix005-pinball-orchestrator-%s__git-%s", stamp, git_sha)
))[1L]
...
orchestrator_root <- file.path(
  ...
  "qdesn_tt500_vb_stage4b_gausmix005_pinball_refinement",
  orchestrator_tag
)
...
materialize_args <- c(
  file.path("scripts", "materialize_qdesn_tt500_vb_stage4b_gausmix005_pinball_refinement.R"),
...
materialize_status <- if (isTRUE(skip_materialize)) {
  if (!file.exists(materialization_manifest)) {
    stop("Cannot --skip-materialize because the Stage 4B materialization manifest does not exist.", call. = FALSE)
  }
  cat(sprintf("[stage4b] materialize skipped; using committed manifest: %s\n", materialization_manifest))
  0L
} else {
  run_cmd("materialize", "Rscript", materialize_args)
}
if (!identical(materialize_status, 0L)) {
  stop("Stage 4B materialization failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (!identical(prepare_status, 0L)) {
  stop("Stage 4B prepare preflight failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (isTRUE(do_smoke)) {
  ...
  if (!identical(smoke_status, 0L)) stop("Stage 4B smoke failed. Inspect orchestrator logs.", call. = FALSE)
}
...
manifest_path <- file.path(orchestrator_root, "manifest", "qdesn_tt500_vb_stage4b_gausmix005_pinball_refinement_orchestrator_manifest.json")
