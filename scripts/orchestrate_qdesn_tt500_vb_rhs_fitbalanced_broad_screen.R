...
workers <- min(int_arg("--workers", 40L), 64L)
...
run_tag <- as.character(get_arg("--run-tag", sprintf("qdesn-vb-rhs-fitbalanced-broad-%s__git-%s", stamp, git_sha)))[1L]
orchestrator_tag <- as.character(get_arg("--orchestrator-tag", sprintf("qdesn-vb-rhs-fitbalanced-broad-orchestrator-%s__git-%s", stamp, git_sha)))[1L]
dry_run <- has_flag("--dry-run")
prepare_only <- has_flag("--prepare-only")
do_smoke <- has_flag("--smoke") && !has_flag("--skip-smoke")
...
orchestrator_root <- file.path(
  ...
  "qdesn_tt500_vb_rhs_fitbalanced_broad",
  orchestrator_tag
)
...
materialize_status <- if (isTRUE(skip_materialize)) {
  if (!file.exists(materialization_manifest)) {
    stop("Cannot --skip-materialize because the fit-balanced broad materialization manifest does not exist.", call. = FALSE)
  }
  cat(sprintf("[rhs-fitbalanced] materialize skipped; using manifest: %s\n", materialization_manifest))
  0L
} else {
  run_cmd(
    label = "materialize",
    cmd = "Rscript",
    args = c(
      file.path("scripts", "materialize_qdesn_tt500_vb_rhs_fitbalanced_broad_screen.R"),
...
if (!identical(materialize_status, 0L)) {
  stop("Q-DESN RHS fit-balanced broad materialization failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (!isTRUE(skip_prepare)) {
  ...
  if (!identical(prepare_status, 0L)) {
    stop("Q-DESN RHS fit-balanced broad prepare preflight failed. Inspect orchestrator logs.", call. = FALSE)
  }
}
...
if (isTRUE(do_smoke)) {
  ...
  if (!identical(smoke_status, 0L)) {
    stop("Q-DESN RHS fit-balanced broad smoke failed. Inspect orchestrator logs.", call. = FALSE)
  }
}
...
manifest <- list(
  ...
  stage = "qdesn_vb_rhs_fitbalanced_broad",
