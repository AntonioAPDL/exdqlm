...
usage() {
  cat <<'EOF'
Usage:
  scripts/cleanup_qdesn_validation_rds_payloads.sh [--execute] [--run-label LABEL] [--scope-root PATH] [--allow-live-sessions]
...
Defaults:
  ...
  - By default scans results/qdesn_mcmc_validation and reports/qdesn_mcmc_validation.
  - --scope-root may be supplied one or more times to restrict cleanup to a
    specific campaign/run subtree.
...
if [[ "${#SCOPE_ROOTS[@]}" -eq 0 ]]; then
  SCOPE_ROOTS=("results/qdesn_mcmc_validation" "reports/qdesn_mcmc_validation")
fi
...
if [[ "${#SEARCH_ROOTS[@]}" -eq 0 ]]; then
  echo "No existing cleanup scope roots were found." >&2
  exit 1
fi
...
done < <(find "${SEARCH_ROOTS[@]}" -type f -path '*/models/forecast_objects.rds' | sort)

while IFS= read -r path; do
  add_delete_candidate "${path}" "rhs_trace"
done < <(find "${SEARCH_ROOTS[@]}" -type f -path '*/models/rhs_trace.rds' | sort)

while IFS= read -r path; do
  add_delete_candidate "${path}" "timing_summary"
done < <(find "${SEARCH_ROOTS[@]}" -type f -path '*/models/timing_summary.rds' | sort)
...
done < <(find "${SEARCH_ROOTS[@]}" -type f -path '*_sources/*' -name 'sim_output.rds' | sort)
...
done < <(find "${SEARCH_ROOTS[@]}" -type f \( -name '*.RData' -o -name '*.rdata' \) | sort)
