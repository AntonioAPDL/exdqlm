...
usage <- function() {
  cat(
    "Usage: scripts/run_qdesn_validation_repair_wave2.R [options]\n\n",
    "Options:\n",
...
write_plan_summary <- function(path, run_tag, git_sha, manifest_path, profiles_df, stages_df) {
  lines <- c(
    "# QDESN Validation Repair Wave 2",
    "",
...
manifest_path <- resolve_path(
  get_arg("--manifest", file.path("config", "validation", "qdesn_validation_repair_wave2_manifest.yaml")),
  must_work = TRUE
)
...
run_tag <- as.character(get_arg(
  "--run-tag",
  sprintf("qdesn-validation-repair-wave2-%s__git-%s", format(Sys.time(), "%Y%m%d-%H%M%S"), git_sha)
))[1L]
...
report_workspace <- resolve_path(
  file.path((cfg$outputs %||% list())$report_root %||% file.path("reports", "qdesn_mcmc_validation", "qdesn_validation_repair_wave2"), run_tag),
  must_work = FALSE
)
results_workspace <- resolve_path(
  file.path((cfg$outputs %||% list())$results_root %||% file.path("results", "qdesn_mcmc_validation", "qdesn_validation_repair_wave2"), run_tag),
  must_work = FALSE
)
...
for (ii in seq_along(stages_cfg)) {
  ...
  stage_screen_manifest <- list(
    meta = list(
      name = sprintf("%s__%s", safe_chr((cfg$meta %||% list())$name, "qdesn_validation_repair_wave2"), stage_id),
...
write_plan_summary(
  path = file.path(summary_dir, "repair_wave2_plan.md"),
  run_tag = run_tag,
  git_sha = git_sha,
...
write_json_safe(
  ...
  file.path(manifest_dir, "repair_wave2_manifest.json")
)

if (!isTRUE(execute)) {
  cat(sprintf("Prepared repair wave 2 workspace: %s\n", report_workspace))
  cat(sprintf("Plan summary: %s\n", file.path(summary_dir, "repair_wave2_plan.md")))
  quit(status = 0)
}
...
for (ii in seq_along(stage_context)) {
  ...
  write_result_summary(
    path = file.path(summary_dir, "repair_wave2_results.md"),
    run_tag = run_tag,
...
write_json_safe(
  ...
  file.path(manifest_dir, "repair_wave2_completed.json")
)

cat(sprintf("Repair wave 2 workspace: %s\n", report_workspace))
cat(sprintf("Plan summary: %s\n", file.path(summary_dir, "repair_wave2_plan.md")))
cat(sprintf("Result summary: %s\n", file.path(summary_dir, "repair_wave2_results.md")))
