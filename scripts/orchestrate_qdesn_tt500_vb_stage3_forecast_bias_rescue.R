...
run_tag <- as.character(get_arg(
  "--run-tag",
  sprintf("qdesn-tt500-vb-stage3-forecast-bias-rescue-%s__git-%s", stamp, git_sha)
))[1L]
orchestrator_tag <- as.character(get_arg(
  "--orchestrator-tag",
  sprintf("qdesn-tt500-vb-stage3-forecast-bias-rescue-orchestrator-%s__git-%s", stamp, git_sha)
))[1L]
...
orchestrator_root <- file.path(
  ...
  "qdesn_tt500_vb_stage3_forecast_bias_rescue",
  orchestrator_tag
)
...
materialize_status <- run_cmd(
  ...
  cmd = "Rscript",
  args = c(
    file.path("scripts", "materialize_qdesn_tt500_vb_stage3_forecast_bias_rescue.R"),
    "--report-root", report_root,
    "--workers", as.character(workers),
    "--target-profiles-per-cell", as.character(target_profiles_per_cell),
    if (isTRUE(refresh_materialized)) "--refresh-materialized" else character(0)
  )
)
if (!identical(materialize_status, 0L)) {
  stop("Stage 3 materialization failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (!identical(prepare_status, 0L)) {
  stop("Stage 3 prepare preflight failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (isTRUE(do_smoke)) {
  ...
  if (!identical(smoke_status, 0L)) {
    stop("Stage 3 smoke failed. Inspect orchestrator logs.", call. = FALSE)
  }
}
...
manifest <- list(
  ...
  report_root_source = report_root,
...
manifest_path <- file.path(orchestrator_root, "manifest", "qdesn_tt500_vb_stage3_forecast_bias_rescue_orchestrator_manifest.json")
