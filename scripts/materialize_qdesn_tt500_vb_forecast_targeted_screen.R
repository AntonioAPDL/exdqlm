...
default_report_root <- file.path(
  "reports", "qdesn_mcmc_validation", "qdesn_dynamic_fitforecast_v2_tt500_vb_hardcell_forecast_refinement",
...
stage_file <- "qdesn_dynamic_fitforecast_v2_tt500_vb_forecast_targeted"
...
source_profiles_path <- resolve_path(
  get_arg("--source-profiles", file.path("config", "validation", "qdesn_dynamic_fitforecast_v2_tt500_vb_hardcell_forecast_refinement_profiles.csv")),
  must_work = TRUE
)
base_defaults_path <- resolve_path(
  get_arg("--base-defaults", file.path("config", "validation", "qdesn_dynamic_fitforecast_v2_tt500_vb_hardcell_forecast_refinement_defaults.yaml")),
  must_work = TRUE
)
...
diagnostic_out <- resolve_path(
  get_arg("--diagnostic-out", file.path(report_root, "diagnostics", "qdesn_tt500_vb_forecast_targeted_screen")),
  must_work = FALSE
)
...
screening_wave <- as.character(get_arg("--screening-wave", paste0("forecast_targeted_", format(Sys.Date(), "%Y_%m_%d"))))[1L]
...
diagnostic_paths <- list(
  cell_plan = file.path(diag_tables, "qdesn_tt500_vb_forecast_targeted_cell_plan.csv"),
  candidate_ledger = file.path(diag_tables, "qdesn_tt500_vb_forecast_targeted_candidate_ledger.csv"),
  selected_profiles = file.path(diag_tables, "qdesn_tt500_vb_forecast_targeted_selected_profiles.csv"),
  cell_assignments = file.path(diag_tables, "qdesn_tt500_vb_forecast_targeted_cell_assignments.csv"),
  summary = file.path(diag_summary, "qdesn_tt500_vb_forecast_targeted_screen.md"),
  manifest = file.path(diag_manifest, "qdesn_tt500_vb_forecast_targeted_screen_manifest.json")
)
...
cell_display <- plan$cell_plan[, intersect(
  c(
    ...
    "current_best_forecast_pinball_ratio", "bottleneck_metric"
  ),
  names(plan$cell_plan)
), drop = FALSE]
summary_lines <- c(
  "# Q-DESN TT500 VB Forecast-Targeted Screen Materialization",
  "",
  ...
  "",
  "This screen is cell-specific: each selected Q-DESN profile is evaluated only on the family x tau cell it was designed to repair. Sentinel cells keep a small guard set so previously-good behavior is not silently lost.",
  "",
  "## Cell Plan",
