...
launch_script <- file.path(repo_root, "scripts", "launch_qdesn_dynamic_exdqlm_crossstudy_validation.R")
...
default_run_tag <- sprintf(
  "qdesn-dynamic-exdqlm-crossstudy-deepdesn-rowfaithseed-canary-%s__git-%s",
...
status <- system2(
  "Rscript",
  c(launch_script, "--defaults", defaults_path, "--grid", grid_path, "--allow-grid-subset", child_args),
  stdout = "",
  stderr = ""
)
