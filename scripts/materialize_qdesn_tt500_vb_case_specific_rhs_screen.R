...
is_fitrmse_followup <- isTRUE(is_fitrmse_v3) || isTRUE(is_fitrmse_v4) || isTRUE(is_fitrmse_v45) || isTRUE(is_fitrmse_v46) || isTRUE(is_fitrmse_v47)
is_fitrmse_v4plus <- isTRUE(is_fitrmse_v4) || isTRUE(is_fitrmse_v45) || isTRUE(is_fitrmse_v46) || isTRUE(is_fitrmse_v47)
screening_stage_label <- if (isTRUE(is_fitrmse_v47)) {
...
screening_stage_stub <- if (isTRUE(is_fitrmse_v47)) {
...
diagnostic_stub <- if (isTRUE(is_fitrmse_v47)) {
...
max_profiles_per_cell <- int_arg("--max-profiles-per-cell", if (isTRUE(is_fitrmse_v47)) 36L else if (isTRUE(is_fitrmse_v45)) 36L else if (isTRUE(is_fitrmse_v3)) 34L else 28L)
...
role_quota <- function(status, target_n) {
  desired <- if (isTRUE(is_fitrmse_v47)) {
    switch(as.character(status)[1L],
...
candidate_pool_for_cell <- function(sub) {
  ...
  anchor_tune <- if (isTRUE(is_fitrmse_v47) && identical(class$bottleneck, "fit_check")) {
    anchor_fit_check
  } else if (isTRUE(is_fitrmse_v47) && identical(class$status, "forecast_mae_hard")) {
    anchor_forecast
  } else {
    anchor_fit
  }

  alpha_grid <- if (isTRUE(is_fitrmse_v47)) {
  ...
  rho_grid <- if (isTRUE(is_fitrmse_v47)) {
  ...
  memory_grid <- if (isTRUE(is_fitrmse_v47)) c(1L, 2L, 3L, 5L, 8L, 10L, 12L, 15L, 20L, 30L, 45L, 60L, 90L, 120L, 150L) else if (isTRUE(is_fitrmse_v46)) c(1L, 2L, 3L, 5L, 8L, 10L, 12L, 15L, 20L, 30L, 45L, 60L, 90L, 120L) else if (isTRUE(is_fitrmse_v45)) c(1L, 2L, 3L, 5L, 8L, 10L, 12L, 15L, 20L, 30L, 45L, 60L, 90L) else if (isTRUE(is_fitrmse_v4)) c(1L, 2L, 3L, 5L, 8L, 10L, 12L, 15L, 20L, 30L, 45L, 60L, 90L) else if (isTRUE(is_fitrmse_v3)) c(3L, 5L, 10L, 15L, 20L, 30L, 45L, 60L, 90L) else c(5L, 10L, 15L, 20L, 30L, 45L, 60L, 90L)
  tau0_grid <- if (isTRUE(is_fitrmse_v47)) c(3e-7, 1e-6, 3e-6, 1e-5, 3e-5, 1e-4, 3e-4, 1e-3, 3e-3, 1e-2, 3e-2) else if (isTRUE(is_fitrmse_v46)) c(1e-6, 3e-6, 1e-5, 3e-5, 1e-4, 3e-4, 1e-3, 3e-3, 1e-2) else if (isTRUE(is_fitrmse_v45)) c(1e-6, 3e-6, 1e-5, 3e-5, 1e-4, 3e-4, 1e-3, 3e-3, 1e-2) else if (isTRUE(is_fitrmse_v4)) c(3e-6, 1e-5, 3e-5, 1e-4, 3e-4, 1e-3, 3e-3) else if (isTRUE(is_fitrmse_v3)) c(1e-5, 3e-5, 1e-4, 3e-4, 1e-3, 3e-3) else c(1e-4, 3e-4, 1e-3, 3e-3)
  sparse_grid <- if (isTRUE(is_fitrmse_v47)) {
    data.frame(
  ...
  local_n_cap <- if (isTRUE(is_fitrmse_v47)) 60L else 50L
  ...
  local_alpha <- nearest_values(anchor_tune$alpha, alpha_grid, if (isTRUE(is_fitrmse_v47)) 7L else 5L)
  local_rho <- nearest_values(anchor_tune$rho, rho_grid, if (isTRUE(is_fitrmse_v47)) 7L else 5L)
  local_m <- sort(unique(c(nearest_values(anchor_tune$m, memory_grid, if (isTRUE(is_fitrmse_v47)) 6L else 4L), 15L, 30L)))
  ...
  if (identical(family, "gausmix") || (identical(family, "laplace") && tau <= 0.05) || (isTRUE(is_fitrmse_v47) && identical(family, "normal") && abs(tau - 0.5) < 1e-8)) {
    local_m <- sort(unique(c(local_m, 60L, 90L)))
  }
  if (isTRUE(is_fitrmse_v47) && (tau <= 0.05 || (identical(family, "normal") && abs(tau - 0.5) < 1e-8))) {
    local_m <- sort(unique(c(local_m, 120L, 150L)))
  }
  local_tau0 <- if (isTRUE(is_fitrmse_v47)) {
  ...
  add_grid("local_fit", local_depth, local_n, local_alpha, local_rho, local_m, local_tau0, sparse_grid[2:min(5L, nrow(sparse_grid)), , drop = FALSE], source = anchor_tune, pool_limit = if (isTRUE(is_fitrmse_v47)) 180L else if (isTRUE(is_fitrmse_v46)) 150L else if (isTRUE(is_fitrmse_v45)) 180L else if (isTRUE(is_fitrmse_v4)) 150L else 120L)
  if (isTRUE(is_fitrmse_v47)) {
    add_grid(
      "fit_check_guardrail",
      unique(c(anchor_fit_check$D, 1L, 2L)),
      unique(c(anchor_fit_check$n_each, 6L, 8L, 10L, 12L, 16L, 20L, 24L, 30L, 40L, 50L)),
      nearest_values(anchor_fit_check$alpha, alpha_grid, 7L),
      nearest_values(anchor_fit_check$rho, rho_grid, 7L),
      unique(c(anchor_fit_check$m, 1L, 2L, 3L, 5L, 8L, 12L, 15L, 20L, 30L, 45L, 60L)),
      sort(unique(c(nearest_values(anchor_fit_check$rhs_tau0, tau0_grid, 7L), 3e-7, 1e-6, 3e-6, 1e-5, 3e-5, 1e-4, 3e-4))),
      sparse_grid[1:min(6L, nrow(sparse_grid)), , drop = FALSE],
      source = anchor_fit_check,
      pool_limit = 160L
    )
  }
  compact_n <- if (isTRUE(is_fitrmse_v47)) c(4L, 6L, 8L, 10L, 12L, 16L, 20L, 24L, 30L) else if (isTRUE(is_fitrmse_v46)) c(4L, 6L, 8L, 10L, 12L, 16L, 20L) else if (isTRUE(is_fitrmse_v45)) c(4L, 6L, 8L, 10L, 12L, 15L, 20L, 25L) else if (isTRUE(is_fitrmse_v4)) c(4L, 6L, 8L, 10L, 12L, 15L, 20L, 25L) else c(8L, 10L, 12L, 15L, 20L, 25L)
  compact_m <- if (isTRUE(is_fitrmse_v47)) c(1L, 2L, 3L, 5L, 8L, 12L, 15L, 20L) else if (isTRUE(is_fitrmse_v46)) c(1L, 2L, 3L, 5L, 8L, 12L, 15L) else if (isTRUE(is_fitrmse_v45)) c(1L, 2L, 3L, 5L, 8L, 15L, 20L) else if (isTRUE(is_fitrmse_v4)) c(1L, 2L, 3L, 5L, 8L, 10L, 15L, 20L) else c(5L, 10L, 15L, 20L, 30L)
  compact_alpha <- alpha_grid[seq_len(if (isTRUE(is_fitrmse_v47)) 14L else if (isTRUE(is_fitrmse_v46)) 12L else if (isTRUE(is_fitrmse_v45)) 10L else if (isTRUE(is_fitrmse_v4)) 12L else 7L)]
  compact_rho <- rho_grid[seq_len(if (isTRUE(is_fitrmse_v47)) 12L else if (isTRUE(is_fitrmse_v46)) 10L else if (isTRUE(is_fitrmse_v45)) 10L else if (isTRUE(is_fitrmse_v4)) 10L else 8L)]
  compact_tau0 <- tau0_grid[seq.int(2L, if (isTRUE(is_fitrmse_v47)) 9L else 7L)]
  add_grid("fit_compact", c(1L, 2L), compact_n, compact_alpha, compact_rho, compact_m, compact_tau0, sparse_grid[1:min(if (isTRUE(is_fitrmse_v47)) 5L else 4L, nrow(sparse_grid)), , drop = FALSE], source = anchor_fit, pool_limit = if (isTRUE(is_fitrmse_v47)) 150L else if (isTRUE(is_fitrmse_v46)) 120L else if (isTRUE(is_fitrmse_v45)) 140L else if (isTRUE(is_fitrmse_v4)) 150L else 120L)
  ...
  memory_tau0 <- if (isTRUE(is_fitrmse_v47)) c(1e-5, 3e-5, 1e-4, 3e-4, 1e-3) else c(1e-4, 3e-4, 1e-3)
  add_grid("memory_guardrail", c(1L, 2L), c(20L, 30L, 40L, if (isTRUE(is_fitrmse_v47)) 50L else integer(0)), c(0.05, 0.10, 0.20, 0.30, 0.40, if (isTRUE(is_fitrmse_v47)) 0.50 else numeric(0)), c(0.60, 0.70, 0.80, 0.85, 0.90, if (isTRUE(is_fitrmse_v47)) 0.95 else numeric(0)), c(45L, 60L, 90L, if (isTRUE(is_fitrmse_v47)) c(120L, 150L) else integer(0)), memory_tau0, sparse_grid[3:min(if (isTRUE(is_fitrmse_v47)) 8L else 5L, nrow(sparse_grid)), , drop = FALSE], source = anchor_forecast, pool_limit = if (isTRUE(is_fitrmse_v47)) 120L else 80L)
  add_grid("forecast_guardrail", unique(c(anchor_forecast$D, 1L, 2L)), unique(c(anchor_forecast$n_each, 20L, 30L, 40L, if (isTRUE(is_fitrmse_v47)) 50L else integer(0))), nearest_values(anchor_forecast$alpha, alpha_grid, if (isTRUE(is_fitrmse_v47)) 7L else 5L), nearest_values(anchor_forecast$rho, rho_grid, if (isTRUE(is_fitrmse_v47)) 7L else 5L), unique(c(anchor_forecast$m, 30L, 60L, 90L, if (isTRUE(is_fitrmse_v47)) c(120L, 150L) else integer(0))), c(1e-5, 3e-5, 1e-4, 3e-4), sparse_grid[3:min(if (isTRUE(is_fitrmse_v47)) 8L else 6L, nrow(sparse_grid)), , drop = FALSE], source = anchor_forecast, pool_limit = if (isTRUE(is_fitrmse_v47)) 120L else 80L)
  seed_anchor <- if (isTRUE(is_fitrmse_v47)) anchor_tune else anchor_fit
  ...
  role_order <- c(anchor = 1L, fit_micro = 2L, fit_compact = 3L, local_fit = 4L, fit_check_guardrail = 5L, shrinkage = 6L, memory_guardrail = 7L, forecast_guardrail = 8L, seed_check = 9L)
  ...
  if (isTRUE(is_fitrmse_v47)) {
    check_roles <- candidates$role_group %in% c("fit_check_guardrail") |
      (identical(class$bottleneck, "fit_check") & candidates$role_group %in% c("local_fit", "seed_check"))
    forecast_roles <- candidates$role_group %in% c("forecast_guardrail", "memory_guardrail") |
...
plan <- list(
  ...
  manifest = list(
    ...
    design = if (isTRUE(is_fitrmse_v47)) {
...
materialized <- exdqlm:::qdesn_dynamic_fitforecast_materialize_forecast_targeted_stage(
  ...
  stage_desc = if (isTRUE(is_fitrmse_v47)) {
...
defaults$screening_profiles$design <- sprintf(
  ...
  if (isTRUE(is_fitrmse_v47)) "case-targeted v4.7 fit-check/blocker follow-up" else if (isTRUE(is_fitrmse_v46)) "case-targeted v4.6 blocker follow-up" else if (isTRUE(is_fitrmse_v45)) "case-targeted v4.5 bottleneck refinement" else if (isTRUE(is_fitrmse_v4)) "case-targeted v4 fit-RMSE/forecast-MAE" else if (isTRUE(is_fitrmse_v3)) "case-targeted v3 fit-RMSE" else "case-specific",
...
summary_lines <- c(
  if (isTRUE(is_fitrmse_v47)) "# Q-DESN 500-Observation VB Case-Targeted RHS v4.7 Screen" else if (isTRUE(is_fitrmse_v46)) "# Q-DESN 500-Observation VB Case-Targeted RHS v4.6 Screen" else if (isTRUE(is_fitrmse_v45)) "# Q-DESN 500-Observation VB Case-Targeted RHS v4.5 Screen" else if (isTRUE(is_fitrmse_v4)) "# Q-DESN 500-Observation VB Case-Targeted RHS v4 Screen" else if (isTRUE(is_fitrmse_v3)) "# Q-DESN 500-Observation VB Case-Targeted RHS v3 Fit-RMSE Screen" else "# Q-DESN 500-Observation VB Case-Specific RHS Screen",
  "",
