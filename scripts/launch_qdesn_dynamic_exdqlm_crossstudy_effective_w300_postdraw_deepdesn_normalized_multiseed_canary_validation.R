...
grid_path <- file.path("config", "validation", "qdesn_dynamic_exdqlm_crossstudy_effective_w300_postdraw_deepdesn_normalized_multiseed_canary_grid.csv")

status <- system2(
  "Rscript",
  c(script_path, "--defaults", defaults_path, "--grid", grid_path, "--allow-grid-subset", args),
  stdout = "",
  stderr = ""
)
