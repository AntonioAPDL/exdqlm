...
tau0_override <- as.character(get_arg("--tau0-override", ""))[1L]
allow_ultra_low_tau0 <- has_flag("--allow-ultra-low-tau0")
if (isTRUE(full) && !isTRUE(launch_approved)) {
  stop("qvbm3 low-tau full launch requires --full --launch-approved.", call. = FALSE)
}
...
orchestrator_tag <- get_arg(
  "--orchestrator-tag",
  sprintf("%s_vb_%s__git-%s", stage_prefix, stamp_short, git_short)
)
orchestrator_root <- resolve_path(file.path("reports", stage_prefix, "orch", orchestrator_tag), must_work = FALSE)
...
bundle_postrun_gate <- function(row, run_tag) {
  ...
  status_files <- list.files(run_root, pattern = "root_status[.]txt$", recursive = TRUE, full.names = TRUE)
  ...
  gate <- list(
    ...
    n_unknown = as.integer(sum(!status_values %in% c("SUCCESS", "FAIL", "RUNNING"))),
  ...
  if (length(status_files) < nrow(target_specs) || gate$n_running > 0L || gate$n_unknown > 0L || gate$n_success < 1L) {
    stop(sprintf("Bundle '%s' post-run gate failed. Gate: %s", as.character(row$bundle_code[[1L]]), gate_path), call. = FALSE)
  }
  gate
}
...
if (!isTRUE(skip_materialize)) {
  steps[[length(steps) + 1L]] <- run_cmd(
    ...
    c(
      ...
      if (nzchar(trimws(tau0_override))) c("--tau0-override", tau0_override) else character(),
      if (isTRUE(allow_ultra_low_tau0)) "--allow-ultra-low-tau0" else character()
    )
  )
}
...
if (length(bundle_filter)) {
  index <- index[as.character(index$bundle_code) %in% bundle_filter | as.character(index$bundle_id) %in% bundle_filter, , drop = FALSE]
  if (!nrow(index)) stop("No bundles remain after --bundles filter.", call. = FALSE)
}
index <- index[order(index$bundle_code), , drop = FALSE]

if (isTRUE(materialize_only) || isTRUE(dry_run)) {
  manifest_path <- write_json(
    list(generated_at = as.character(Sys.time()), mode = if (isTRUE(materialize_only)) "materialize_only" else "dry_run", orchestrator_root = orchestrator_root, index_path = index_path, bundles = index, steps = steps),
    file.path(orchestrator_root, "manifest", "qvbm3_lowtau_orchestrator_manifest.json")
  )
...
run_one_bundle <- function(row, mode = c("prepare", "full")) {
  ...
  run_tag <- sprintf("m3lt%s%s_%s_%s", bundle_code, substr(mode, 1L, 1L), stamp_short, git_short)
  runner_args <- c(
  ...
  label <- sprintf("%s_%s", if (identical(mode, "prepare")) "10p" else "20f", bundle_code)
  out <- run_cmd(label, "Rscript", runner_args)
  if (identical(mode, "full")) out$postrun_gate <- bundle_postrun_gate(row, run_tag)
  out$bundle_code <- bundle_code
  out$mode <- mode
  out$run_tag <- run_tag
  out
}
...
if (isTRUE(prepare_only)) {
  manifest_path <- write_json(
    list(generated_at = as.character(Sys.time()), mode = "prepare_only", orchestrator_root = orchestrator_root, index_path = index_path, bundles = index, steps = steps),
    file.path(orchestrator_root, "manifest", "qvbm3_lowtau_orchestrator_manifest.json")
  )
...
manifest_path <- write_json(
  list(
    ...
    tau0_override = if (nzchar(trimws(tau0_override))) tau0_override else NULL,
...
cat(sprintf("qvbm3 low-tau orchestrator complete. Manifest: %s\n", manifest_path))
