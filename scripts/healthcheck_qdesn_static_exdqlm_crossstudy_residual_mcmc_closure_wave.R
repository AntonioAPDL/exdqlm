...
manifest_path <- resolve_path(get_arg("--manifest", file.path("config", "validation", "qdesn_static_exdqlm_crossstudy_residual_mcmc_closure_wave_manifest.yaml")), must_work = TRUE)
...
completed_manifest <- if (file.exists(file.path(outer_report_root, "manifest", "residual_mcmc_closure_completed.json"))) {
  jsonlite::fromJSON(file.path(outer_report_root, "manifest", "residual_mcmc_closure_completed.json"))
} else {
  list()
}
