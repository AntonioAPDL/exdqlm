...
if (!nzchar(Sys.which("tmux"))) {
  stop("tmux is required for the detached residual-wave launcher.", call. = FALSE)
}

runner_rel <- file.path("scripts", "run_qdesn_static_exdqlm_crossstudy_residual_mcmc_closure_wave.R")
...
if (!is.list(manifest)) {
  stop("Residual closure manifest YAML must parse to a list.", call. = FALSE)
}
...
base_report_root <- resolve_path(
  campaign_cfg$reports_root %||% file.path("reports", "qdesn_mcmc_validation", "static_exdqlm_crossstudy_residual_mcmc_closure_wave"),
  must_work = FALSE
)
...
session_name <- as.character(get_arg(
  "--tmux-session",
  sprintf("qdesn_resid_%s", format(Sys.time(), "%m%d_%H%M%S"))
))[1L]
...
if (!identical(as.integer(launch_status), 0L)) {
  stop("Failed to launch detached residual closure wave.", call. = FALSE)
}
