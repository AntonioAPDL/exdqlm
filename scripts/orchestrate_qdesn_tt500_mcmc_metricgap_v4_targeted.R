...
if (full && !launch_approved) {
  stop("Full v4 targeted launch requires --full --launch-approved.", call. = FALSE)
}
...
orchestration_tag <- sprintf(
  "qdesn-tt500-mcmc-metricgap-v4-targeted-orch-%s__git-%s",
  stamp,
  git_short
)
...
if (!skip_materialize) {
  steps[[length(steps) + 1L]] <- run_step(
    "00_materialize",
    "Rscript",
    c("scripts/materialize_qdesn_tt500_mcmc_metricgap_v4_targeted.R", "--workers", as.character(workers))
  )
}
...
if (nrow(targets) != 75L ||
    length(unique(as.character(targets$spec_id))) != 75L ||
    as.integer(defaults$study_contract$budget$mcmc_n_burn) != 2000L ||
    as.integer(defaults$study_contract$budget$mcmc_n_mcmc) != 8000L ||
    as.integer(materialization$counts$target_mcmc_atomic_specs) != 75L ||
    as.character(materialization$launch_status) != "prepared_not_launched") {
  stop("V4 targeted materialization does not certify the exact 75-spec screening contract.", call. = FALSE)
}
...
smoke_target <- targets[
  as.character(targets$screening_profile_id.x) %in% smoke_profiles &
    as.character(targets$family.x) == as.character(defaults$smoke$family)[1L] &
    abs(as.numeric(targets$tau.x) - as.numeric(defaults$smoke$tau)[1L]) <= 1e-8,
  ,
  drop = FALSE
]
if (nrow(smoke_target) != 1L) {
  stop("Could not resolve exactly one v4 targeted smoke spec.", call. = FALSE)
}
...
if (!skip_prepare) {
  prepare_tag <- sprintf(
    "qdesn-tt500-mcmc-metricgap-v4-targeted-prepare-%s__git-%s",
    stamp,
    git_short
  )
  steps[[length(steps) + 1L]] <- run_step(
...
if (run_smoke) {
  smoke_tag <- sprintf(
    "qdesn-tt500-mcmc-metricgap-v4-targeted-smoke-%s__git-%s",
    stamp,
    git_short
  )
  steps[[length(steps) + 1L]] <- run_step(
...
if (full) {
  if (worktree_dirty()) {
    stop(
      "Full v4 targeted launch requires a clean committed worktree. Commit materialized artifacts, then rerun with --skip-materialize.",
      call. = FALSE
    )
  }
  run_tag <- as.character(get_arg(
    "--run-tag",
    sprintf(
      "qdesn-tt500-mcmc-metricgap-v4-targeted-full-%s__git-%s",
  ...
  tmux_session <- as.character(get_arg(
    "--tmux-session",
    sprintf("qdesn_tt500_mcmc_metricgap_v4_targeted_%s", format(Sys.time(), "%Y%m%d_%H%M%S"))
  ))[1L]
...
orchestration_manifest <- list(
  ...
  unresolved_threshold = as.numeric(defaults$study_contract$screening_policy$unresolved_threshold),
...
orchestration_manifest_path <- file.path(
  orchestration_root,
  "manifest",
  "qdesn_tt500_mcmc_metricgap_v4_targeted_orchestration.json"
)
jsonlite::write_json(
