...
workers <- min(int_arg("--workers", 24L), 40L)
max_profiles <- int_arg("--max-profiles", 48L)
git_sha <- trimws(system("git rev-parse --short HEAD", intern = TRUE))
stamp <- format(Sys.time(), "%Y%m%d-%H%M%S")
run_tag <- as.character(get_arg("--run-tag", sprintf("qdesn-tt500-vb-rhs-optimization-%s__git-%s", stamp, git_sha)))[1L]
orchestrator_tag <- as.character(get_arg("--orchestrator-tag", sprintf("qdesn-tt500-vb-rhs-optimization-orchestrator-%s__git-%s", stamp, git_sha)))[1L]
...
orchestrator_root <- file.path(repo_root, "reports", "qdesn_mcmc_validation", "qdesn_tt500_vb_rhs_optimization", orchestrator_tag)
...
materialize_status <- if (isTRUE(skip_materialize)) {
  if (!file.exists(materialization_manifest)) {
    stop("Cannot --skip-materialize because the RHS optimization materialization manifest does not exist.", call. = FALSE)
  }
  cat(sprintf("[rhs-opt] materialize skipped; using committed manifest: %s\n", materialization_manifest))
  0L
} else {
  run_cmd(
    label = "materialize",
    cmd = "Rscript",
    args = c(
      file.path("scripts", "materialize_qdesn_tt500_vb_rhs_optimization_screen.R"),
...
if (!identical(materialize_status, 0L)) {
  stop("RHS optimization materialization failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (!identical(prepare_status, 0L)) {
  stop("RHS optimization prepare preflight failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (isTRUE(do_smoke)) {
  ...
  if (!identical(smoke_status, 0L)) {
    stop("RHS optimization smoke failed. Inspect orchestrator logs.", call. = FALSE)
  }
}
