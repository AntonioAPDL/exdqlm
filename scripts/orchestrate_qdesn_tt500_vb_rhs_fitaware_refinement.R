...
run_tag <- as.character(get_arg(
  "--run-tag",
  sprintf("qdesn-tt500-vb-rhs-fitaware-refinement-%s__git-%s", stamp, git_sha)
))[1L]
orchestrator_tag <- as.character(get_arg(
  "--orchestrator-tag",
  sprintf("qdesn-tt500-vb-rhs-fitaware-refinement-orchestrator-%s__git-%s", stamp, git_sha)
))[1L]
...
orchestrator_root <- file.path(
  ...
  "qdesn_tt500_vb_rhs_fitaware_refinement",
  orchestrator_tag
)
...
materialize_args <- c(
  file.path("scripts", "materialize_qdesn_tt500_vb_rhs_fitaware_refinement.R"),
...
materialize_status <- if (isTRUE(skip_materialize)) {
  if (!file.exists(materialization_manifest)) {
    stop("Cannot --skip-materialize because the RHS fit-aware materialization manifest does not exist.",
         call. = FALSE)
  }
  cat(sprintf("[rhs-fitaware] materialize skipped; using committed manifest: %s\n", materialization_manifest))
  0L
} else {
  run_cmd("materialize", "Rscript", materialize_args)
}
if (!identical(materialize_status, 0L)) {
  stop("Q-DESN RHS fit-aware materialization failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (!identical(prepare_status, 0L)) {
  stop("Q-DESN RHS fit-aware prepare preflight failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (isTRUE(do_smoke)) {
  ...
  if (!identical(smoke_status, 0L)) stop("Q-DESN RHS fit-aware smoke failed. Inspect orchestrator logs.", call. = FALSE)
}
...
if (isTRUE(do_full) && !isTRUE(prepare_only)) {
  ...
  if (identical(run_status, 0L) && !is.na(campaign_report_root) && !isTRUE(skip_rank)) {
    generic_rank_status <- run_cmd(
      "generic_rank",
      "Rscript",
      c(file.path("scripts", "rank_qdesn_tt500_vb_screen.R"), "--report-root", campaign_report_root, "--top-n", "20")
    )
...
manifest <- list(
  ...
  stage = "qdesn_vb_rhs_fitaware_refinement",
...
manifest_path <- file.path(orchestrator_root, "manifest", "qdesn_tt500_vb_rhs_fitaware_refinement_orchestrator_manifest.json")
