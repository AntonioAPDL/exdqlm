...
# Restored for the Q-DESN fit+forecast 0.5.0 validation branch from
# /data/jaguir26/local/src/exdqlm__wt__qdesn_0p4p0_integration at commit
# c232d457463e007d473a3fe9b2469e70c3a1ab2a ("Avoid persisting mcmc vb init artifacts").
# This is the pre-launch compatibility entrypoint expected by R/run_esn_pipeline.R.
...
if (identical(readout_input_mode_effective, "dlm_decomp_lags")) {
  ...
  if (!is.null(decomp_state)) {
    ...
    if (length(structured_fit) == nrow(y_full) && all(is.finite(structured_fit))) {
      decomp_fit_plot_df <- tibble::tibble(
        t = as.numeric(y_full$t),
        y_obs = as.numeric(y_full$y),
        y_dlm = as.numeric(structured_fit)
      )
    } else {
      log_msg(
...
# ================================================================
# 3d) DLM structured fit diagnostics (full + last-window)
# ================================================================
...
# ================================================================
# 3e) Overlapping forecast fan charts (95% bands)
# ================================================================
...
# ================================================================
# 3f) Posterior parameter plots: γ, σ histograms + β forest
# ================================================================
...
if (isTRUE(save_outputs)) {
  ...
  runtime_summary <- list(
    ...
    likelihood_family = likelihood_family,
    beta_prior_type = vb_prior_beta_type,
    started_at = as.character(pipeline_start_time),
    finished_at = as.character(Sys.time()),
    elapsed_seconds = as.numeric(difftime(Sys.time(), pipeline_start_time, units = "secs")),
    T_use = as.integer(T_use),
  ...
  jsonlite::write_json(
    list(
      ...
      inputs = list(file_long = file_long),
