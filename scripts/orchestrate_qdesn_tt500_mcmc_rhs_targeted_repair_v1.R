...
materialization_manifest_path <- resolve_path(file.path("config", "validation", paste0(stage_file, "_materialization_manifest.json")), must_work = FALSE)
...
if (!file.exists(target_specs_path)) {
  stop(sprintf("Missing target specs file: %s", target_specs_path), call. = FALSE)
}
...
defaults_for_smoke <- if (file.exists(defaults_path)) yaml::read_yaml(defaults_path) else list()
smoke_cfg <- defaults_for_smoke$smoke %||% list()
smoke_rows <- target_specs
if (!is.null(smoke_cfg$family) && "family.x" %in% names(smoke_rows)) {
  smoke_rows <- smoke_rows[as.character(smoke_rows$family.x) == as.character(smoke_cfg$family)[1L], , drop = FALSE]
}
if (!is.null(smoke_cfg$tau) && "tau.x" %in% names(smoke_rows)) {
  smoke_rows <- smoke_rows[abs(as.numeric(smoke_rows$tau.x) - as.numeric(smoke_cfg$tau)[1L]) < 1e-8, , drop = FALSE]
}
if (!is.null(smoke_cfg$screening_profile_ids) && "screening_profile_id.x" %in% names(smoke_rows)) {
  smoke_profiles <- as.character(unlist(smoke_cfg$screening_profile_ids, use.names = FALSE))
  smoke_rows <- smoke_rows[as.character(smoke_rows$screening_profile_id.x) %in% smoke_profiles, , drop = FALSE]
}
if (!nrow(smoke_rows)) {
  smoke_rows <- target_specs[1L, , drop = FALSE]
}
smoke_spec_id <- as.character(smoke_rows$spec_id[[1L]])
...
if (!is.null(full_step)) {
  cat("full_launch: detached\n")
}
