...
audit <- exdqlm:::qdesn_dynamic_fitforecast_write_campaign_audit(
  ...
  out_dir = out_dir,
  expected_roots = int_arg("--expected-roots", 648L),
...
if (isTRUE(prune_rhs_trace)) {
  cleanup <- exdqlm:::qdesn_dynamic_fitforecast_prune_success_rhs_trace(
    results_root = summary$results_root[[1L]],
    dry_run = prune_dry_run
  )
  cleanup_dir <- file.path(dirname(audit$output_paths$summary), "..", "cleanup")
  cleanup_dir <- normalizePath(cleanup_dir, winslash = "/", mustWork = FALSE)
  dir.create(cleanup_dir, recursive = TRUE, showWarnings = FALSE)
  cleanup_summary <- file.path(cleanup_dir, "qdesn_tt500_vb_rhs_trace_cleanup_summary.csv")
  cleanup_roots <- file.path(cleanup_dir, "qdesn_tt500_vb_rhs_trace_cleanup_roots.csv")
  utils::write.csv(cleanup$summary, cleanup_summary, row.names = FALSE)
  utils::write.csv(cleanup$root_cleanup, cleanup_roots, row.names = FALSE)
  cat(sprintf("rhs_trace_cleanup_summary: %s\n", cleanup_summary))
  cat(sprintf("rhs_trace_cleanup_roots: %s\n", cleanup_roots))
  cat(sprintf(
    "rhs_trace_cleanup eligible=%d pruned=%d dry_run=%s\n",
    as.integer(cleanup$summary$eligible[[1L]]),
    as.integer(cleanup$summary$pruned[[1L]]),
    as.character(cleanup$summary$dry_run[[1L]])
  ))
}
