...
stage_file <- "qdesn_dynamic_fitforecast_v2_tt500_vb_stage3_forecast_bias_rescue"
...
source_profiles_path <- resolve_path(
  get_arg("--source-profiles", file.path("config", "validation", "qdesn_dynamic_fitforecast_v2_tt500_vb_forecast_targeted_profiles.csv")),
  must_work = TRUE
)
base_defaults_path <- resolve_path(
  get_arg("--base-defaults", file.path("config", "validation", "qdesn_dynamic_fitforecast_v2_tt500_vb_forecast_targeted_defaults.yaml")),
  must_work = TRUE
)
...
diagnostic_out <- resolve_path(
  get_arg("--diagnostic-out", file.path(report_root, "diagnostics", "qdesn_tt500_vb_stage3_forecast_bias_rescue")),
  must_work = FALSE
)
...
target_profiles_per_cell <- int_arg("--target-profiles-per-cell", 48L)
refresh_grid <- !has_flag("--no-refresh-grid")
refresh_materialized <- has_flag("--refresh-materialized")
screening_wave <- as.character(get_arg("--screening-wave", paste0("stage3_forecast_bias_", format(Sys.Date(), "%Y_%m_%d"))))[1L]
...
diagnostic_paths <- list(
  cell_plan = file.path(diag_tables, "qdesn_tt500_vb_stage3_cell_plan.csv"),
  candidate_ledger = file.path(diag_tables, "qdesn_tt500_vb_stage3_candidate_ledger.csv"),
  selected_profiles = file.path(diag_tables, "qdesn_tt500_vb_stage3_selected_profiles.csv"),
  cell_assignments = file.path(diag_tables, "qdesn_tt500_vb_stage3_cell_assignments.csv"),
  summary = file.path(diag_summary, "qdesn_tt500_vb_stage3_forecast_bias_rescue.md"),
  manifest = file.path(diag_manifest, "qdesn_tt500_vb_stage3_forecast_bias_rescue_manifest.json")
)
...
cell_display <- plan$cell_plan[, intersect(
  c(
    ...
    "current_best_forecast_pinball_ratio", "n_profiles_evaluated",
    "n_profiles_beating_all_primary", "bottleneck_metric"
  ),
...
summary_lines <- c(
  ...
  "",
  "This lane is intentionally narrow: it only runs cells with no Q-DESN VB profile beating the best DQLM/exDQLM VB baseline on all four primary metrics. The spec grid emphasizes lower-inertia, short-memory, input-responsive reservoirs to diagnose and repair rolling-origin forecast bias.",
  "",
  "## Cell Plan",
...
materialized <- exdqlm:::qdesn_dynamic_fitforecast_materialize_forecast_targeted_stage(
  ...
  stage_desc = "Q-DESN TT500 VB Stage 3 forecast-bias rescue for cells still trailing the best DQLM/exDQLM VB baseline.",
