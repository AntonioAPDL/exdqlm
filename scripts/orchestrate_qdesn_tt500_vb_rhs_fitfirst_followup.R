...
orchestrator_root <- file.path(
  ...
  "qdesn_tt500_vb_rhs_fitfirst_followup",
  orchestrator_tag
)
...
run_cmd <- function(label, cmd, args) {
  ...
  cat(sprintf("[rhs-fitfirst-followup] %s start: %s\n", label, Sys.time()))
  status <- system2(cmd, args = args, stdout = log_path, stderr = log_path)
  cat(sprintf("[rhs-fitfirst-followup] %s status=%d end: %s\n", label, as.integer(status), Sys.time()))
  as.integer(status)
}
...
materialize_status <- if (isTRUE(skip_materialize)) {
  if (!file.exists(materialization_manifest)) {
    stop("Cannot --skip-materialize because the RHS fit-first follow-up materialization manifest does not exist.", call. = FALSE)
  }
  cat(sprintf("[rhs-fitfirst-followup] materialize skipped; using manifest: %s\n", materialization_manifest))
  0L
} else {
  run_cmd(
    label = "materialize",
...
if (!identical(materialize_status, 0L)) {
  stop("Q-DESN RHS fit-first follow-up materialization failed. Inspect orchestrator logs.", call. = FALSE)
}
if (!file.exists(materialization_manifest)) {
...
if (!isTRUE(materialize_only) && !isTRUE(skip_prepare) && (isTRUE(prepare_only) || isTRUE(do_smoke) || isTRUE(do_full))) {
  ...
  if (!identical(prepare_status, 0L)) {
    stop("Q-DESN RHS fit-first follow-up prepare preflight failed. Inspect orchestrator logs.", call. = FALSE)
  }
}
...
if (isTRUE(do_smoke) && !isTRUE(materialize_only)) {
  ...
  if (!identical(smoke_status, 0L)) {
    stop("Q-DESN RHS fit-first follow-up smoke failed. Inspect orchestrator logs.", call. = FALSE)
  }
}
