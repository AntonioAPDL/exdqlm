test_that("tau050 failed-mcmc sfreeze defaults encode the latent-s crash-only relaunch contract", {
  ...
  defaults <- exdqlm:::qdesn_dynamic_crossstudy_load_defaults(file.path(
    ...
    "config",
    "validation",
    "qdesn_dynamic_exdqlm_crossstudy_tau050_failed_mcmc_sfreeze_defaults.yaml"
  ))
  ...
  expect_identical(
    ...
    "qdesn_dynamic_exdqlm_crossstudy_tau050_failed_mcmc_sfreeze_validation"
  )
  expect_identical(as.character(defaults$study_contract$id), "tau050_failed_mcmc_sfreeze")
  ...
  expect_true(isTRUE(cfg$inference$mcmc$latent_s$enabled))
  expect_identical(as.integer(cfg$inference$mcmc$latent_s$freeze_burnin_iters), 50L)
  expect_true(isTRUE(cfg$inference$mcmc$latent_s$freeze_only_during_burn))
  expect_identical(as.integer(cfg$inference$mcmc$latent_s$sparse_update_every), 10L)
  expect_identical(as.integer(cfg$inference$mcmc$latent_s$sparse_update_until_iter), 500L)
  expect_true(isTRUE(cfg$inference$mcmc$latent_s$force_first_postwarmup_update))
  expect_true(isTRUE(cfg$inference$mcmc$latent_s$trace))
})

test_that("tau050 failed-mcmc sfreeze defaults accept the audited AL and EXAL crash-only grids", {
  repo_root <- normalizePath(pkgload::pkg_path(), winslash = "/", mustWork = TRUE)
  defaults <- exdqlm:::qdesn_dynamic_crossstudy_load_defaults(file.path(
    repo_root,
    "config",
