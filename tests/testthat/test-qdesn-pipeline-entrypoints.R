test_that("Q-DESN compatibility pipeline entrypoints are present", {
  ...
  sim_lines <- paste(readLines(sim_path, warn = FALSE), collapse = "\n")
  real_lines <- paste(readLines(real_path, warn = FALSE), collapse = "\n")
  ...
  expect_match(sim_lines, "run_manifest.json")
  expect_match(real_lines, "run_manifest.json")
  expect_match(sim_lines, "digits = NA", fixed = TRUE)
  expect_match(real_lines, "digits = NA", fixed = TRUE)
