test_that("tau050 failed-mcmc thetafreeze defaults encode the theta-plus-tau crash-only relaunch contract", {
  ...
  expect_identical(as.integer(defaults$pipeline$inference$vb$rhs$freeze_tau_warmup_iters), 50L)
  expect_true(isTRUE(defaults$pipeline$inference$vb$rhs$force_tau_after_warmup))
  expect_identical(as.integer(defaults$pipeline$inference$vb$rhs$min_tau_updates), 2L)
  ...
  expect_true(isTRUE(cfg$inference$mcmc$vb_warm_start_control$rhs$force_tau_after_warmup))
  expect_identical(as.integer(cfg$inference$mcmc$vb_warm_start_control$rhs$min_tau_updates), 2L)
