...
qdesn_fgav1_stage <- "qdesn_dynamic_fitforecast_v2_500obs_forecast_gap_adaptive_mcmc_v1"
qdesn_fgav1_branch <- "validation/qdesn-forecast-gap-adaptive-mcmc-v1-1.0.0"
qdesn_fgav1_base_commit <- "8e8000af40661526062c5e20279dc82e056a29b6"
qdesn_fgav1_article_interface <- file.path(
...
.qdesn_fgav1_profile_schema <- function(x) {
  missing <- setdiff(qdesn_fgav1_profile_fields, names(x))
  for (field in missing) x[[field]] <- NA
  x[, qdesn_fgav1_profile_fields, drop = FALSE]
}

qdesn_fgav1_target_contract <- function() {
  data.frame(
    target_cell_id = c(
      "al_normal_t0p05", "al_normal_t0p50", "exal_normal_t0p50",
      "exal_gausmix_t0p50", "al_gausmix_t0p50",
      "exal_normal_t0p25", "al_gausmix_t0p05", "exal_gausmix_t0p05"
    ),
    tier = c(rep("A", 5L), rep("B", 3L)),
    likelihood_target = c(
      "al", "al", "exal", "exal", "al", "exal", "al", "exal"
    ),
    family = c(
      "normal", "normal", "normal", "gausmix", "gausmix",
      "normal", "gausmix", "gausmix"
    ),
    tau = c(.05, .50, .50, .50, .50, .25, .05, .05),
    objective_metric = c(
      rep("forecast_qtrue_mae_H1000", 6L),
      "forecast_check_loss_H1000", "forecast_check_loss_H1000"
    ),
    target_metrics = c(
      rep(paste(qdesn_fgav1_promotion_metrics, collapse = ";"), 5L),
      "forecast_qtrue_mae_H1000",
      paste(qdesn_fgav1_promotion_metrics, collapse = ";"),
      "forecast_check_loss_H1000"
    ),
    candidates_per_cell = c(rep(12L, 5L), rep(8L, 3L)),
    discovery_sources = 2L,
...
qdesn_fgav1_interface <- function(repo_root) {
  path <- qdesn_ssv2_path(repo_root, qdesn_fgav1_article_interface,
                          must_work = TRUE)
  x <- qdesn_ssv2_read_csv(path)
  expected <- c(
    "inference", "model_variant", "family", "tau",
    qdesn_fgav1_target_metrics, "source_registry_hash_value",
    "preprocessing_scope"
  )
  missing <- setdiff(expected, names(x))
  if (length(missing)) {
    stop(sprintf("The v7 interface is missing: %s", paste(missing, collapse = ", ")),
         call. = FALSE)
  }
  if (!identical(unique(x$source_registry_hash_value), qdesn_ssv2_registry_hash)) {
    stop("The v7 interface does not use the frozen source registry.", call. = FALSE)
  }
  x
}

qdesn_fgav1_metric_role_ledger <- function(repo_root) {
  contract <- qdesn_fgav1_target_contract()
  interface <- qdesn_fgav1_interface(repo_root)
  q <- interface[
  ...
  for (i in seq_len(nrow(contract))) {
    ...
    metrics <- strsplit(spec$target_metrics, ";", fixed = TRUE)[[1L]]
    for (metric in metrics) {
      fields <- .qdesn_fgav1_metric_source_fields(metric)
      k <- k + 1L
      rows[[k]] <- data.frame(
        target_cell_id = spec$target_cell_id,
        tier = spec$tier,
        likelihood_target = spec$likelihood_target,
        family = spec$family,
        tau = spec$tau,
        metric = metric,
        promotion_priority = ifelse(
          metric == "forecast_qtrue_mae_H1000", "primary", "secondary"
        ),
        current_value = as.numeric(current[[metric]][[1L]]),
        best_dqlm_exdqlm_value = min(as.numeric(comparator[[metric]]), na.rm = TRUE),
        relative_gap_pct = 100 * (
          as.numeric(current[[metric]][[1L]]) /
            min(as.numeric(comparator[[metric]]), na.rm = TRUE) - 1
        ),
        authority_candidate_id = as.character(current[[fields[[1L]]]][[1L]]),
        authority_metric_path = as.character(current[[fields[[2L]]]][[1L]]),
        authority_metric_sha256 = qdesn_ssv2_sha256(
          as.character(current[[fields[[2L]]]][[1L]])
        ),
        interface_path = qdesn_fgav1_article_interface,
        interface_sha256 = qdesn_ssv2_sha256(qdesn_ssv2_path(
          repo_root, qdesn_fgav1_article_interface, must_work = TRUE
        )),
        stringsAsFactors = FALSE
      )
    }
  ...
  rownames(out) <- NULL
  if (nrow(out) != 14L || anyDuplicated(paste(
    out$target_cell_id, out$metric, sep = "\r"
  )) || any(!is.finite(out$current_value)) || any(out$relative_gap_pct <= 0)) {
    stop("The 14-role v7 forecast-gap ledger failed its frozen contract.", call. = FALSE)
  }
  out
}

qdesn_fgav1_evidence_source_manifest <- function(repo_root, history) {
  files <- sort(unique(as.character(history$source_file)))
  rows <- lapply(files, function(rel) {
    path <- qdesn_ssv2_path(repo_root, rel)
    index <- history$source_file == rel
    token <- tolower(rel)
    evidence_class <- if (grepl(
      "postm0|m0_structural|independent_exal_m0|lower_tail|tierb", token
    )) {
      "exact_m0_or_postm0_design_history"
    } else {
      "pre_m0_or_untyped_design_history"
    }
    data.frame(
      source_file = rel, source_exists = file.exists(path),
      source_sha256 = qdesn_ssv2_sha256(path),
      history_rows = sum(index),
      unique_profile_signatures = length(unique(history$profile_signature[index])),
      evidence_class = evidence_class,
      candidate_use = if (evidence_class == "exact_m0_or_postm0_design_history") {
        "design_prior_and_exact_duplicate_guard"
      } else {
        "design_prior_and_duplicate_guard_not_negative_exal_m0_evidence"
      },
      stringsAsFactors = FALSE
    )
  })
  out <- do.call(rbind, rows)
  rownames(out) <- NULL
  if (!nrow(out) || any(!out$source_exists) || anyNA(out$source_sha256)) {
    stop("The historical evidence-source manifest contains a missing source.",
         call. = FALSE)
  }
  out
}

.qdesn_fgav1_metric_source_fields <- function(metric) {
  switch(metric,
...
.qdesn_fgav1_adjacent_request <- function(metric_path) {
  candidates <- c(
...
.qdesn_fgav1_find_request <- function(repo_root, candidate_id, metric_path) {
  adjacent <- .qdesn_fgav1_adjacent_request(metric_path)
  ...
  audit_files <- list.files(
    qdesn_ssv2_path(repo_root, "validation", "fitforecast_v2", "promotions"),
    pattern = "[.]csv$", recursive = TRUE, full.names = TRUE
  )
  audit_files <- audit_files[grepl(
    "execution_contract_audit|candidate|confirmation|closeout", basename(audit_files),
    ignore.case = TRUE
  )]
  for (path in sort(audit_files)) {
    audit <- tryCatch(qdesn_ssv2_read_csv(path), error = function(e) NULL)
    if (is.null(audit) || !"fit_request_path" %in% names(audit)) next
    id_fields <- intersect(
      c("spec_id", "candidate_id", "source_candidate_id", "fit_source_candidate_id"),
      names(audit)
    )
    if (!length(id_fields)) next
    hit <- Reduce(`|`, lapply(id_fields, function(field) {
      as.character(audit[[field]]) == candidate_id
    }))
    requests <- unique(as.character(audit$fit_request_path[hit]))
    requests <- requests[nzchar(requests) & file.exists(requests)]
    if (!length(requests)) next
    request_path <- normalizePath(requests[[1L]], winslash = "/", mustWork = TRUE)
    if ("fit_request_sha256" %in% names(audit)) {
      expected <- unique(as.character(audit$fit_request_sha256[
        hit & as.character(audit$fit_request_path) == requests[[1L]]
      ]))
      expected <- expected[nzchar(expected)]
      if (length(expected) && !qdesn_ssv2_sha256(request_path) %in% expected) next
    }
    return(request_path)
  }
...
qdesn_fgav1_targets <- function(repo_root, freeze_requests = FALSE) {
  contract <- qdesn_fgav1_target_contract()
  interface <- qdesn_fgav1_interface(repo_root)
  q <- interface[
    interface$inference == "mcmc" &
  ...
  rows <- lapply(seq_len(nrow(contract)), function(i) {
    ...
    fields <- .qdesn_fgav1_metric_source_fields(spec$objective_metric)
    ...
    frozen_rel <- file.path(
      "config", "validation", paste0(qdesn_fgav1_stage, "_frozen_parent_requests"),
    ...
    source_request <- if (file.exists(frozen_abs) && !isTRUE(freeze_requests)) {
      ...
      .qdesn_fgav1_find_request(repo_root, candidate_id, metric_path)
    }
    ...
    current <- vapply(qdesn_fgav1_target_metrics, function(metric) {
      as.numeric(row[[metric]][[1L]])
    }, numeric(1L))
    comparator <- vapply(qdesn_fgav1_target_metrics, function(metric) {
  ...
  out <- do.call(rbind, rows)
  rownames(out) <- NULL
  if (nrow(out) != 8L || sum(out$tier == "A") != 5L ||
      sum(out$tier == "B") != 3L ||
      anyDuplicated(out$target_cell_id)) {
    stop("The forecast-gap target contract has drifted.", call. = FALSE)
  }
  out
}

qdesn_fgav1_parent_profiles <- function(repo_root, targets) {
  rows <- lapply(seq_len(nrow(targets)), function(i) {
    ...
    profile$candidate_id <- paste0("fgav1_", target$target_cell_id, "_parent")
    profile$screening_profile_id <- profile$candidate_id
    profile$design_role <- "exact_v7_metric_parent_control"
  ...
  out <- do.call(rbind, rows)
  rownames(out) <- NULL
  .qdesn_fgav1_profile_schema(qdesn_ssv2_ensure_effective_dimension(out))
}

.qdesn_fgav1_clone <- function(parent, arm, role) {
  x <- parent
  x$selection_arm <- arm
  x$design_role <- role
  x
...
.qdesn_fgav1_finalize_profile <- function(x) {
...
.qdesn_fgav1_local_arms <- function(parent) {
  lower <- .qdesn_fgav1_clone(parent, "local_tau_lower", "local_rhs_scale_exploitation")
  lower$rhs_tau0 <- max(1e-8, parent$rhs_tau0 / 10)
  upper <- .qdesn_fgav1_clone(parent, "local_tau_upper", "local_rhs_scale_exploitation")
  upper$rhs_tau0 <- min(1e-3, parent$rhs_tau0 * 10)
  readout <- .qdesn_fgav1_clone(
  ...
  .qdesn_fgav1_finalize_profile(out)
}

.qdesn_fgav1_mechanism_arms <- function(parent) {
...
.qdesn_fgav1_compact_arms <- function(parent) {
  tau0 <- max(1e-8, min(1e-5, as.numeric(parent$rhs_tau0)))
  low <- .qdesn_ssv2_profile_row(
    1L, 4L, 2L, .001, .88, 1L, tau0, 2L, 0L, 300L,
    "compact", "near_zero", "persistent", "postm0_compact_forecast",
    "compact_low_alpha"
  )
  low$pi_w <- qdesn_ssv2_pack(.0025)
  low$pi_in <- qdesn_ssv2_pack(.05)
  high <- .qdesn_ssv2_profile_row(
    1L, 12L, 15L, .75, .95, 1L, max(1e-8, tau0 / 10), 3L, 0L, 300L,
    "compact", "high", "persistent", "postm0_compact_forecast",
    "compact_high_alpha"
  )
  high$pi_w <- qdesn_ssv2_pack(.02)
  high$pi_in <- qdesn_ssv2_pack(.08)
  out <- rbind(low, high)
  for (nm in c("target_cell_id", "family", "tau", "priority",
               "objective_metric", "current_value", "comparator_value",
               "parent_anchor_id", "likelihood_target", "target_metrics")) {
...
.qdesn_fgav1_lag_arms <- function(parent) {
  tau0 <- max(1e-8, min(1e-4, as.numeric(parent$rhs_tau0)))
  readout <- .qdesn_ssv2_profile_row(
    1L, max(20L, min(70L, as.integer(parent$total_states))), 5L,
    .05, .90, 2L, tau0, 12L, 0L, 300L,
    "single", "low", "persistent", "readout_memory_decomposition",
    "lag_readout_heavy"
  )
  reservoir <- .qdesn_ssv2_profile_row(
    2L, c(30L, 20L), 120L, c(.35, .85), c(.88, .98), 4L,
    max(1e-8, tau0 / 3), 1L, 2L, 450L,
    "tapered", "slow_to_fast", "slow_to_fast",
    "reservoir_memory_decomposition", "lag_reservoir_heavy"
  )
  out <- rbind(readout, reservoir)
  for (nm in c("target_cell_id", "family", "tau", "priority",
               "objective_metric", "current_value", "comparator_value",
               "parent_anchor_id", "likelihood_target", "target_metrics")) {
    out[[nm]] <- parent[[nm]][[1L]]
...
.qdesn_fgav1_extrapolation_arms <- function(parent, universe, excluded, offset) {
  pool <- universe[
  ...
  b <- .qdesn_ssv2_maximin(
    ...
    .qdesn_fgav1_bind_fill(parent, a), 1L, offset = offset + 7919L
  )
...
qdesn_fgav1_candidate_profiles <- function(parents, universe, history) {
  rows <- list()
  for (i in seq_len(nrow(parents))) {
    parent <- parents[i, , drop = FALSE]
    local <- .qdesn_fgav1_local_arms(parent)
    compact <- .qdesn_fgav1_compact_arms(parent)
    mechanism <- .qdesn_fgav1_mechanism_arms(parent)
    lag <- .qdesn_fgav1_lag_arms(parent)
    used <- c(
      history$profile_signature, parent$profile_signature,
      local$profile_signature, compact$profile_signature,
      mechanism$profile_signature, lag$profile_signature
    )
    extrapolation <- .qdesn_fgav1_extrapolation_arms(
      parent, universe, used, qdesn_ssv2_seed(parent$target_cell_id, "fgav1")
    )
    cell <- .qdesn_fgav1_bind_fill(local, compact, mechanism, lag, extrapolation)
    if (identical(as.character(parent$priority[[1L]]), "tier_b")) {
      keep_arms <- c(
        "local_tau_lower", "local_readout_memory", "compact_low_alpha",
        "compact_high_alpha", "mechanism_slow", "lag_readout_heavy",
        "lag_reservoir_heavy", "history_boundary_maximin"
      )
      cell <- cell[match(keep_arms, cell$selection_arm), , drop = FALSE]
    }
    duplicate_history <- cell$profile_signature %in% c(
      history$profile_signature, parent$profile_signature
    ) | duplicated(cell$profile_signature)
    if (any(duplicate_history)) {
      ...
      cell <- .qdesn_fgav1_finalize_profile(cell)
    }
    hashes <- vapply(cell$profile_signature, digest::digest, character(1L),
                     algo = "sha256", serialize = FALSE)
    cell$candidate_id <- sprintf(
      "fgav1_%s_%02d_%s", parent$target_cell_id, seq_len(nrow(cell)),
      substr(hashes, 1L, 10L)
    )
    cell$screening_profile_id <- cell$candidate_id
    rows[[i]] <- .qdesn_fgav1_profile_schema(cell)
  }
  out <- do.call(rbind, rows)
  rownames(out) <- NULL
  expected <- c(rep(12L, sum(parents$priority == "tier_a")),
                rep(8L, sum(parents$priority == "tier_b")))
  observed <- as.integer(table(factor(
    out$target_cell_id, levels = parents$target_cell_id
  )))
  if (nrow(out) != 84L || !identical(observed, expected) ||
    ...
    stop("The 84-profile history-aware candidate contract failed.", call. = FALSE)
  }
  out
}

qdesn_fgav1_budget <- function(stage) {
  ...
  if (grepl("sealed$", stage)) {
    return(list(n_burn = 2000L, n_mcmc = 6000L, draws = 160L))
  }
  list(n_burn = 1000L, n_mcmc = 3000L, draws = 100L)
}

qdesn_fgav1_timeout_seconds <- function(stage) {
  if (stage == "smoke") 1800L else if (stage == "calibration") 21600L else
    if (grepl("confirmation$", stage)) 604800L else 86400L
}

qdesn_fgav1_make_job <- function(repo_root, profile, target, source, stage,
  ...
  budget <- qdesn_fgav1_budget(stage)
  ...
  job$config$validation$timeout_seconds <- qdesn_fgav1_timeout_seconds(stage)
  job$config$telemetry$progress_every <- 50L
  job$config$telemetry$heartbeat_seconds <- 1800L
  job$config$telemetry$stale_seconds <- 1800L
  job$config$outputs$retention_profile <- "storage_light_forecast_gap_adaptive_mcmc_v1"
  old_id <- job$job_id
  job$job_id <- sub(paste0("^", base_stage), stage, old_id)
  job$stage <- stage
  job$spec_id <- paste0("forecast_gap_adaptive_mcmc_v1__", job$job_id)
  job$config$validation_spec_id <- job$spec_id
  job$root_spec$root_id <- job$job_id
  job$root_spec$screening_stage <- qdesn_fgav1_stage
  ...
  job$current_metric_values <- as.list(setNames(
    as.numeric(target[1L, paste0("current_", qdesn_fgav1_target_metrics)]),
    qdesn_fgav1_target_metrics
  ))
  job$comparator_metric_values <- as.list(setNames(
    as.numeric(target[1L, paste0("comparator_", qdesn_fgav1_target_metrics)]),
    qdesn_fgav1_target_metrics
  ))
  job$study_contract$validation_stage <- qdesn_fgav1_stage
  ...
  job$schema_version <- "qdesn_forecast_gap_adaptive_mcmc_v1_job_v1"
  job
}

qdesn_fgav1_job_root <- function(repo_root, run_tag, job_id) {
  qdesn_ssv2_path(
    repo_root, "results", "qdesn_mcmc_validation", qdesn_fgav1_stage,
    run_tag, "jobs", job_id
  )
}

qdesn_fgav1_compact_forecast_metric_value <- function(job_root, metric) {
...
qdesn_fgav1_metric_values <- function(job_root) {
  setNames(vapply(qdesn_fgav1_target_metrics, function(metric) {
    ...
    if (!is.finite(value) && grepl("^forecast_", metric)) {
      value <- qdesn_fgav1_compact_forecast_metric_value(job_root, metric)
    }
    value
