...
ffv2_row_metrics <- function(config,
  ...
  base <- list(
    ...
    candidate_id = as.character(config$candidate_id %||% NA_character_),
    screen_stage = as.character(config$screen_stage %||% NA_character_),
    candidate_notes = as.character(config$candidate_notes %||% NA_character_),
    screen_sentinel = as.logical(config$screen_sentinel %||% FALSE),
