...
output <- normalizePath(get_arg(
  "--output", file.path(state_root, "tier_a_sealed_handoff_verification.json")
), winslash = "/", mustWork = FALSE)
adaptive_root <- file.path(state_root, "adaptive")

gate <- qdesn_ssv2_read_csv(file.path(adaptive_root, "tier_a_replication_gate.csv"))
advance <- qdesn_ssv2_read_json(file.path(
  adaptive_root, "advance_after_tier_a_replication.json"
))
ranking <- qdesn_ssv2_read_csv(file.path(adaptive_root, "tier_a_sealed_ranking.csv"))
plan_path <- file.path(adaptive_root, "tier_a_sealed_plan.csv")
plan <- qdesn_ssv2_read_csv(plan_path)
targets <- qdesn_ssv2_read_csv(file.path(
  repo_root, "config", "validation", paste0(qdesn_ltcv1_stage, "_target_cells.csv")
))
targets <- targets[targets$tier == "A", , drop = FALSE]
...
job_checks <- vapply(seq_along(jobs), function(i) {
  ...
  identical(as.character(job$job_id), as.character(row$job_id)) &&
    identical(as.character(job$stage), "tier_a_sealed") &&
    job$source_id %in% c("dev12", "dev13", "dev14", "dev15") &&
...
candidate_sets_match <- all(vapply(targets$target_cell_id, function(cell_id) {
  expected <- c(
    ...
    paste0("ltcv1_", cell_id, "_parent")
  )
  all(vapply(c("dev12", "dev13", "dev14", "dev15"), function(source_id) {
...
allowed_changes <- grepl(
  paste0(
    "^validation/fitforecast_v2/(docs/QDESN_LOWER_TAIL_CELLWISE_MCMC_V1_",
    "PROTOCOL_2026-08-11[.]md|scripts/.*qdesn_lower_tail_cellwise_mcmc_v1.*|",
    "tests/testthat/test-qdesn-lower-tail-cellwise-mcmc-v1[.]R)$"
  ),
  changed_paths
)
...
checks <- c(
  replication_gate = nrow(gate) == 1L && isTRUE(gate$gate_pass[[1L]]) &&
    gate$complete_artifact_jobs[[1L]] == 24L && gate$failed_jobs[[1L]] == 0L &&
    gate$missing_jobs[[1L]] == 0L,
  advancement = identical(as.character(advance$decision), "advance_to_tier_a_sealed"),
  target_cells = nrow(targets) == 6L && length(unique(plan$target_cell_id)) == 6L,
  ranking = nrow(ranking) == 12L && all(table(ranking$target_cell_id) == 2L) &&
    all(table(ranking$rank) == 6L),
  plan_shape = nrow(plan) == 72L && all(table(plan$target_cell_id) == 12L) &&
  ...
  design_ancestor = length(design_commit) == 1L && nzchar(design_commit) &&
    ancestor_status == 0L,
  computational_kernel_unchanged = length(changed_paths) > 0L && all(allowed_changes),
  branch = identical(system("git branch --show-current", intern = TRUE),
                     qdesn_ltcv1_branch)
)
result <- list(
  schema_version = "qdesn_lower_tail_cellwise_mcmc_v1_sealed_handoff_v1",
