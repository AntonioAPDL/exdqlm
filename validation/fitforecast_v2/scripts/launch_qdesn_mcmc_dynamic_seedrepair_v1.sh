...
SESSION="${3:-ffv2_qdesn_dynamic_seedrepair_v1_$(date +%Y%m%d_%H%M%S)}"
cd "$REPO_ROOT"
...
PIPELINE="validation/fitforecast_v2/scripts/run_qdesn_mcmc_dynamic_seedrepair_v1_pipeline.sh"
printf '%s\n' "$SESSION" > "$STATE_ROOT/tmux_session.txt"
printf '%s\n' "$RUN_ID" > "$STATE_ROOT/run_id.txt"
printf '%s\n' "$(git rev-parse HEAD)" > "$STATE_ROOT/launch_commit.txt"

tmux new-session -d -s "$SESSION" \
  "cd '$REPO_ROOT' && bash '$PIPELINE' '$REPO_ROOT' '$RUN_ID' > '$STDOUT_LOG' 2>&1"
sleep 2
if ! tmux has-session -t "$SESSION" 2>/dev/null; then
