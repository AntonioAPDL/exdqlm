...
"${rscript_bin}" \
  "${harness_root}/scripts/verify_independent_metric_interval_coupling_pilot_v1_plan.R" \
...
"${rscript_bin}" \
  "${harness_root}/scripts/closeout_independent_metric_interval_coupling_pilot_v1.R" \
  --repo-root "${repo_root}" \
  --state-root "${state_root}"
