...
if [[ "$(git branch --show-current)" != "validation/qdesn-alpha-rho-topology-v1-1.0.0" ]]; then
...
RUN_ID="qdesn_alpha_rho_topology_v1_${STAMP}"
SESSION="ffv2_qdesn_arv1_${STAMP}"
STATE_ROOT="reports/shared_fitforecast_v2_orchestration/${RUN_ID}"
mkdir -p "$STATE_ROOT"
LOG_PATH="$STATE_ROOT/pipeline.stdout.log"

tmux new-session -d -s "$SESSION" \
  "cd '$REPO_ROOT' && bash validation/fitforecast_v2/scripts/run_qdesn_alpha_rho_topology_v1_pipeline.sh '$REPO_ROOT' '$WORKERS' '$RUN_ID' > '$LOG_PATH' 2>&1"
