...
PLAN="$ADAPTIVE_ROOT/tier_b_replication_plan.csv"
...
VERIFY_HANDOFF="validation/fitforecast_v2/scripts/verify_qdesn_tierb_cellwise_mcmc_v1_replication_handoff.R"
...
"$R_SCRIPT" --vanilla -e 'library(exdqlm); stopifnot(as.character(packageVersion("exdqlm")) == "1.0.0")' \
  > "$STATE_ROOT/tier_b_replication_package_load.log" 2>&1 || {
    echo "The fingerprinted exdqlm 1.0.0 campaign library is unavailable." >&2; exit 3;
  }

set_stage tier_b_replication_preflight
record_status tier_b_replication_preflight STARTED "discovery_gate;plan;source;seed;hash contracts"
"$R_SCRIPT" "$VERIFY_HANDOFF" --repo-root "$REPO_ROOT" --state-root "$STATE_ROOT" \
  --output "$STATE_ROOT/tier_b_replication_handoff_verification.json" \
  > "$STATE_ROOT/tier_b_replication_handoff_verification.log" 2>&1
record_status tier_b_replication_preflight COMPLETED "16 roots;dev19;r02;storage-light"

heartbeat_loop & HEARTBEAT_PID="$!"
set_stage tier_b_replication_resource_gate
wait_for_resources
...
set_stage tier_b_replication
CONFIG_LIST="$STATE_ROOT/tier_b_replication_configs.txt"
...
record_status tier_b_replication STARTED \
...
taskset -c "$CPU_SET" xargs -r -n 1 -P "$PARALLELISM" \
  ...
  < "$CONFIG_LIST" > "$STATE_ROOT/tier_b_replication_workers.log" 2>&1
RC="$?"
set -e
printf '%s\n' "$RC" > "$STATE_ROOT/tier_b_replication_worker_exit_code.txt"
"$R_SCRIPT" "$HEALTH" --repo-root "$REPO_ROOT" --run-tag "$RUN_TAG" \
  --plan "$PLAN" --output "$STATE_ROOT/tier_b_replication_health.csv" \
  > "$STATE_ROOT/tier_b_replication_health.log" 2>&1 || true
[[ "$RC" -eq 0 ]] || {
  record_status tier_b_replication FAILED \
    "worker_exit=${RC};same_run_tag_resumes_completed_jobs"; exit "$RC";
}
"$R_SCRIPT" "$VERIFY" --repo-root "$REPO_ROOT" \
  --materialization-root "$MATERIALIZATION_ROOT" --stage tier_b_replication \
  --plan "$PLAN" --run-tag "$RUN_TAG" \
  --output "$STATE_ROOT/tier_b_replication_verification.json" \
  > "$STATE_ROOT/tier_b_replication_verification.log" 2>&1
record_status tier_b_replication COMPLETED "jobs=${JOBS};finite_storage_light=all"

set_stage tier_b_replication_closeout
"$R_SCRIPT" "$ADVANCE" --repo-root "$REPO_ROOT" --from tier_b_replication \
  --run-tag "$RUN_TAG" --materialization-root "$MATERIALIZATION_ROOT" \
  --output-root "$ADAPTIVE_ROOT" \
  > "$STATE_ROOT/advance_after_tier_b_replication.log" 2>&1
record_status tier_b_replication_closeout COMPLETED \
  "paired rankings complete;48-job sealed plan materialized;not launched"
set_stage replication_complete
write_heartbeat
record_status replication_complete COMPLETED \
  "Tier-B replication closed;sealed evaluation waits for review;article v6 unchanged"
printf 'Tier-B cellwise MCMC v1 replication complete: %s\n' "$RUN_TAG"
