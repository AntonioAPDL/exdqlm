...
RUN_ID="${2:-qdesn_mcmc_dynamic_seedrepair_v1_$(date +%Y%m%d_%H%M%S)}"
...
LOCK_FILE="reports/shared_fitforecast_v2_orchestration/qdesn_mcmc_dynamic_seedrepair_v1.lock"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "Another dynamic-seed-repair v1 pipeline holds $LOCK_FILE" >&2
  exit 2
fi
...
EXPECTED_BRANCH="validation/qdesn-mcmc-dynamic-seedrepair-v1-1.0.0"
...
DISCOVERY_RUN_TAG="qdesn-dsr1-discovery-${STAMP}__git-${GIT_SHORT}"
...
RUN_ID=$RUN_ID
DISCOVERY_RUN_TAG=$DISCOVERY_RUN_TAG
GIT_COMMIT=$GIT_SHA
WORKTREE=$REPO_ROOT
...
ARTICLE_PROMOTION_APPROVED=FALSE
EOF
...
record_status "contract_materialize" "STARTED" "regenerate sources and assert committed contract stability"
"$R_SCRIPT" validation/fitforecast_v2/scripts/materialize_qdesn_mcmc_dynamic_seedrepair_v1.R \
...
record_status "contract_materialize" "COMPLETED" "sources regenerated; tracked contract unchanged"

set_stage "contract_verify"
record_status "contract_verify" "STARTED" "branch=${BRANCH};commit=${GIT_SHA}"
"$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_mcmc_dynamic_seedrepair_v1.R \
  --output "$STATE_ROOT/contract_verification.json" > "$STATE_ROOT/contract_verification.log" 2>&1
record_status "contract_verify" "COMPLETED" "240 corrected discovery specs; confirmation gated"

set_stage "prepare_only"
PREPARE_TAG="qdesn-dsr1-prepare-${STAMP}__git-${GIT_SHORT}"
record_status "prepare_only" "STARTED" "$PREPARE_TAG"
"${COMMON[@]}" --workers 1 \
  --defaults "${CONFIG_STUB}_discovery_defaults.yaml" \
  --grid "${CONFIG_STUB}_discovery_grid.csv" \
  --batch full --prepare-only --run-tag "$PREPARE_TAG" \
  > "$STATE_ROOT/prepare_only.log" 2>&1
find "results/qdesn_mcmc_validation/${STAGE}_discovery" -path "*${PREPARE_TAG}*" -type f \
...
set_stage "smoke"
SMOKE_TAG="qdesn-dsr1-smoke-${STAMP}__git-${GIT_SHORT}"
...
SMOKE_ROOT="results/qdesn_mcmc_validation/${STAGE}_discovery/${SMOKE_TAG}"
...
set_stage "discovery_full"
record_status "discovery_full" "STARTED" "run_tag=${DISCOVERY_RUN_TAG};workers=20;cpus=${CPU_SET}"
set +e
taskset -c "$CPU_SET" "${COMMON[@]}" --workers "$WORKERS" \
  --defaults "${CONFIG_STUB}_discovery_defaults.yaml" \
  --grid "${CONFIG_STUB}_discovery_grid.csv" \
  --batch full --run-tag "$DISCOVERY_RUN_TAG" \
  > "$STATE_ROOT/discovery_full.log" 2>&1
DISCOVERY_RC="$?"
set -e
printf '%s\n' "$DISCOVERY_RC" > "$STATE_ROOT/discovery_exit_code.txt"
if [[ "$DISCOVERY_RC" -eq 0 ]]; then
  record_status "discovery_full" "COMPLETED" "exit_code=0"
else
  record_status "discovery_full" "COMPLETED_WITH_FAILURES" "exit_code=${DISCOVERY_RC};audit finite outputs"
fi

set_stage "storage_audit"
DISCOVERY_ROOT="results/qdesn_mcmc_validation/${STAGE}_discovery/${DISCOVERY_RUN_TAG}"
STORAGE_OK=1
if storage_audit "$DISCOVERY_ROOT" "$STATE_ROOT/discovery_binary_payload_audit.tsv"; then
...
set_stage "closeout"
set +e
"$R_SCRIPT" validation/fitforecast_v2/scripts/audit_qdesn_mcmc_dynamic_seedrepair_v1.R \
  --state-root "$STATE_ROOT" --output-root "$STATE_ROOT/closeout" \
  > "$STATE_ROOT/closeout.log" 2>&1
AUDIT_RC="$?"
set -e
if [[ "$AUDIT_RC" -eq 0 ]]; then
  record_status "closeout" "COMPLETED" "dynamic_seedrepair_discovery_gate.json"
else
...
write_heartbeat
if [[ "$DISCOVERY_RC" -eq 0 && "$STORAGE_OK" -eq 1 && "$AUDIT_RC" -eq 0 ]]; then
  record_status "pipeline_complete" "COMPLETED" "Discovery closed; full confirmation and article promotion remain gated"
  exit 0
fi
record_status "pipeline_complete" "COMPLETED_WITH_FAILURES" "inspect finite discovery evidence; no automatic continuation"
exit 1
