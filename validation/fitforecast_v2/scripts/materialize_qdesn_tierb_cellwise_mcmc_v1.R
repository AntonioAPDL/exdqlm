...
suppressPackageStartupMessages({
  required <- c("digest", "jsonlite", "yaml", "exdqlm")
...
if (!identical(as.character(description[1L, "Version"]), "1.0.0")) {
  stop("Tier-B cellwise MCMC v1 requires exdqlm 1.0.0.", call. = FALSE)
}
...
targets <- qdesn_tbcv1_targets(repo_root, freeze_requests = FALSE)
...
history <- history[
  !grepl(qdesn_tbcv1_stage, history$source_file, fixed = TRUE), , drop = FALSE
]
...
if (use_frozen_designs) {
  ...
  parents <- qdesn_tbcv1_parent_profiles(repo_root, targets)
  profiles <- qdesn_tbcv1_candidate_profiles(repo_root, parents, history)
}
admitted <- profiles$history_disposition ==
  "admitted_configured_unrun_not_evaluated"
if (nrow(parents) != 4L || nrow(profiles) != 32L || sum(admitted) != 16L ||
    any(profiles$profile_signature[!admitted] %in% history$profile_signature)) {
...
history_audit$source_class <- ifelse(
  grepl("lower_tail_cellwise_mcmc_v1_candidate_profiles", history_audit$source_file),
  "configured_in_closed_lower_tail_campaign", "historical_configured_design"
)
history_audit$screening_disposition <- "excluded_from_new_signature_generation"
history_audit_path <- qdesn_ssv2_write_csv(
  history_audit, paste0(stub, "_typed_history_audit.csv")
)
allowlist_path <- qdesn_ssv2_write_csv(
  profiles[admitted, c(
    "target_cell_id", "candidate_id", "selection_arm", "profile_signature",
    "design_provenance", "history_disposition"
  )], paste0(stub, "_configured_unrun_allowlist.csv")
)

replicates <- source_cfg$replicates
roles <- vapply(replicates, function(x) as.character(x$role), character(1L))
if (length(replicates) != 8L || sum(roles == "discovery") != 3L ||
    sum(roles == "replication") != 1L || sum(roles == "sealed_holdout") != 4L) {
  stop("Source contract must contain 3 discovery, 1 replication, and 4 sealed blocks.",
       call. = FALSE)
}
seed_rows <- list()
root_rows <- list()
for (i in seq_along(replicates)) {
  ...
  manifest <- list(
    meta = list(
      ...
      notes = sprintf("Tier-B cellwise %s source %s.",
    ...
    qdesn_materialization = list(staged_root = file.path(
      "results", "qdesn_mcmc_validation", qdesn_tbcv1_stage,
...
source_parent <- as.character(source_cfg$generation$output_parent)
if (!grepl("^/", source_parent)) source_parent <- file.path(repo_root, source_parent)
source_binary_paths <- list.files(
  source_parent, pattern = "[.](rds|rda|RData)$", recursive = TRUE,
  full.names = TRUE, ignore.case = TRUE
)
if (length(source_binary_paths) != 64L ||
    any(!grepl("/source_replicates/", source_binary_paths, fixed = TRUE))) {
  stop("The predeclared 64-file source-authority binary contract failed.",
       call. = FALSE)
}
...
resolve_window <- function(profile, target, source_id) {
  ...
  staged <- qdesn_ssv2_stage_source_window(
    ...
    file.path(repo_root, "results", "qdesn_mcmc_validation",
              qdesn_tbcv1_stage, "staged_source_windows")
  )
...
tier_b_targets <- targets[targets$tier == "B", , drop = FALSE]
tier_b_ids <- tier_b_targets$target_cell_id
tier_b_profiles <- profiles[profiles$target_cell_id %in% tier_b_ids, , drop = FALSE]
tier_b_parents <- parents[parents$target_cell_id %in% tier_b_ids, , drop = FALSE]

smoke_profiles <- do.call(rbind, lapply(c("laplace", "gausmix"), function(family) {
  x <- tier_b_profiles[tier_b_profiles$family == family, , drop = FALSE]
  x[which.max(x$effective_readout_dimension), , drop = FALSE]
}))
smoke_plan <- do.call(rbind, lapply(seq_len(nrow(smoke_profiles)), function(i) {
  p <- smoke_profiles[i, , drop = FALSE]
  write_job(p, target_map[[p$target_cell_id[[1L]]]], "dev16", "smoke")
}))

calibration_plan <- do.call(rbind, lapply(tier_b_ids, function(cell_id) {
  cell <- tier_b_profiles[tier_b_profiles$target_cell_id == cell_id, , drop = FALSE]
  p <- cell[which.max(cell$effective_readout_dimension), , drop = FALSE]
  write_job(p, target_map[[cell_id]], "dev16", "calibration")
}))

discovery_rows <- list()
k <- 0L
for (cell_id in tier_b_ids) {
  cell <- rbind(
    tier_b_profiles[tier_b_profiles$target_cell_id == cell_id, , drop = FALSE],
    tier_b_parents[tier_b_parents$target_cell_id == cell_id, , drop = FALSE]
  )
  for (i in seq_len(nrow(cell))) {
    for (source_id in c("dev16", "dev17", "dev18")) {
      k <- k + 1L
      discovery_rows[[k]] <- write_job(
        cell[i, , drop = FALSE], target_map[[cell_id]], source_id,
        "tier_b_discovery", reservoir_seed_id = "r01"
      )
    }
  }
}
discovery_plan <- do.call(rbind, discovery_rows)
if (nrow(smoke_plan) != 2L || nrow(calibration_plan) != 4L ||
    nrow(discovery_plan) != 108L) {
  stop("Materialized plan counts do not match 2/4/108.", call. = FALSE)
}
...
plan_paths <- c(
  ...
  tier_b_discovery = qdesn_ssv2_write_csv(
    discovery_plan, file.path(output_root, "tier_b_discovery_plan.csv")
  )
)
...
tracked_paths <- c(
  source_cfg_path, target_path, history_path, history_audit_path, allowlist_path,
  ...
  file.path(repo_root, "validation", "fitforecast_v2", "scripts", c(
    "materialize_qdesn_tierb_cellwise_mcmc_v1.R",
    "run_qdesn_tierb_cellwise_mcmc_v1_chain.R",
    "healthcheck_qdesn_tierb_cellwise_mcmc_v1.R",
    "verify_qdesn_tierb_cellwise_mcmc_v1.R",
    "advance_qdesn_tierb_cellwise_mcmc_v1.R",
    "run_qdesn_tierb_cellwise_mcmc_v1_pipeline.sh",
    "launch_qdesn_tierb_cellwise_mcmc_v1.sh",
    "run_qdesn_tierb_cellwise_mcmc_v1_replication.sh",
    "launch_qdesn_tierb_cellwise_mcmc_v1_replication.sh",
    "verify_qdesn_tierb_cellwise_mcmc_v1_replication_handoff.R",
    "run_qdesn_tierb_cellwise_mcmc_v1_sealed.sh",
    "launch_qdesn_tierb_cellwise_mcmc_v1_sealed.sh",
    "verify_qdesn_tierb_cellwise_mcmc_v1_sealed_handoff.R",
    "materialize_qdesn_tierb_cellwise_mcmc_v1_confirmation.R",
    "verify_qdesn_tierb_cellwise_mcmc_v1_confirmation.R",
    "run_qdesn_tierb_cellwise_mcmc_v1_confirmation.sh",
    "launch_qdesn_tierb_cellwise_mcmc_v1_confirmation.sh",
    "closeout_qdesn_tierb_cellwise_mcmc_v1_confirmation.R"
  )),
  file.path(repo_root, "validation", "fitforecast_v2", "tests", "testthat",
            "test-qdesn-tierb-cellwise-mcmc-v1.R"),
  file.path(repo_root, "validation", "fitforecast_v2", "docs",
            "QDESN_TIERB_CELLWISE_MCMC_V1_PROTOCOL_2026-08-13.md")
)
...
manifest <- list(
  schema_version = "qdesn_tierb_cellwise_mcmc_v1_materialization_v1",
  generated_at = as.character(Sys.time()),
  git_commit = system("git rev-parse HEAD", intern = TRUE),
  package_version = "1.0.0",
  base_authority_commit = qdesn_tbcv1_base_commit,
  ...
  history_audit_sha256 = qdesn_ssv2_sha256(history_audit_path),
  configured_unrun_allowlist_path = allowlist_path,
  configured_unrun_allowlist_sha256 = qdesn_ssv2_sha256(allowlist_path),
  ...
  counts = list(
    target_cells = nrow(targets), tier_b_cells = nrow(tier_b_targets),
    candidates = nrow(profiles), configured_unrun_candidates = sum(admitted),
    new_signature_candidates = sum(!admitted),
    parent_controls = nrow(parents), historical_signatures = nrow(history),
    source_authority_binary_files = length(source_binary_paths),
    source_authority_binary_bytes = sum(file.info(source_binary_paths)$size),
    fitted_model_binary_files = 0L,
    smoke_jobs = nrow(smoke_plan), calibration_jobs = nrow(calibration_plan),
    tier_b_discovery_jobs = nrow(discovery_plan)
  ),
  gates = list(
    tier_b_replication = "three candidates per cell after paired discovery evidence",
    tier_b_sealed = "two candidates per cell after development replication",
    confirmation = "metric-specific finalists; explicit human launch approval",
    tier_a = qdesn_tbcv1_closed_tiera_decision,
    tier_b = "only the four never-run AL fit-RMSE cells are in scope",
...
cat(sprintf("smoke=%d calibration=%d tier_b_discovery=%d\n",
