...
lsof_lines <- tryCatch(system2(
  "lsof", c("-nP", "-a", paste0("-u", Sys.getenv("USER")), "-d", "1,2", "-Fpn"),
  stdout = TRUE, stderr = FALSE
), error = function(e) character())
open_output_paths <- sub("^n", "", grep("^n/", lsof_lines, value = TRUE))

trace_progress <- function(path, expected_total) {
  empty <- list(iteration = NA_real_, total = expected_total, phase = NA_character_,
  ...
  phase_name <- intersect(c("phase", "stage"), names(x))
  list(
    iteration = if (length(iter_name)) {
      as.numeric(utils::tail(x[[iter_name[[1L]]]], 1L))
    } else nrow(x),
    total = if (length(total_name)) {
      as.numeric(utils::tail(x[[total_name[[1L]]]], 1L))
    } else expected_total,
    phase = if (length(phase_name)) as.character(utils::tail(x[[phase_name[[1L]]]], 1L)) else NA_character_,
    source = "progress_trace"
  )
}

latest_progress <- function(trace_path, child_log_path, expected_n_burn,
                            expected_n_mcmc) {
  expected_total <- as.integer(expected_n_burn) + as.integer(expected_n_mcmc)
  trace <- trace_progress(trace_path, expected_total)
  child <- list(iteration = NA_real_, total = expected_total, phase = NA_character_,
                source = NA_character_)
  if (file.exists(child_log_path)) {
    lines <- tryCatch(system2("tail", c("-n", "240", child_log_path), stdout = TRUE),
                      error = function(e) character())
    parsed <- qdesn_ssv2_parse_progress_lines(lines, expected_n_burn, expected_n_mcmc)
    child <- c(parsed, list(source = "pipeline_child_live_log"))
  }
  values <- c(trace$iteration, child$iteration)
  if (!any(is.finite(values))) return(trace)
  if (is.finite(child$iteration) &&
      (!is.finite(trace$iteration) || child$iteration >= trace$iteration)) child else trace
}
...
rows <- lapply(seq_len(nrow(plan)), function(i) {
  ...
  child_log_path <- file.path(root, "logs", "pipeline_child_live.log")
  child_log_open <- child_log_path %in% open_output_paths
  alive <- grepl(plan$config_path[[i]], process_text, fixed = TRUE) || child_log_open
  ...
  progress <- latest_progress(
    file.path(root, "progress_trace.csv"), child_log_path,
    plan$expected_n_burn[[i]], plan$expected_n_mcmc[[i]]
  )
  data.frame(
    ...
    process_alive = alive, child_log_open = child_log_open,
