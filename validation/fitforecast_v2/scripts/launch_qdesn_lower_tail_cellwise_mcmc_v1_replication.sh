...
SESSION="${SESSION:-ffv2_qdesn_ltcv1_replication_$(date +%Y%m%d_%H%M%S)}"
WORKERS="${WORKERS:-20}"
...
tmux new-session -d -s "$SESSION" \
  "cd '$REPO_ROOT' && WORKERS='$WORKERS' validation/fitforecast_v2/scripts/run_qdesn_lower_tail_cellwise_mcmc_v1_replication.sh '$REPO_ROOT' '$RUN_ID' '$RUN_TAG' > '$STDOUT_LOG' 2>&1"
