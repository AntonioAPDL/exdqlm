...
job_checks <- lapply(jobs, function(job) {
  ...
  data.frame(
    schema = identical(as.character(job$schema_version),
                       "qdesn_lower_tail_cellwise_mcmc_v1_job_v1"),
...
static_checks <- c(
  target_count = nrow(targets) == 10L && sum(targets$tier == "A") == 6L &&
    sum(targets$tier == "B") == 4L,
  profile_count = nrow(profiles) == 80L && nrow(parents) == 10L,
  ...
  plans = nrow(plans$smoke) == 2L && nrow(plans$calibration) == 6L &&
    nrow(plans$tier_a_discovery) == 108L,
...
result <- list(
  schema_version = "qdesn_lower_tail_cellwise_mcmc_v1_verification_v1",
