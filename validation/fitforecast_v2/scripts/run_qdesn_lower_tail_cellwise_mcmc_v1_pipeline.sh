...
RUN_ID="${2:-qdesn_lower_tail_cellwise_mcmc_v1_$(date +%Y%m%d_%H%M%S)}"
RUN_TAG="${3:-qdesn-lower-tail-cellwise-v1-$(date +%Y%m%d_%H%M%S)__git-$(git -C "$REPO_ROOT" rev-parse --short HEAD)}"
...
fi
{
  ...
  printf 'WORKTREE=%s\nWORKERS=%s\nTHREADS_PER_WORKER=1\n' \
    "$REPO_ROOT" "$WORKERS"
  printf 'CPU_SET=PENDING_RESOURCE_GATE\nSMOKE=2\nCALIBRATION=6\nTIER_A_DISCOVERY=108\n'
  printf 'AUTO_STOP_AFTER_TIER_A_DISCOVERY=TRUE\nARTICLE_UPDATE_AUTOMATIC=FALSE\n'
} > "$STATE_ROOT/run_tags.env"

set_stage materialize
record_status materialize STARTED "v6 authority;80 history-aware candidates;7 source blocks"
"$R_SCRIPT" validation/fitforecast_v2/scripts/materialize_qdesn_lower_tail_cellwise_mcmc_v1.R \
...
record_status materialize COMPLETED "2 smoke;6 calibration;108 discovery"

set_stage tests
"$R_SCRIPT" -e 'pkgload::load_all(".", quiet=TRUE); stopifnot(as.character(packageVersion("exdqlm")) == "1.0.0")' \
  > "$STATE_ROOT/package_load.log" 2>&1
"$R_SCRIPT" -e 'pkgload::load_all(".", quiet=TRUE); testthat::test_file("validation/fitforecast_v2/tests/testthat/test-qdesn-lower-tail-cellwise-mcmc-v1.R", reporter="summary", stop_on_failure=TRUE, stop_on_warning=TRUE)' \
...
"$R_SCRIPT" -e 'x <- read.csv(commandArgs(TRUE)[1]); stopifnot(nrow(x)==6L, all(x$status=="SUCCESS"), all(x$binary_count==0L), max(x$elapsed_seconds, na.rm=TRUE)<=21600)' \
  "$STATE_ROOT/calibration_verification_runtime.csv" \
  > "$STATE_ROOT/calibration_runtime_gate.log" 2>&1
record_status calibration_runtime_gate PASS "6 representative roots;maximum elapsed <= 6 hours"
...
record_status tier_a_discovery_closeout COMPLETED \
  "paired rankings complete;24-job replication plan materialized;not launched"
