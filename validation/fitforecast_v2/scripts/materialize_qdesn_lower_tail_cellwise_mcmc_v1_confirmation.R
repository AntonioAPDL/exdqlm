...
checks <- c(
  sealed_gate = nrow(gate) == 1L && isTRUE(gate$gate_pass[[1L]]) &&
    gate$successful_jobs[[1L]] == 72L && gate$failed_jobs[[1L]] == 0L,
  sealed_decision = identical(as.character(advance$decision),
                              "confirmation_manifest_only_wait_for_explicit_approval"),
  eligible = nrow(eligible) == 2L && !anyDuplicated(paste(eligible$target_cell_id,
                                                          eligible$metric)),
  handoff = nrow(handoff) == 6L && all(table(handoff$target_cell_id) == 3L) &&
...
for (i in seq_len(nrow(eligible))) {
  ...
  windows[[i]] <- source
  for (chain in 1:3) {
    job <- qdesn_ltcv1_make_job(repo_root, profile, target, source,
    ...
    job$study_contract$approval_instruction <-
      "2026-08-12 user instruction to implement the approved confirmation plan"
...
if (nrow(plan) != 6L || any(table(plan$target_cell_id) != 3L) || anyDuplicated(plan$job_id)) {
...
qdesn_ssv2_write_json(list(
  schema_version = "qdesn_lower_tail_cellwise_mcmc_v1_confirmation_v1",
  generated_at = as.character(Sys.time()), validation_commit = system("git rev-parse HEAD", intern = TRUE),
  source_registry_hash_value = qdesn_ssv2_registry_hash,
  explicit_human_approval = TRUE, confirmation_jobs = 6L, chains_per_candidate = 3L,
...
cat(sprintf("confirmation_materialized jobs=6 output=%s\n", output_root))
