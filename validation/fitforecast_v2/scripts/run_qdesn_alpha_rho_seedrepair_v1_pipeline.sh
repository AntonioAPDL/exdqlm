...
MIN_MEMORY_GB="${MIN_MEMORY_GB:-160}"
...
PREP_TAG="qdesn-arsr1-prepare-${STAMP}__git-${GIT_SHORT}"
SMOKE_TAG="qdesn-arsr1-seed-smoke-${STAMP}__git-${GIT_SHORT}"
FULL_TAG="qdesn-arsr1-full-${STAMP}__git-${GIT_SHORT}"
...
CURRENT_STAGE="seed_smoke"
run_logged "$STATE_ROOT/seed_smoke.log" \
  "${COMMON[@]}" \
...
check_storage_light \
  ...
  "$STATE_ROOT/seed_smoke_binary_payload_audit.tsv"

CURRENT_STAGE="seed_smoke_audit"
run_logged "$STATE_ROOT/seed_smoke_audit.log" \
  "$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_alpha_rho_seedrepair_smoke.R \
  --run-tag "$SMOKE_TAG" --output-root "$STATE_ROOT/seed_smoke_audit"
...
CURRENT_STAGE="audit"
run_logged "$STATE_ROOT/audit.log" \
  "$R_SCRIPT" validation/fitforecast_v2/scripts/audit_qdesn_alpha_rho_seedrepair_v1.R \
  --run-tag "$FULL_TAG" --output-root "$STATE_ROOT/audit"
DECISION="$($R_SCRIPT -e 'x <- jsonlite::read_json(commandArgs(TRUE)[1]); cat(x$decision)' "$STATE_ROOT/audit/seedrepair_gate.json")"
record_status "GATE_DECISION" "$DECISION"

CURRENT_STAGE="pipeline_complete"
record_status "COMPLETED" "decision=${DECISION};full_budget_confirmation_not_launched"
write_heartbeat
