...
if (!nzchar(run_id)) {
  ...
  candidates <- candidates[grepl(
    "/qdesn_mcmc_chain_aggregate_confirm_v1_[0-9]{8}_[0-9]{6}$", candidates
  )]
  if (!length(candidates)) stop("No chain-aggregate confirmation v1 run found.", call. = FALSE)
...
closeout_complete <-
  grepl("^CHAIN_AGGREGATE_CONFIRMATION_COMPLETE_", closeout_decision) &&
  ...
  identical(as.integer(closeout_gate$complete_five_chain_designs %||% NA_integer_), 11L) &&
