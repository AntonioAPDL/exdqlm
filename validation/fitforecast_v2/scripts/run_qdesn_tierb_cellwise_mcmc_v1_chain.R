...
job_root <- qdesn_tbcv1_job_root(repo_root, run_tag, job_id)
...
contracts <- c(
  ...
  al_kernel = !identical(expected_method, qdesn_ssv2_method_id),
  ...
  timeout = identical(as.integer(job$config$validation$timeout_seconds),
                      qdesn_tbcv1_timeout_seconds(as.character(job$stage))),
...
t0 <- Sys.time()
result <- tryCatch(
  exdqlm:::.qdesn_validation_run_one_method(
...
if (!is.null(result) && identical(as.character(result$status), "SUCCESS")) {
  postprocess_error <- tryCatch({
    source_df <- exdqlm:::.qdesn_validation_read_source_series(job$root_spec)
    ...
    metrics <- exdqlm:::.qdesn_static_crossstudy_collect_metrics_from_summary(
      result$summary, q_true_full
    )
    signoff_cfg <- exdqlm:::.qdesn_validation_signoff_cfg(defaults)
    ...
    signoff <- exdqlm:::.qdesn_validation_mcmc_signoff_from_rows(
      meta_row, result$health, result$progress_trace, signoff_cfg$mcmc
    )
    fit <- exdqlm:::.qdesn_static_crossstudy_fit_summary_row(
...
metric_values <- qdesn_tbcv1_metric_values(job_root)
