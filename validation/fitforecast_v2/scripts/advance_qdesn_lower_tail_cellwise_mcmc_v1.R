...
output_root <- normalizePath(get_arg("--output-root"), winslash = "/",
                             mustWork = FALSE)
if (!from_stage %in% c(
  "tier_a_discovery", "tier_a_replication", "tier_a_sealed"
...
stages <- switch(from_stage,
  tier_a_discovery = "tier_a_discovery",
  tier_a_replication = c("tier_a_discovery", "tier_a_replication"),
  tier_a_sealed = "tier_a_sealed"
)
...
for (cell_id in targets$target_cell_id) {
  ...
  parent_id <- paste0("ltcv1_", cell_id, "_parent")
...
profile_from_plan <- function(candidate_id) {
  stages <- c("tier_a_discovery", "tier_a_replication", "tier_a_sealed")
...
decision <- NULL
if (from_stage == "tier_a_discovery") {
  ...
  qdesn_ssv2_write_csv(ranking, file.path(output_root, "tier_a_replication_ranking.csv"))
  plan <- write_plan(selected, "tier_a_replication", "dev11", "r02")
  if (nrow(plan) != 24L) stop("Tier-A replication plan must contain 24 jobs.")
  decision <- "advance_to_tier_a_replication"
} else if (from_stage == "tier_a_replication") {
  selected <- setNames(lapply(
  ...
  qdesn_ssv2_write_csv(ranking, file.path(output_root, "tier_a_sealed_ranking.csv"))
  plan <- write_plan(
    selected, "tier_a_sealed", c("dev12", "dev13", "dev14", "dev15"), "r03"
  )
  if (nrow(plan) != 72L) stop("Tier-A sealed plan must contain 72 jobs.")
  decision <- "advance_to_tier_a_sealed"
} else {
  ...
  qdesn_ssv2_write_csv(
    eligible, file.path(output_root, "tier_a_sealed_eligible_metrics.csv")
  )
  qdesn_ssv2_write_csv(
    confirmation, file.path(output_root, "tier_a_confirmation_manifest.csv")
  )
  if (nrow(confirmation) > 24L) stop("Confirmation exceeds the 24-chain cap.")
...
manifest <- list(
  schema_version = "qdesn_lower_tail_cellwise_mcmc_v1_advancement_v1",
