...
if (from_stage == "tier_a_discovery") {
  ...
  plan <- write_plan(selected, "tier_a_replication", "dev32", "r02")
  if (nrow(plan) != 20L) stop("Replication plan must contain 20 jobs.")
  ...
  plan <- write_plan(
    selected, "tier_a_sealed", c("dev33", "dev34", "dev35", "dev36"), "r03"
  )
  if (nrow(plan) != 60L) stop("Sealed plan must contain 60 jobs.")
  ...
  if (nrow(confirmation) > 15L) stop("Confirmation exceeds the 15-chain cap.")
...
manifest <- list(
  schema_version = "qdesn_postm0_legacy_recheck_v1_advancement_v1",
