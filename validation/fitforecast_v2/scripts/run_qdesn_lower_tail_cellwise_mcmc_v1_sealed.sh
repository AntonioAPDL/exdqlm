...
PLAN="$ADAPTIVE_ROOT/tier_a_sealed_plan.csv"
...
VERIFY_HANDOFF="validation/fitforecast_v2/scripts/verify_qdesn_lower_tail_cellwise_mcmc_v1_sealed_handoff.R"
...
set_stage tier_a_sealed_preflight
record_status tier_a_sealed_preflight STARTED \
...
"$R_SCRIPT" "$VERIFY_HANDOFF" --repo-root "$REPO_ROOT" --state-root "$STATE_ROOT" \
  --output "$STATE_ROOT/tier_a_sealed_handoff_verification.json" \
  > "$STATE_ROOT/tier_a_sealed_handoff_verification.log" 2>&1
record_status tier_a_sealed_preflight COMPLETED \
  "72 roots;dev12-dev15;r03;storage-light;confirmation disabled"

heartbeat_loop & HEARTBEAT_PID="$!"
set_stage tier_a_sealed_resource_gate
wait_for_resources
...
set_stage tier_a_sealed
CONFIG_LIST="$STATE_ROOT/tier_a_sealed_configs.txt"
...
record_status tier_a_sealed STARTED \
...
taskset -c "$CPU_SET" xargs -r -n 1 -P "$PARALLELISM" \
  ...
  < "$CONFIG_LIST" > "$STATE_ROOT/tier_a_sealed_workers.log" 2>&1
RC="$?"
set -e
printf '%s\n' "$RC" > "$STATE_ROOT/tier_a_sealed_worker_exit_code.txt"
"$R_SCRIPT" "$HEALTH" --repo-root "$REPO_ROOT" --run-tag "$RUN_TAG" \
  --plan "$PLAN" --output "$STATE_ROOT/tier_a_sealed_health.csv" \
  > "$STATE_ROOT/tier_a_sealed_health.log" 2>&1 || true
[[ "$RC" -eq 0 ]] || {
  record_status tier_a_sealed FAILED \
    "worker_exit=${RC};same_run_tag_resumes_completed_jobs"; exit "$RC";
}
"$R_SCRIPT" "$VERIFY" --repo-root "$REPO_ROOT" \
  --materialization-root "$MATERIALIZATION_ROOT" --stage tier_a_sealed \
  --plan "$PLAN" --run-tag "$RUN_TAG" \
  --output "$STATE_ROOT/tier_a_sealed_verification.json" \
  > "$STATE_ROOT/tier_a_sealed_verification.log" 2>&1
record_status tier_a_sealed COMPLETED "jobs=${JOBS};finite_storage_light=all"

set_stage tier_a_sealed_closeout
"$R_SCRIPT" "$ADVANCE" --repo-root "$REPO_ROOT" --from tier_a_sealed \
  --run-tag "$RUN_TAG" --materialization-root "$MATERIALIZATION_ROOT" \
  --output-root "$ADAPTIVE_ROOT" \
  > "$STATE_ROOT/advance_after_tier_a_sealed.log" 2>&1
record_status tier_a_sealed_closeout COMPLETED \
...
record_status sealed_complete COMPLETED \
  "Tier-A sealed closed;canonical confirmation waits for explicit approval;article v6 unchanged"
printf 'Lower-tail cellwise MCMC v1 sealed evaluation complete: %s\n' "$RUN_TAG"
