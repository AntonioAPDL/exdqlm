...
RUN_ID="${2:-independent_location_orthogonalized_tau0_v2_$(date +%Y%m%d_%H%M%S)}"
RUN_TAG="${3:-independent-location-orthogonalized-tau0-v2-$(date +%Y%m%d_%H%M%S)__git-$(git -C "$REPO" rev-parse --short HEAD)}"
...
exec 9>"$REPO/reports/shared_fitforecast_v2_orchestration/independent_location_orthogonalized_tau0_v2.lock"
flock -n 9
...
available_cpus() {
  ...
  ps -eLo psr=,pcpu= | awk -v n="$n" -v cap="$MAX_CORE_LOAD" '
    { usage[$1 + 0] += $2 }
    END { for (i = 0; i < n; i++) if ((usage[i] + 0) <= cap) print i }
  '
}
...
resource_snapshot() {
  ...
  printf '%s %.1f %.1f %s' \
    ...
    "$(df -Pk "$REPO" | awk 'NR==2{print $4/1048576}')" "$idle"
}
...
wait_for_resources() {
  ...
  while true; do
    ...
    if awk -v i="$idle" -v m="$mem" -v d="$disk" -v ri="$required" \
      ...
      'BEGIN{exit !((i>=ri)&&(m>=rm)&&(d>=rd))}'; then return 0; fi
...
plan_jobs() {
  local plan="$1"
  if [[ ! -f "$plan" ]]; then printf '0\n'; return; fi
  local lines
  lines="$(wc -l < "$plan")"
  if ((lines <= 1)); then printf '0\n'; else printf '%d\n' "$((lines - 1))"; fi
}
...
run_stage() {
  ...
  jobs="$(plan_jobs "$plan")"
  if ((jobs == 0)); then record "$stage" SKIPPED no_advancing_candidates; return 0; fi
  ...
  if ((${#cpu_ids[@]} != workers)); then
    record "$stage" FAIL "could_not_reserve_$workers_cpus"; return 1
  fi
  ...
  while IFS=$'\t' read -r job_id config_path; do
    worker=$((index % workers)); cpu_id="${cpu_ids[$worker]}"
    printf '%s\t%s\n' "$job_id" "$config_path" >> "$STATE/${stage}_worker_${worker}.tsv"
    printf '%s,%s,%s,%s\n' "$job_id" "$config_path" "$cpu_id" "$worker" >> "$assignment_csv"
    index=$((index + 1))
  done < "$assignment_tsv"
  ...
  "$R" validation/fitforecast_v2/scripts/verify_independent_location_orthogonalized_tau0_v2.R \
...
"$R" validation/fitforecast_v2/scripts/materialize_independent_location_orthogonalized_tau0_v2.R \
...
record materialize COMPLETED "smoke=2;initial_replication=4;screen=33"

run_stage smoke "$MAT/smoke_plan.csv" 2
run_stage initial_replication "$MAT/initial_replication_plan.csv" 4
run_stage screen "$MAT/screen_plan.csv" "$WORKERS"
record discovery_closeout STARTED metric_specific_selection
"$R" validation/fitforecast_v2/scripts/closeout_independent_location_orthogonalized_tau0_v2.R \
  --repo-root "$REPO" --run-tag "$RUN_TAG" --materialization-root "$MAT" \
  --output-root "$CLOSEOUT" --phase discovery > "$STATE/discovery_closeout.log" 2>&1
record discovery_closeout COMPLETED \
  "adaptive_replication_jobs=$(plan_jobs "$MAT/replication_plan.csv")"

run_stage replication "$MAT/replication_plan.csv" "$WORKERS"
record replication_closeout STARTED matched_panel_gate
"$R" validation/fitforecast_v2/scripts/closeout_independent_location_orthogonalized_tau0_v2.R \
  --repo-root "$REPO" --run-tag "$RUN_TAG" --materialization-root "$MAT" \
  --output-root "$CLOSEOUT" --phase replication > "$STATE/replication_closeout.log" 2>&1
record replication_closeout COMPLETED \
  "confirmation_jobs=$(plan_jobs "$MAT/confirmation_plan.csv")"

run_stage confirmation "$MAT/confirmation_plan.csv" "$WORKERS"
record final_closeout STARTED metric_specific_confirmation
"$R" validation/fitforecast_v2/scripts/closeout_independent_location_orthogonalized_tau0_v2.R \
  --repo-root "$REPO" --run-tag "$RUN_TAG" --materialization-root "$MAT" \
  --output-root "$CLOSEOUT" --phase final > "$STATE/final_closeout.log" 2>&1
record final_closeout COMPLETED integration_handoff_required_for_any_gain
