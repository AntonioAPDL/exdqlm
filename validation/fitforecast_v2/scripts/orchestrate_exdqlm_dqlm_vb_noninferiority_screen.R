...
candidates_path <- args$candidates %||%
  file.path(harness_root, "config", "exdqlm_dqlm_vb_noninferiority_screen_candidates_20260704.csv")
run_tag <- args$`run-tag` %||%
  sprintf("20260704_exdqlm_dqlm_vb_noninferiority_screen__git-%s", git_sha)
orchestrator_tag <- args$`orchestrator-tag` %||%
  sprintf("exdqlm-dqlm-vb-noninferiority-orchestrator-%s__git-%s", stamp, git_sha)
...
smoke_candidates <- ffv2_split_csv_arg(args$`smoke-candidates` %||% "c13_reference,ni11_trend100_season1_df09975s099")
...
run_cmd <- function(label, cmd, cmd_args, env = character(0)) {
  ...
  cat(sprintf("[exdqlm-screen] %s start: %s\n", label, Sys.time()))
  status <- system2(cmd, args = cmd_args, stdout = log_path, stderr = log_path, env = env)
  cat(sprintf("[exdqlm-screen] %s status=%d end: %s\n", label, as.integer(status), Sys.time()))
  as.integer(status)
}

prepare_status <- NA_integer_
if (skip_prepare) {
  cat(sprintf("[exdqlm-screen] prepare skipped; using run root: %s\n", run_root))
...
if (!identical(prepare_status, 0L)) {
  stop("exDQLM/DQLM VB non-inferiority prepare failed. Inspect orchestrator logs.",
       call. = FALSE)
}
...
select_smoke_row_ids <- function(manifest_path) {
  ...
  if (!nrow(selected)) {
    selected <- manifest[as.logical(manifest$screen_sentinel), , drop = FALSE]
  }
...
if (do_smoke) {
  ...
  if (!identical(smoke_status, 0L)) {
    stop("exDQLM/DQLM VB non-inferiority smoke failed. Inspect orchestrator logs.",
         call. = FALSE)
  }
}
...
} else if (full_requested && dry_run && !prepare_only) {
  full_status <- run_cmd(
    "full_run",
    "Rscript",
    c(
...
manifest <- list(
  ...
  stage = "exdqlm_dqlm_vb_noninferiority_screen",
  repo_root = repo_root,
