...
if (!nzchar(run_id)) {
  ...
  candidates <- candidates[grepl("/qdesn_mcmc_dynamic_seedrepair_v1_[0-9]{8}_[0-9]{6}$", candidates)]
  if (!length(candidates)) stop("No dynamic-seed-repair v1 orchestration directory found.", call. = FALSE)
...
run_tag <- unname(env[["DISCOVERY_RUN_TAG"]])
if (!length(run_tag) || is.na(run_tag)) run_tag <- ""
expected <- 240L
campaign_parent <- file.path(
  repo_root, "results", "qdesn_mcmc_validation",
  "qdesn_dynamic_fitforecast_v2_500obs_mcmc_dynamic_seedrepair_v1_discovery"
)
...
cat("Full-budget confirmation and article promotion are not launched automatically.\n")
