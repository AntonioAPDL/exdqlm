...
if (!nzchar(run_id)) {
  ...
  candidates <- candidates[grepl("/qdesn_alpha_rho_cellwise_v2_[0-9]{8}_[0-9]{6}$", candidates)]
  if (!length(candidates)) stop("No alpha/rho cellwise v2 orchestration directory found.", call. = FALSE)
...
phase <- if (grepl("^refinement|pipeline_complete", latest_stage)) "refinement" else "coarse"
run_tag <- unname(env[[if (phase == "refinement") "REFINEMENT_RUN_TAG" else "COARSE_RUN_TAG"]])
expected <- if (phase == "coarse") 270L else suppressWarnings(as.integer(env[["REFINEMENT_EXPECTED"]]))
if (!is.finite(expected)) expected <- 0L
...
progress_paths <- if (dir.exists(campaign_root)) list.files(campaign_root, pattern = "progress_trace.*[.]csv$|fit_status[.]txt$|root_status[.]txt$", recursive = TRUE, full.names = TRUE) else character()
...
stamp <- sub("^qdesn_alpha_rho_cellwise_v2_", "", run_id)
session <- paste0("ffv2_qdesn_arv2_", stamp)
...
pct <- if (expected > 0L) 100 * finished / expected else 0
terminal_stage <- latest_stage %in% c("pipeline_complete", "pipeline_stopped_by_coarse_gate")
health <- if (tmux_live && is.finite(age_minutes) && age_minutes <= 30) {
  "ACTIVE"
} else if (tmux_live && grepl("resource_gate", latest_stage)) {
  "WAITING_FOR_RESOURCES"
} else if (tmux_live) {
  "LIVE_REVIEW_PROGRESS"
...
cat("| Phase | Health | Finished | Remaining | Success | Failed | Running | Fit summaries | H1000 files | Heavy payloads | Progress age |\n")
cat("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
cat(sprintf(
...
cat("planned coarse roots: `270`; dynamic refinement cap: `60`; full-budget confirmation: `not launched by this pipeline`\n")
