...
job_root <- idlc_v1_job_root(repo, run_tag, job_id)
...
contracts <- c(
  ...
  dimension = identical(as.integer(job$root_spec$effective_readout_dimension),
                        exact_dimension),
  capacity = exact_dimension <= idlc_v1_max_effective_dimension,
...
design_error <- tryCatch({
  idlc_v1_write_design_diagnostics(job, observed_path, job_root)
  NA_character_
...
metric_values <- idlc_v1_metric_values(job_root)
...
diagnostic_paths <- idlc_v1_required_diagnostic_paths(job_root)
