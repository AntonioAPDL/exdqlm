...
job_ok <- vapply(seq_along(jobs), function(i) {
  x <- jobs[[i]]
  identical(x$stage, "tier_a_confirmation") && x$source_id == "canonical_article" &&
...
checks <- c(
  branch = identical(system("git branch --show-current", intern = TRUE), qdesn_ltcv1_branch),
  plan = nrow(plan) == 6L && all(table(plan$target_cell_id) == 3L) &&
    identical(sort(unique(plan$chain_id)), 1:3),
  manifest = manifest$confirmation_jobs == 6L && isTRUE(manifest$explicit_human_approval) &&
...
if (runtime) {
  runtime_rows <- do.call(rbind, lapply(seq_len(nrow(plan)), function(i) {
    job_root <- qdesn_ltcv1_job_root(repo_root, run_tag, plan$job_id[[i]])
  ...
  checks <- c(checks, runtime_complete = nrow(runtime_rows) == 6L,
...
result <- list(schema_version = "qdesn_lower_tail_cellwise_mcmc_v1_confirmation_verification_v1",
