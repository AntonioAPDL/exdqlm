...
job_root <- qdesn_fgav1_job_root(repo_root, run_tag, job_id)
...
contracts <- c(
  ...
  timeout = identical(as.integer(job$config$validation$timeout_seconds),
                      qdesn_fgav1_timeout_seconds(as.character(job$stage))),
...
metric_values <- qdesn_fgav1_metric_values(job_root)
