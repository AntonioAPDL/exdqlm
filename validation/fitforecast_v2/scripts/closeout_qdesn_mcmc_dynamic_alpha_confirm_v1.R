...
if (nrow(grid) != 30L || nrow(profiles) != 30L || nrow(targets) != 30L ||
    nrow(pair_map) != 18L || nrow(article_context) != 2L ||
    !identical(materialization$source_registry_hash_value, expected_registry_hash)) {
  stop("The closeout inputs do not match the frozen 30-fit contract.", call. = FALSE)
}
...
decision <- if (length(missing_ids) || length(unexpected_ids) || complete_metrics != 30L) {
  "BLOCK_INCOMPLETE_CONFIRMATION"
} else if (contract_passes != 30L || complete_pairs != 18L) {
...
cat(sprintf("observed_specs: %d/30\n", length(observed_ids)))
cat(sprintf("complete_metrics: %d/30\n", complete_metrics))
cat(sprintf("execution_contract_passes: %d/30\n", contract_passes))
cat(sprintf("paired_comparisons: %d/18\n", complete_pairs))
