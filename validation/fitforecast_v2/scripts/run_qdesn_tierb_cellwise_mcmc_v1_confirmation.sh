...
[[ "${QDESN_TBCV1_CONFIRMATION_APPROVED:-false}" == "true" ]] || { echo "Explicit confirmation approval is required." >&2; exit 3; }
[[ "$WORKERS" -ge 1 && "$WORKERS" -le 12 ]] || { echo "WORKERS must be 1..12" >&2; exit 3; }
cd "$REPO_ROOT"
...
MAT="validation/fitforecast_v2/scripts/materialize_qdesn_tierb_cellwise_mcmc_v1_confirmation.R"
VERIFY="validation/fitforecast_v2/scripts/verify_qdesn_tierb_cellwise_mcmc_v1_confirmation.R"
CLOSE="validation/fitforecast_v2/scripts/closeout_qdesn_tierb_cellwise_mcmc_v1_confirmation.R"
LOCK="reports/shared_fitforecast_v2_orchestration/qdesn_tierb_cellwise_mcmc_v1_confirmation.lock"
...
wait_for_resources(){
  while true; do
    ...
    if awk -v l="$load" -v m="$memory" -v d="$disk" -v i="$idle" -v ml="$MAX_LOAD" -v mm="$MIN_MEMORY_GB" -v md="$MIN_DISK_GB" -v w="$WORKERS" 'BEGIN{exit !((l<=ml)&&(m>=mm)&&(d>=md)&&(i>=w))}'; then
      record tier_b_confirmation_resource_gate PASS "load=${load};memory_gb=${memory};disk_gb=${disk};idle_cpus=${idle}"
      return
    fi
    record tier_b_confirmation_resource_gate WAIT "load=${load};memory_gb=${memory};disk_gb=${disk};idle_cpus=${idle};workers=${WORKERS}"
    sleep "$POLL_SECONDS"
  done
}
exec 9>"$LOCK"; flock -n 9 || { echo "Confirmation already active." >&2; exit 3; }
[[ "$(git branch --show-current)" == "validation/qdesn-tierb-cellwise-mcmc-v1-1.0.0" ]] || { echo "Wrong branch" >&2; exit 3; }
...
"$R_SCRIPT" --vanilla -e 'library(exdqlm); stopifnot(as.character(packageVersion("exdqlm")) == "1.0.0")' \
  > "$STATE_ROOT/tier_b_confirmation_package_load.log" 2>&1 || {
...
stage tier_b_confirmation_preflight; record tier_b_confirmation_preflight STARTED "approved;canonical source;sealed handoff;storage"
...
record tier_b_confirmation_preflight COMPLETED \
  "up to 12 chains;canonical article source;inherited AL kernel;5000+20000"
...
JOBS="$(wc -l < "$LIST")"; PARALLELISM="$WORKERS"
[[ "$JOBS" -lt "$PARALLELISM" ]] && PARALLELISM="$JOBS"
stage tier_b_confirmation_resource_gate; wait_for_resources
CPU_SET="${CPU_SET:-$(select_cpus)}"
[[ "$(tr ',' '\n' <<< "$CPU_SET" | sed '/^$/d' | wc -l)" -eq "$WORKERS" ]] || { record tier_b_confirmation_cpu_selection FAILED "cpus=${CPU_SET}"; exit 3; }
record tier_b_confirmation_cpu_selection COMPLETED "workers=${WORKERS};threads=1;cpus=${CPU_SET}"
stage tier_b_confirmation; record tier_b_confirmation STARTED \
  "jobs=${JOBS};parallelism=${PARALLELISM};one_thread_each;resumable"
set +e
taskset -c "$CPU_SET" xargs -r -n 1 -P "$PARALLELISM" "$R_SCRIPT" "$WORKER" --repo-root "$REPO_ROOT" --run-tag "$RUN_TAG" --config < "$LIST" > "$STATE_ROOT/confirmation_workers.log" 2>&1
RC="$?"; set -e; printf '%s\n' "$RC" > "$STATE_ROOT/confirmation_worker_exit_code.txt"
[[ "$RC" -eq 0 ]] || { record tier_b_confirmation FAILED "worker_exit=${RC};resume_same_run_tag"; exit "$RC"; }
...
record tier_b_confirmation COMPLETED "${JOBS}/${JOBS} finite;storage-light;config-matched"
stage tier_b_confirmation_closeout
"$R_SCRIPT" "$CLOSE" --repo-root "$REPO_ROOT" --state-root "$STATE_ROOT" --run-tag "$RUN_TAG" > "$STATE_ROOT/confirmation_closeout.log" 2>&1
record tier_b_confirmation_closeout COMPLETED "promotion ledger materialized;article unchanged"
stage confirmation_complete; record confirmation_complete COMPLETED "manual promotion review required;article v6 unchanged"
printf 'Tier-B canonical confirmation complete: %s\n' "$RUN_TAG"
