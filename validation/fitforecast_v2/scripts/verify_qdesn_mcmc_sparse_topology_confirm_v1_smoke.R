...
output_root <- normalizePath(get_arg(
  "--output-root",
  file.path("reports", "shared_fitforecast_v2_orchestration", "sparse_topology_smoke_audit")
...
pass <- nrow(grid) == 3L && nrow(targets) == 3L && nrow(audit) == 3L &&
  setequal(expected_ids, observed_ids) && length(fit_paths) == 3L &&
  length(horizon_paths) == 3L && all(metric_complete) &&
...
cat(sprintf("smoke: %s (%d/3 requests; %d/3 finite summaries)\n",
            if (pass) "PASS" else "FAIL", nrow(observed), sum(metric_complete)))
if (!pass) stop("Sparse-topology confirmation smoke failed.", call. = FALSE)
