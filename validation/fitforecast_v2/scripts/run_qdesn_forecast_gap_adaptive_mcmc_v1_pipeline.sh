...
RUN_ID="${2:-qdesn_forecast_gap_adaptive_mcmc_v1_$(date +%Y%m%d_%H%M%S)}"
RUN_TAG="${3:-qdesn-forecast-gap-adaptive-v1-$(date +%Y%m%d_%H%M%S)__git-$(git -C "$REPO_ROOT" rev-parse --short HEAD)}"
...
ADVANCE="validation/fitforecast_v2/scripts/advance_qdesn_forecast_gap_adaptive_mcmc_v1.R"
LOCK_FILE="reports/shared_fitforecast_v2_orchestration/qdesn_forecast_gap_adaptive_mcmc_v1.lock"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "Another forecast-gap adaptive pipeline holds $LOCK_FILE" >&2
  exit 2
fi
...
fi
{
  ...
  printf 'REPLICATION_MAX=64\nSEALED_MAX=96\nCONFIRMATION_MAX=42\n'
  printf 'AUTO_ADVANCE_WITH_HARD_GATES=TRUE\nARTICLE_UPDATE_AUTOMATIC=FALSE\n'
} > "$STATE_ROOT/run_tags.env"

set_stage materialize
record_status materialize STARTED "v7 authority;84 history-aware candidates;8 source blocks"
"$R_SCRIPT" validation/fitforecast_v2/scripts/materialize_qdesn_forecast_gap_adaptive_mcmc_v1.R \
...
record_status materialize COMPLETED "2 smoke;8 calibration;184 discovery"

set_stage tests
"$R_SCRIPT" -e 'pkgload::load_all(".", quiet=TRUE); stopifnot(as.character(packageVersion("exdqlm")) == "1.0.0")' \
  > "$STATE_ROOT/package_load.log" 2>&1
"$R_SCRIPT" -e 'pkgload::load_all(".", quiet=TRUE); testthat::test_file("validation/fitforecast_v2/tests/testthat/test-qdesn-forecast-gap-adaptive-mcmc-v1.R", reporter="summary", stop_on_failure=TRUE, stop_on_warning=TRUE)' \
...
record_status tests COMPLETED "package;schema;history;source;storage;stage contracts pass"
...
"$R_SCRIPT" -e 'x <- read.csv(commandArgs(TRUE)[1]); stopifnot(nrow(x)==8L, all(x$status=="SUCCESS"), all(x$binary_count==0L), max(x$elapsed_seconds, na.rm=TRUE)<=21600)' \
  "$STATE_ROOT/calibration_verification_runtime.csv" \
  > "$STATE_ROOT/calibration_runtime_gate.log" 2>&1
record_status calibration_runtime_gate PASS "8 representative roots;maximum elapsed <= 6 hours"

set_stage discovery
run_stage discovery "$MATERIALIZATION_ROOT/discovery_plan.csv"

set_stage discovery_closeout
"$R_SCRIPT" "$ADVANCE" --repo-root "$REPO_ROOT" --from discovery \
  --run-tag "$RUN_TAG" --materialization-root "$MATERIALIZATION_ROOT" \
  --output-root "$ADAPTIVE_ROOT" \
  > "$STATE_ROOT/advance_after_discovery.log" 2>&1
record_status discovery_closeout COMPLETED \
  "paired rankings complete;64-job replication plan materialized"

set_stage replication
run_stage replication "$ADAPTIVE_ROOT/replication_plan.csv"

set_stage replication_closeout
"$R_SCRIPT" "$ADVANCE" --repo-root "$REPO_ROOT" --from replication \
  --run-tag "$RUN_TAG" --materialization-root "$MATERIALIZATION_ROOT" \
  --output-root "$ADAPTIVE_ROOT" \
  > "$STATE_ROOT/advance_after_replication.log" 2>&1
record_status replication_closeout COMPLETED \
  "four-source rankings complete;96-job sealed plan materialized"

set_stage sealed
run_stage sealed "$ADAPTIVE_ROOT/sealed_plan.csv"

set_stage sealed_closeout
"$R_SCRIPT" "$ADVANCE" --repo-root "$REPO_ROOT" --from sealed \
  --run-tag "$RUN_TAG" --materialization-root "$MATERIALIZATION_ROOT" \
  --output-root "$ADAPTIVE_ROOT" \
  > "$STATE_ROOT/advance_after_sealed.log" 2>&1
record_status sealed_closeout COMPLETED \
  "metric-specific sealed decisions and confirmation handoff materialized"

set_stage confirmation_materialize
"$R_SCRIPT" validation/fitforecast_v2/scripts/materialize_qdesn_forecast_gap_adaptive_mcmc_v1_confirmation.R \
  --repo-root "$REPO_ROOT" --state-root "$STATE_ROOT" \
  --output-root "$STATE_ROOT/confirmation" \
  > "$STATE_ROOT/confirmation_materialize.log" 2>&1
"$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_forecast_gap_adaptive_mcmc_v1_confirmation.R \
  --repo-root "$REPO_ROOT" --state-root "$STATE_ROOT" --run-tag "$RUN_TAG" \
...
if [[ "$CONFIRMATION_JOBS" -gt 0 ]]; then
  set_stage confirmation
  run_stage confirmation "$CONFIRMATION_PLAN"
  "$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_forecast_gap_adaptive_mcmc_v1_confirmation.R \
    --repo-root "$REPO_ROOT" --state-root "$STATE_ROOT" --run-tag "$RUN_TAG" \
    --runtime true --output "$STATE_ROOT/confirmation_verification.json" \
    > "$STATE_ROOT/confirmation_verification.log" 2>&1
fi

set_stage confirmation_closeout
"$R_SCRIPT" validation/fitforecast_v2/scripts/closeout_qdesn_forecast_gap_adaptive_mcmc_v1_confirmation.R \
  --repo-root "$REPO_ROOT" --state-root "$STATE_ROOT" --run-tag "$RUN_TAG" \
  > "$STATE_ROOT/confirmation_closeout.log" 2>&1
record_status confirmation_closeout COMPLETED \
  "metric-specific strict-mean promotion ledger;diagnostics descriptive;article unchanged"

set_stage complete
write_heartbeat
record_status complete COMPLETED \
  "full campaign closed;integration handoff ready;article v7 unchanged"
printf 'Forecast-gap adaptive MCMC v1 complete: %s\n' "$RUN_TAG"
