...
gate <- qdesn_ssv2_read_csv(file.path(adaptive_root, "tier_a_discovery_gate.csv"))
advance <- qdesn_ssv2_read_json(file.path(
  adaptive_root, "advance_after_tier_a_discovery.json"
))
...
targets <- qdesn_ssv2_read_csv(file.path(
  ...
  paste0(qdesn_ltcv1_stage, "_target_cells.csv")
))
...
job_checks <- vapply(seq_along(jobs), function(i) {
  ...
  identical(as.character(job$job_id), as.character(row$job_id)) &&
    identical(as.character(job$stage), "tier_a_replication") &&
    identical(as.character(job$source_id), "dev11") &&
...
checks <- c(
  ...
  advancement = identical(as.character(advance$decision),
                          "advance_to_tier_a_replication"),
  target_cells = nrow(targets) == 6L && length(unique(plan$target_cell_id)) == 6L,
  ranking = nrow(ranking) == 18L && all(table(ranking$target_cell_id) == 3L) &&
    all(table(ranking$rank) == 6L),
  plan_shape = nrow(plan) == 24L && all(table(plan$target_cell_id) == 4L),
  paired_source = identical(unique(plan$source_id), "dev11") &&
...
result <- list(
  schema_version = "qdesn_lower_tail_cellwise_mcmc_v1_replication_handoff_v1",
  ...
  plan_sha256 = qdesn_ssv2_sha256(file.path(
    adaptive_root, "tier_a_replication_plan.csv"
  ))
)
