...
selected_designs <- read_csv(paths$selected_designs)
...
if (nrow(grid) != 21L || nrow(profiles) != 21L || nrow(targets) != 21L ||
    nrow(pair_map) != 9L || nrow(selected_designs) != 7L ||
    nrow(topology_audit) != 21L || !all(as_bool(topology_audit$topology_valid)) ||
    nrow(article_context) != 2L ||
    !identical(materialization$source_registry_hash_value, expected_registry_hash)) {
  stop("The closeout inputs do not match the frozen 21-fit contract.", call. = FALSE)
}
...
profile_fields <- c(
  ...
  "recurrent_edges_target", "base_design_id", "replication_purpose"
)
...
replication_targets <- data.frame(
  source_base_design_id = c(
  ...
  metric = c(
    ...
    "forecast_check_loss_H1000", "fit_qtrue_rmse",
    "forecast_qtrue_mae_H1000", "forecast_check_loss_H1000"
  ),
  stringsAsFactors = FALSE
)
replication_rows <- lapply(seq_len(nrow(replication_targets)), function(i) {
  target <- replication_targets[i, , drop = FALSE]
  values <- if (nrow(metrics)) {
    num(metrics[[target$metric]][
      metrics$source_base_design_id == target$source_base_design_id
    ])
  } else numeric()
  values <- values[is.finite(values)]
  authority_value <- num(article_context[
    article_context$model_variant == target$model_variant,
    target$metric, drop = TRUE
  ])
  below <- if (length(values) && length(authority_value)) {
    values < authority_value[[1L]] - 1e-10
  } else logical()
  median_value <- if (length(values)) stats::median(values) else NA_real_
  data.frame(
    source_base_design_id = target$source_base_design_id,
    model_variant = target$model_variant,
    metric = target$metric,
    authority_value = if (length(authority_value)) authority_value[[1L]] else NA_real_,
    fresh_replicates_expected = 3L,
    fresh_replicates_observed = length(values),
    fresh_replicates_below_authority = sum(below),
    fresh_minimum = if (length(values)) min(values) else NA_real_,
    fresh_median = median_value,
    fresh_maximum = if (length(values)) max(values) else NA_real_,
    median_improves_authority = is.finite(median_value) && length(authority_value) &&
      median_value < authority_value[[1L]] - 1e-10,
    two_of_three_improve = length(values) == 3L && sum(below) >= 2L,
    replication_confirmed = length(values) == 3L && sum(below) >= 2L &&
      is.finite(median_value) && length(authority_value) &&
      median_value < authority_value[[1L]] - 1e-10,
    stringsAsFactors = FALSE
  )
})
replication_summary <- do.call(rbind, replication_rows)
replication_summary_path <- write_csv(
  replication_summary, file.path(output_root, "promoted_metric_replication_summary.csv")
)
...
replication_confirmed_count <- sum(as_bool(replication_summary$replication_confirmed))
decision <- if (length(missing_ids) || length(unexpected_ids) || complete_metrics != 21L) {
  "BLOCK_INCOMPLETE_CONFIRMATION"
} else if (contract_passes != 21L || complete_pairs != 9L) {
  "BLOCK_EXECUTION_CONTRACT_FAILURE"
} else if (nrow(storage_audit)) {
  "BLOCK_STORAGE_POLICY_FAILURE"
} else if (winner_count > 0L) {
  "CONFIRMATION_COMPLETE_WITH_NEW_METRIC_IMPROVEMENTS_PENDING_REVIEW"
} else if (replication_confirmed_count == nrow(replication_summary)) {
  "CONFIRMATION_COMPLETE_ALL_PROMOTED_METRICS_REPLICATED"
} else {
  "CONFIRMATION_COMPLETE_PARTIAL_OR_NO_PROMOTED_METRIC_REPLICATION"
}
...
gate <- list(
  ...
  promoted_metric_replications_confirmed = replication_confirmed_count,
  promoted_metric_replications_expected = nrow(replication_summary),
  ...
  next_action = switch(
    decision,
    CONFIRMATION_COMPLETE_WITH_NEW_METRIC_IMPROVEMENTS_PENDING_REVIEW =
      "Review fresh winners and replication evidence before a separate article promotion.",
    CONFIRMATION_COMPLETE_ALL_PROMOTED_METRICS_REPLICATED =
      "Freeze repeatability evidence; retain the v3 article envelope unless a fresh value is lower.",
    CONFIRMATION_COMPLETE_PARTIAL_OR_NO_PROMOTED_METRIC_REPLICATION =
      "Retain v3 as best-observed evidence and document which promoted metrics did not replicate.",
...
cat(sprintf("observed_specs: %d/21\n", length(observed_ids)))
cat(sprintf("complete_metrics: %d/21\n", complete_metrics))
cat(sprintf("execution_contract_passes: %d/21\n", contract_passes))
cat(sprintf("paired_comparisons: %d/9\n", complete_pairs))
cat(sprintf("promoted_metric_replications: %d/%d\n",
            replication_confirmed_count, nrow(replication_summary)))
