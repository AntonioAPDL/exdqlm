...
pair_keys <- unique(results[c("target_cell_id", "source_id", "reservoir_seed_id")])
pair_rows <- lapply(seq_len(nrow(pair_keys)), function(i) {
  key <- pair_keys[i, , drop = FALSE]
  x <- results[
    results$target_cell_id == key$target_cell_id &
      results$source_id == key$source_id &
      results$reservoir_seed_id == key$reservoir_seed_id,
    , drop = FALSE
  ]
  ...
  delta <- finalist$objective_value[[1L]] - anchor$objective_value[[1L]]
  data.frame(
    target_cell_id = key$target_cell_id,
    source_id = key$source_id,
    reservoir_seed_id = key$reservoir_seed_id,
    objective_metric = anchor$objective_metric[[1L]],
    anchor_candidate_id = anchor$candidate_id[[1L]],
    finalist_candidate_id = finalist$candidate_id[[1L]],
    anchor_value = anchor$objective_value[[1L]],
    finalist_value = finalist$objective_value[[1L]],
    finalist_minus_anchor = delta,
    finalist_improvement_pct = 100 * (anchor$objective_value[[1L]] -
      finalist$objective_value[[1L]]) / anchor$objective_value[[1L]],
...
summary_rows <- lapply(split(pairs, pairs$target_cell_id), function(x) {
  ...
  data.frame(
    ...
    objective_metric = x$objective_metric[[1L]],
    ...
    current_article_value = target$current_value[[1L]],
    comparator_value = target$comparator_value[[1L]],
...
summary <- summary[order(summary$decision, -summary$mean_improvement_pct), , drop = FALSE]
summary_path <- qdesn_ssv2_write_csv(
  summary, file.path(output_root, "paired_selection_ledger.csv")
)

eligible_ids <- summary$finalist_candidate_id[
  summary$decision == "ELIGIBLE_FOR_CANONICAL_FULL_BUDGET_CONFIRMATION"
]
...
confirmation$selection_ledger_path <- rep(summary_path, nrow(confirmation))
confirmation$selection_ledger_sha256 <- rep(
  qdesn_ssv2_sha256(summary_path), nrow(confirmation)
)
...
manifest_path <- qdesn_ssv2_write_json(list(
  ...
  run_tag = run_tag,
  jobs = nrow(results),
  paired_blocks = nrow(pairs),
  target_cells = nrow(summary),
  eligible_target_cells = sum(
    summary$decision == "ELIGIBLE_FOR_CANONICAL_FULL_BUDGET_CONFIRMATION"
  ),
  selection_rule = list(
  ...
  outputs = list(
    ...
    selection = list(path = summary_path, sha256 = qdesn_ssv2_sha256(summary_path)),
...
cat(sprintf(
  "decision=PAIRED_CALIBRATION_CLOSED jobs=%d pairs=%d cells=%d eligible=%d manifest=%s\n",
  nrow(results), nrow(pairs), nrow(summary), nrow(confirmation), manifest_path
))
