...
if (!identical(as.character(description[1L, "Version"]), "1.0.0")) {
  stop("Forecast-gap adaptive MCMC v1 requires exdqlm 1.0.0.", call. = FALSE)
}

output_root <- normalizePath(get_arg(
  "--output-root",
  file.path(repo_root, "reports", "shared_fitforecast_v2_orchestration",
            "qdesn_forecast_gap_adaptive_mcmc_v1_materialization")
...
targets <- qdesn_fgav1_targets(
  repo_root, freeze_requests = !has_flag("--use-frozen-designs") &&
    !has_flag("--reuse-frozen-requests")
)
target_path <- qdesn_ssv2_write_csv(targets, paste0(stub, "_target_cells.csv"))
metric_roles <- qdesn_fgav1_metric_role_ledger(repo_root)
metric_role_path <- qdesn_ssv2_write_csv(
  metric_roles, paste0(stub, "_metric_role_ledger.csv")
)
...
history <- history[
  !grepl(qdesn_fgav1_stage, history$source_file, fixed = TRUE), , drop = FALSE
]
history_path <- qdesn_ssv2_write_csv(history, history_path)
evidence_source_file <- paste0(stub, "_evidence_source_manifest.csv")
evidence_sources <- if (file.exists(evidence_source_file) &&
                        !has_flag("--refresh-history")) {
  qdesn_ssv2_read_csv(evidence_source_file)
} else {
  qdesn_fgav1_evidence_source_manifest(repo_root, history)
}
evidence_source_path <- qdesn_ssv2_write_csv(
  evidence_sources, evidence_source_file
)
...
if (use_frozen_designs) {
  ...
  parents <- qdesn_fgav1_parent_profiles(repo_root, targets)
  profiles <- qdesn_fgav1_candidate_profiles(parents, universe, history)
}
if (nrow(parents) != 8L || nrow(profiles) != 84L ||
...
if (length(replicates) != 8L || sum(roles == "discovery") != 2L ||
    sum(roles == "replication") != 2L || sum(roles == "sealed_holdout") != 4L) {
  stop("Source contract must contain 2 discovery, 2 replication, and 4 sealed blocks.",
       call. = FALSE)
}
seed_rows <- list()
root_rows <- list()
for (i in seq_along(replicates)) {
  ...
  manifest <- list(
    meta = list(
      ...
      notes = sprintf("Forecast-gap adaptive %s source %s.",
    ...
    qdesn_materialization = list(staged_root = file.path(
      "results", "qdesn_mcmc_validation", qdesn_fgav1_stage,
...
target_ids <- targets$target_cell_id

smoke_profiles <- rbind(
  profiles[profiles$likelihood_target == "al", , drop = FALSE][1L, ],
  profiles[profiles$likelihood_target == "exal", , drop = FALSE][1L, ]
)
smoke_plan <- do.call(rbind, lapply(seq_len(nrow(smoke_profiles)), function(i) {
  p <- smoke_profiles[i, , drop = FALSE]
  write_job(p, target_map[[p$target_cell_id[[1L]]]], "dev37", "smoke")
}))

calibration_plan <- do.call(rbind, lapply(target_ids, function(cell_id) {
  cell <- profiles[profiles$target_cell_id == cell_id, , drop = FALSE]
  p <- cell[which.max(cell$effective_readout_dimension), , drop = FALSE]
  write_job(p, target_map[[cell_id]], "dev37", "calibration")
}))
...
for (cell_id in target_ids) {
  cell <- rbind(
    profiles[profiles$target_cell_id == cell_id, , drop = FALSE],
    parents[parents$target_cell_id == cell_id, , drop = FALSE]
  )
  for (i in seq_len(nrow(cell))) {
    for (source_id in c("dev37", "dev38")) {
      k <- k + 1L
      discovery_rows[[k]] <- write_job(
        cell[i, , drop = FALSE], target_map[[cell_id]], source_id,
        "discovery", reservoir_seed_id = "r01"
      )
    }
  }
}
discovery_plan <- do.call(rbind, discovery_rows)
if (nrow(smoke_plan) != 2L || nrow(calibration_plan) != 8L ||
    nrow(discovery_plan) != 184L) {
  stop("Materialized plan counts do not match 2/8/184.", call. = FALSE)
}
...
plan_paths <- c(
  ...
  discovery = qdesn_ssv2_write_csv(
    discovery_plan, file.path(output_root, "discovery_plan.csv")
  )
)
...
tracked_paths <- c(
  source_cfg_path, target_path, metric_role_path, history_path,
  evidence_source_path, parent_path, profile_path,
  ...
  file.path(repo_root, "validation", "fitforecast_v2", "scripts", c(
    "materialize_qdesn_forecast_gap_adaptive_mcmc_v1.R",
    "run_qdesn_forecast_gap_adaptive_mcmc_v1_chain.R",
    "healthcheck_qdesn_forecast_gap_adaptive_mcmc_v1.R",
    "verify_qdesn_forecast_gap_adaptive_mcmc_v1.R",
    "advance_qdesn_forecast_gap_adaptive_mcmc_v1.R",
    "materialize_qdesn_forecast_gap_adaptive_mcmc_v1_confirmation.R",
    ...
    "promote_qdesn_forecast_gap_adaptive_mcmc_v1.R",
    "verify_qdesn_forecast_gap_adaptive_mcmc_v1_promotion.R"
  )),
  ...
  file.path(repo_root, "validation", "fitforecast_v2", "docs",
            c(
              ...
              "QDESN_FORECAST_GAP_ADAPTIVE_MCMC_V1_V8_CLOSEOUT_PLAN_2026-08-19.md"
            ))
)
...
manifest <- list(
  schema_version = "qdesn_forecast_gap_adaptive_mcmc_v1_materialization_v1",
  generated_at = as.character(Sys.time()),
  git_commit = system("git rev-parse HEAD", intern = TRUE),
  package_version = "1.0.0",
  base_authority_commit = qdesn_fgav1_base_commit,
  ...
  counts = list(
    target_cells = nrow(targets), tier_a_cells = sum(targets$tier == "A"),
    ...
    discovery_jobs = nrow(discovery_plan), metric_roles = nrow(metric_roles),
    evidence_sources = nrow(evidence_sources)
  ),
  gates = list(
    replication = "three candidates per cell after two-source paired discovery evidence",
    sealed = "two candidates per cell after two additional replication sources",
    confirmation = "metric-specific sealed finalists; explicit campaign authorization",
    article = "unchanged until canonical full-budget confirmation"
  ),
  article_state = "v7_frozen_unchanged"
)
