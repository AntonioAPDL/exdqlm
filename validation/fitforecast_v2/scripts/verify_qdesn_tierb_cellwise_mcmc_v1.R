...
tracked_manifest <- qdesn_ssv2_read_csv(paste0(stub, "_tracked_manifest.csv"))
tracked_paths <- file.path(repo_root, tracked_manifest$relative_path)
tracked_manifest_ok <- all(file.exists(tracked_paths)) &&
  all(as.numeric(file.info(tracked_paths)$size) == tracked_manifest$bytes) &&
  all(vapply(tracked_paths, qdesn_ssv2_sha256, character(1L)) ==
        tracked_manifest$sha256)

plan_paths <- c(
  smoke = file.path(materialization_root, "smoke_plan.csv"),
  calibration = file.path(materialization_root, "calibration_plan.csv"),
  tier_b_discovery = file.path(materialization_root, "tier_b_discovery_plan.csv")
)
...
job_checks <- lapply(jobs, function(job) {
  ...
  data.frame(
    schema = identical(as.character(job$schema_version),
                       "qdesn_tierb_cellwise_mcmc_v1_job_v1"),
    ...
    likelihood = identical(likelihood, "al") &&
...
static_checks <- c(
  target_count = nrow(targets) == 4L && all(targets$tier == "B") &&
    all(targets$likelihood_target == "al"),
  profile_count = nrow(profiles) == 32L && nrow(parents) == 4L,
  ...
  configured_unrun = sum(
    profiles$history_disposition == "admitted_configured_unrun_not_evaluated"
  ) == 16L,
  history_excluded = !any(profiles$profile_signature[
    profiles$history_disposition == "new_signature_not_in_history"
  ] %in% history$profile_signature),
  ...
  sources = length(roles) == 8L && sum(roles == "discovery") == 3L &&
    sum(roles == "replication") == 1L && sum(roles == "sealed_holdout") == 4L,
  plans = nrow(plans$smoke) == 2L && nrow(plans$calibration) == 4L &&
    nrow(plans$tier_b_discovery) == 108L,
  ...
  tracked_manifest = tracked_manifest_ok,
  source_binary_contract =
    identical(as.integer(manifest$counts$source_authority_binary_files), 64L) &&
    as.numeric(manifest$counts$source_authority_binary_bytes) > 0 &&
    identical(as.integer(manifest$counts$fitted_model_binary_files), 0L),
...
if (stage != "static") {
  ...
  runtime <- do.call(rbind, lapply(seq_len(nrow(plan)), function(i) {
    ...
    metrics <- qdesn_tbcv1_metric_values(root)
...
result <- list(
  schema_version = "qdesn_tierb_cellwise_mcmc_v1_verification_v1",
