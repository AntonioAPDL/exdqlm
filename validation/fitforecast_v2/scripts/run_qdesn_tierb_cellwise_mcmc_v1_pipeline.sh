...
RUN_ID="${2:-qdesn_tierb_cellwise_mcmc_v1_$(date +%Y%m%d_%H%M%S)}"
RUN_TAG="${3:-qdesn-tierb-cellwise-v1-$(date +%Y%m%d_%H%M%S)__git-$(git -C "$REPO_ROOT" rev-parse --short HEAD)}"
R_SCRIPT="${R_SCRIPT:-/data/jaguir26/local/opt/R/4.6.0/bin/Rscript}"
R_BIN="${R_BIN:-$(dirname "$R_SCRIPT")/R}"
WORKERS="${WORKERS:-16}"
...
PACKAGE_FINGERPRINT_FILE="$PACKAGE_LIB_ROOT/.package_source_fingerprint"
PACKAGE_INSTALL_LOCK="reports/shared_fitforecast_v2_orchestration/qdesn_tierb_cellwise_mcmc_v1_package_install.lock"
mkdir -p "$STATE_ROOT" "$MATERIALIZATION_ROOT" "$ADAPTIVE_ROOT" "$PACKAGE_LIB_ROOT"
...
fi
{
  ...
  printf 'WORKTREE=%s\nWORKERS=%s\nTHREADS_PER_WORKER=1\n' \
    "$REPO_ROOT" "$WORKERS"
  printf 'CPU_SET=PENDING_RESOURCE_GATE\nSMOKE=2\nCALIBRATION=4\nTIER_B_DISCOVERY=108\n'
  printf 'AUTO_STOP_AFTER_TIER_B_DISCOVERY=TRUE\nARTICLE_UPDATE_AUTOMATIC=FALSE\n'
} > "$STATE_ROOT/run_tags.env"

set_stage package_install
PACKAGE_FINGERPRINT="$(git ls-files -s -- DESCRIPTION NAMESPACE R src | sha256sum | awk '{print $1}')"
exec 8>"$PACKAGE_INSTALL_LOCK"
flock 8
INSTALLED_FINGERPRINT="$(cat "$PACKAGE_FINGERPRINT_FILE" 2>/dev/null || true)"
INSTALLED_VERSION="$($R_SCRIPT --vanilla -e 'p <- Sys.getenv("R_LIBS_USER"); .libPaths(c(strsplit(p, .Platform$path.sep, fixed=TRUE)[[1]], .libPaths())); cat(if (requireNamespace("exdqlm", quietly=TRUE)) as.character(packageVersion("exdqlm")) else "")' 2>/dev/null || true)"
if [[ "$INSTALLED_FINGERPRINT" != "$PACKAGE_FINGERPRINT" || "$INSTALLED_VERSION" != "1.0.0" ]]; then
  record_status package_install STARTED "single_build;fingerprint=${PACKAGE_FINGERPRINT}"
  MAKEFLAGS=-j1 "$R_BIN" CMD INSTALL --preclean --clean --no-multiarch \
    --library="$PACKAGE_LIB_ROOT" "$REPO_ROOT" \
    > "$STATE_ROOT/package_install.log" 2>&1
  printf '%s\n' "$PACKAGE_FINGERPRINT" > "$PACKAGE_FINGERPRINT_FILE"
fi
flock -u 8
"$R_SCRIPT" --vanilla -e 'p <- Sys.getenv("R_LIBS_USER"); .libPaths(c(strsplit(p, .Platform$path.sep, fixed=TRUE)[[1]], .libPaths())); library(exdqlm); stopifnot(as.character(packageVersion("exdqlm")) == "1.0.0")' \
  > "$STATE_ROOT/package_load.log" 2>&1
record_status package_install COMPLETED "version=1.0.0;fingerprint=${PACKAGE_FINGERPRINT}"

set_stage materialize
record_status materialize STARTED \
  "v6 authority;32 candidates;16 configured-unrun;16 new signatures;8 source blocks"
...
record_status materialize COMPLETED "2 smoke;4 calibration;108 discovery"

set_stage tests
"$R_SCRIPT" --vanilla -e 'library(exdqlm); testthat::test_file("validation/fitforecast_v2/tests/testthat/test-qdesn-tierb-cellwise-mcmc-v1.R", reporter="summary", stop_on_failure=TRUE, stop_on_warning=TRUE)' \
...
"$R_SCRIPT" -e 'x <- read.csv(commandArgs(TRUE)[1]); stopifnot(nrow(x)==4L, all(x$status=="SUCCESS"), all(x$binary_count==0L), max(x$elapsed_seconds, na.rm=TRUE)<=21600)' \
  "$STATE_ROOT/calibration_verification_runtime.csv" \
  > "$STATE_ROOT/calibration_runtime_gate.log" 2>&1
record_status calibration_runtime_gate PASS "4 representative roots;maximum elapsed <= 6 hours"

set_stage tier_b_discovery
run_stage tier_b_discovery "$MATERIALIZATION_ROOT/tier_b_discovery_plan.csv"

set_stage tier_b_discovery_closeout
"$R_SCRIPT" "$ADVANCE" --repo-root "$REPO_ROOT" --from tier_b_discovery \
  --run-tag "$RUN_TAG" --materialization-root "$MATERIALIZATION_ROOT" \
  --output-root "$ADAPTIVE_ROOT" \
  > "$STATE_ROOT/advance_after_tier_b_discovery.log" 2>&1
record_status tier_b_discovery_closeout COMPLETED \
  "paired rankings complete;16-job replication plan materialized;not launched"

set_stage complete
write_heartbeat
record_status complete COMPLETED \
  "Tier-B discovery closed;replication waits for review;article v6 unchanged"
printf 'Tier-B cellwise MCMC v1 discovery complete: %s\n' "$RUN_TAG"
