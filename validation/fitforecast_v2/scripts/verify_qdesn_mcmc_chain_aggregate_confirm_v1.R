...
handoff <- read_csv("_chain_handoff.csv")
...
chain_seed_ok <- vapply(split(seq_len(nrow(profiles)), profiles$sampler_pair_id), function(i) {
...
checks <- c(
  ...
  frozen_inputs = nrow(frozen) == 7L && all(file.exists(frozen$path)) &&
  ...
  selected_designs = nrow(designs) == 4L &&
    setequal(designs$source_base_design_id, expected_sources),
  profile_count = nrow(profiles) == 12L && !anyDuplicated(profiles$screening_profile_id),
  profile_roles = all(
    profiles$comparison_role == "chain_aggregate_followup_candidate"
  ),
  ...
  assignment_count = nrow(assignments) == 12L && !anyDuplicated(assignments$assignment_id),
  chain_handoff = nrow(handoff) == 4L &&
    setequal(handoff$source_base_design_id, expected_sources) &&
    all(handoff$expected_total_chains == 5L) &&
    all(handoff$estimator_id == "median_of_chain_posterior_point_paths_v1"),
  chain_seeds = length(chain_seed_ok) == 12L && all(chain_seed_ok),
  fresh_seeds = min(profiles$mcmc_seed) >= 981001L &&
    min(profiles$mcmc_rng_seed) >= 982001L &&
    min(profiles$vb_warm_start_seed) >= 983001L &&
    min(profiles$synthesis_seed) >= 984001L,
  topology_audit = nrow(topology) == 12L && all(as_bool(topology$topology_valid)) &&
    all(topology$recurrent_nnz == topology$recurrent_edges_target) &&
    all(topology$dynamic_input_nnz >= 1L),
  grid_count = nrow(grid) == 12L && !anyDuplicated(grid$root_id),
  spec_count = nrow(specs) == 12L && !anyDuplicated(specs$spec_id),
  likelihood_filter = all(specs$likelihood_family == specs$likelihood_target),
  seed_contract = nrow(seed_audit) == 12L && all(seed_audit$status == "PASS"),
  ...
  workers = defaults$runtime$workers == 12L &&
    defaults$runtime$campaign_workers == 12L && defaults$runtime$threads == 1L,
  ...
  smoke = nrow(smoke_grid) == 2L && nrow(smoke_specs) == 2L &&
  ...
  replication_gate = nrow(promotion) == 2L &&
    all(promotion$existing_chains == 2L) && all(promotion$fresh_chains == 3L) &&
    all(promotion$total_chains == 5L) &&
    all(promotion$estimator_id == "median_of_chain_posterior_point_paths_v1") &&
    all(as_bool(promotion$aggregate_must_improve)) &&
    !any(as_bool(promotion$posterior_pooling_claim)),
...
if (!all(rows$passed)) {
  ...
  stop("Chain-aggregate confirmation contract verification failed.", call. = FALSE)
}
cat(sprintf("PASS: %d checks; %d full specs; %d five-chain handoffs.\n",
            nrow(rows), nrow(specs), nrow(handoff)))
