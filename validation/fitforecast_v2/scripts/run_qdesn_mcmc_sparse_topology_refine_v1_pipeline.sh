...
RUN_ID="${2:-qdesn_mcmc_sparse_topology_refine_v1_$(date +%Y%m%d_%H%M%S)}"
...
LOCK_FILE="reports/shared_fitforecast_v2_orchestration/qdesn_mcmc_sparse_topology_refine_v1.lock"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "Another sparse-topology refinement v1 pipeline holds $LOCK_FILE" >&2
  exit 2
fi
...
EXPECTED_BRANCH="validation/qdesn-mcmc-sparse-topology-refine-v1-1.0.0"
...
CONFIRMATION_RUN_TAG="qdesn-strv1-full-${STAMP}__git-${GIT_SHORT}"
...
"$R_SCRIPT" validation/fitforecast_v2/scripts/materialize_qdesn_mcmc_sparse_topology_refine_v1.R \
...
record_status "contract_materialize" "COMPLETED" "168-spec contract regenerated without drift"

set_stage "contract_verify"
record_status "contract_verify" "STARTED" "branch=${BRANCH};commit=${GIT_SHA}"
"$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_mcmc_sparse_topology_refine_v1.R \
...
PREPARE_TAG="qdesn-strv1-prepare-${STAMP}__git-${GIT_SHORT}"
...
set_stage "smoke"
SMOKE_TAG="qdesn-strv1-smoke-${STAMP}__git-${GIT_SHORT}"
...
"$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_mcmc_sparse_topology_refine_v1_smoke.R \
  --run-tag "$SMOKE_TAG" --output-root "$STATE_ROOT/smoke_audit" \
  > "$STATE_ROOT/smoke_audit.log" 2>&1
record_status "smoke" "COMPLETED" "3/3 finite contract-valid fits;runner_exit=${SMOKE_RUNNER_RC}"
...
record_status "full_confirmation" "STARTED" "run_tag=${CONFIRMATION_RUN_TAG};specs=168;workers=20;threads_per_fit=1"
set +e
...
set_stage "closeout"
set +e
"$R_SCRIPT" validation/fitforecast_v2/scripts/closeout_qdesn_mcmc_sparse_topology_refine_v1.R \
