...
WORKERS="${2:-8}"
RUN_ID="${3:-qdesn_alpha_rho_confirmation_v1_$(date +%Y%m%d_%H%M%S)}"
...
if [[ "${FULL_CONFIRMATION_APPROVED:-0}" != "1" ]]; then
  printf 'Full confirmation requires FULL_CONFIRMATION_APPROVED=1.\n' >&2
  exit 2
fi
if [[ "$WORKERS" -lt 1 || "$WORKERS" -gt 8 ]]; then
  printf 'WORKERS must be between 1 and 8.\n' >&2
  exit 2
fi
...
if [[ "${PIPELINE_SELF_TEST:-0}" == "1" ]]; then
  ...
  write_heartbeat
  record_status "COMPLETED" "workers=${WORKERS};max_load=${MAX_LOAD}"
...
PREP_TAG="qdesn-arfc1-prepare-${STAMP}__git-${GIT_SHORT}"
SMOKE_TAG="qdesn-arfc1-smoke-${STAMP}__git-${GIT_SHORT}"
FULL_TAG="qdesn-arfc1-full-${STAMP}__git-${GIT_SHORT}"
...
run_logged "$STATE_ROOT/smoke_audit.log" \
  "$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_alpha_rho_confirmation_smoke.R \
...
run_logged "$STATE_ROOT/closeout.log" \
  "$R_SCRIPT" validation/fitforecast_v2/scripts/closeout_qdesn_alpha_rho_confirmation_v1.R \
  --run-tag "$FULL_TAG" --output-root "$STATE_ROOT/closeout"
DECISION="$($R_SCRIPT -e 'x <- jsonlite::read_json(commandArgs(TRUE)[1]); cat(x$decision)' "$STATE_ROOT/closeout/confirmation_gate.json")"
