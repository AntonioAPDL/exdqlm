...
output_root <- normalizePath(get_arg("--output-root"), winslash = "/",
                             mustWork = FALSE)
if (!from_stage %in% c(
  "tier_b_discovery", "tier_b_replication", "tier_b_sealed"
...
stages <- switch(from_stage,
  ...
  tier_b_replication = c("tier_b_discovery", "tier_b_replication"),
  tier_b_sealed = "tier_b_sealed"
)
...
for (cell_id in targets$target_cell_id) {
  ...
  parent_id <- paste0("tbcv1_", cell_id, "_parent")
...
profile_from_plan <- function(candidate_id) {
  stages <- c("tier_b_discovery", "tier_b_replication", "tier_b_sealed")
...
decision <- NULL
if (from_stage == "tier_b_discovery") {
  ...
  qdesn_ssv2_write_csv(ranking, file.path(output_root, "tier_b_replication_ranking.csv"))
  plan <- write_plan(selected, "tier_b_replication", "dev19", "r02")
  if (nrow(plan) != 16L) stop("Tier-B replication plan must contain 16 jobs.")
  decision <- "advance_to_tier_b_replication"
} else if (from_stage == "tier_b_replication") {
  selected <- setNames(lapply(
  ...
  qdesn_ssv2_write_csv(ranking, file.path(output_root, "tier_b_sealed_ranking.csv"))
  plan <- write_plan(
    selected, "tier_b_sealed", c("dev20", "dev21", "dev22", "dev23"), "r03"
  )
  if (nrow(plan) != 48L) stop("Tier-B sealed plan must contain 48 jobs.")
  decision <- "advance_to_tier_b_sealed"
} else {
  ...
  qdesn_ssv2_write_csv(
    eligible, file.path(output_root, "tier_b_sealed_eligible_metrics.csv")
  )
  qdesn_ssv2_write_csv(
    confirmation, file.path(output_root, "tier_b_confirmation_manifest.csv")
  )
  if (nrow(confirmation) > 12L) stop("Confirmation exceeds the 12-chain cap.")
  decision <- "confirmation_manifest_only_wait_for_explicit_approval"
}
...
manifest <- list(
  schema_version = "qdesn_tierb_cellwise_mcmc_v1_advancement_v1",
  ...
  decision = decision, article_state = "v6_frozen_unchanged"
)
