...
RUN_ID="${2:-qdesn_mcmc_chain_aggregate_confirm_v1_$(date +%Y%m%d_%H%M%S)}"
...
LOCK_FILE="reports/shared_fitforecast_v2_orchestration/qdesn_mcmc_chain_aggregate_confirm_v1.lock"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "Another chain-aggregate confirmation v1 pipeline holds $LOCK_FILE" >&2
  exit 2
fi
...
EXPECTED_BRANCH="validation/qdesn-mcmc-chain-aggregate-confirm-v1-1.0.0"
...
CONFIRMATION_RUN_TAG="qdesn-cagc1-full-${STAMP}__git-${GIT_SHORT}"
CPU_SET="${CPU_SET:-$(select_idle_cpus)}"
CPU_COUNT="$(tr ',' '\n' <<< "$CPU_SET" | sed '/^$/d' | wc -l)"
if [[ "$CPU_COUNT" -ne "$WORKERS" ]]; then
  echo "Expected 12 selected CPUs; found $CPU_COUNT in '$CPU_SET'." >&2
  exit 3
fi
{
  printf 'RUN_ID=%s\n' "$RUN_ID"
...
"$R_SCRIPT" validation/fitforecast_v2/scripts/materialize_qdesn_mcmc_chain_aggregate_confirm_v1.R \
...
record_status "contract_materialize" "COMPLETED" "12-spec contract regenerated without drift"

set_stage "contract_verify"
record_status "contract_verify" "STARTED" "branch=${BRANCH};commit=${GIT_SHA}"
"$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_mcmc_chain_aggregate_confirm_v1.R \
...
PREPARE_TAG="qdesn-cagc1-prepare-${STAMP}__git-${GIT_SHORT}"
...
set_stage "smoke"
SMOKE_TAG="qdesn-cagc1-smoke-${STAMP}__git-${GIT_SHORT}"
...
"$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_mcmc_chain_aggregate_confirm_v1_smoke.R \
...
wait_for_resources
record_status "cpu_selection" "COMPLETED" "workers=12;threads_per_fit=1;cpus=${CPU_SET}"

set_stage "full_confirmation"
record_status "full_confirmation" "STARTED" "run_tag=${CONFIRMATION_RUN_TAG};specs=12;workers=12;threads_per_fit=1"
set +e
...
set_stage "trace_compaction"
record_status "trace_compaction" "STARTED" "keep first;final;and every 50th trace row"
"$R_SCRIPT" validation/fitforecast_v2/scripts/compact_qdesn_progress_traces.R \
  --run-root "results/qdesn_mcmc_validation/${STAGE}/${CONFIRMATION_RUN_TAG}" \
  --output-dir "$STATE_ROOT/trace_compaction" --stride 50 \
  > "$STATE_ROOT/trace_compaction.log" 2>&1
record_status "trace_compaction" "COMPLETED" "status;heartbeat;failure artifacts preserved"
...
set_stage "closeout"
set +e
"$R_SCRIPT" validation/fitforecast_v2/scripts/closeout_qdesn_mcmc_chain_aggregate_confirm_v1.R \
