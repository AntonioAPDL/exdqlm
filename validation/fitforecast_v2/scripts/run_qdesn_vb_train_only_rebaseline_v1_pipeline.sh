...
WORKERS="${2:-18}"
RUN_ID="${3:-qdesn_vb_trainonly_rebaseline_v1_$(date +%Y%m%d_%H%M%S)}"
...
FIT_TIMEOUT_SECONDS="${FIT_TIMEOUT_SECONDS:-86400}"

if [[ "${FULL_TRAINONLY_VB_REBASELINE_APPROVED:-0}" != "1" ]]; then
  printf 'Full VB rebaseline requires FULL_TRAINONLY_VB_REBASELINE_APPROVED=1.\n' >&2
  exit 2
fi
...
for required in \
  "${STUB}_defaults.yaml" "${STUB}_grid.csv" "${STUB}_target_spec_ids.csv" \
  "${STUB}_smoke_defaults.yaml" "${STUB}_smoke_grid.csv" \
  "${STUB}_smoke_target_spec_ids.csv" "${STUB}_materialization_manifest.json"; do
...
PREP_TAG="qdesn-vb-trainonly-v1-prepare-${STAMP}__git-${GIT_SHORT}"
SMOKE_TAG="qdesn-vb-trainonly-v1-smoke-${STAMP}__git-${GIT_SHORT}"
FULL_TAG="qdesn-vb-trainonly-v1-full-${STAMP}__git-${GIT_SHORT}"
...
COMMON=(
  ...
  --allow-grid-subset --methods vb --likelihoods al,exal --priors rhs_ns
  --no-plots --stream-child-stdout --fit-timeout-seconds "$FIT_TIMEOUT_SECONDS"
  --fit-timeout-kill-after-seconds 60
)

CURRENT_STAGE="source_verification"
run_logged "$STATE_ROOT/source_verification.log" \
  "$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_vb_train_only_rebaseline_contract.R \
  --output "$STATE_ROOT/source_verification.json"

CURRENT_STAGE="prepare"
run_logged "$STATE_ROOT/prepare.log" "${COMMON[@]}" --workers "$WORKERS" \
  --defaults "${STUB}_defaults.yaml" --grid "${STUB}_grid.csv" \
  --spec-ids "$TARGET_SPEC_IDS" --batch full --prepare-only --run-tag "$PREP_TAG"

CURRENT_STAGE="smoke"
run_logged "$STATE_ROOT/smoke.log" "${COMMON[@]}" --workers 2 \
  --defaults "${STUB}_smoke_defaults.yaml" --grid "${STUB}_smoke_grid.csv" \
  --spec-ids "$SMOKE_SPEC_IDS" --batch full --run-tag "$SMOKE_TAG"
check_storage_light \
...
run_logged "$STATE_ROOT/smoke_audit.log" \
  "$R_SCRIPT" validation/fitforecast_v2/scripts/verify_qdesn_vb_train_only_rebaseline_smoke.R \
  --run-tag "$SMOKE_TAG" --output-root "$STATE_ROOT/smoke_audit"

CURRENT_STAGE="full_resource_gate"
wait_for_resources
CURRENT_STAGE="full"
run_logged "$STATE_ROOT/full.log" "${COMMON[@]}" --workers "$WORKERS" \
  --defaults "${STUB}_defaults.yaml" --grid "${STUB}_grid.csv" \
  --spec-ids "$TARGET_SPEC_IDS" --batch full --run-tag "$FULL_TAG"
check_storage_light \
...
run_logged "$STATE_ROOT/closeout.log" \
  "$R_SCRIPT" validation/fitforecast_v2/scripts/closeout_qdesn_vb_train_only_rebaseline_v1.R \
  --run-tag "$FULL_TAG" --output-root "$STATE_ROOT/closeout"
DECISION="$($R_SCRIPT -e 'x<-jsonlite::read_json(commandArgs(TRUE)[1]);cat(x$decision)' \
  "$STATE_ROOT/closeout/vb_rebaseline_gate.json")"
