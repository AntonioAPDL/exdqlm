...
## Full confirmation result

The authorized campaign completed under run ID
`independent_exdqlm_mcmc_rolling_state_fix_v1_full_20260829_035028__git-7ace661`.
All 27 jobs, nine family-by-quantile cells, and three chains per cell completed.
All row-health gates passed, all 23 full-confirmation checks passed, and no
fitted-model binary remains. The run used CRAN `exdqlm` 1.1.1 with
`collapsed_slice` throughout.

Fit RMSE and first-origin forecasts were invariant in every chain. Forecast
MAE and check loss improved in all nine cells. Forecast-MAE reductions ranged
from 13.0% to 92.4%; check-loss reductions ranged from 1.0% to 75.3%. This
combination confirms that the historical pathology was caused by the
post-origin MCMC rolling-state bridge rather than the fitted exDQLM model or
the CRAN sampler.

The final decision is
`READY_FOR_INTEGRATION_REPLACE_COMPLETE_EXDQLM_MCMC_BLOCK`. The complete
nine-cell exDQLM MCMC point block and its 27 posterior draw-metric interval
roles form one indivisible replacement. Integration must preserve every
non-exDQLM and VB value, regenerate all dependent tables and figures, and keep
fixed-path point metrics distinct from posterior means of draw-level metrics.

The compact tracked packet is generated by
`freeze_independent_exdqlm_mcmc_rolling_state_fix_v1_promotion.R` and verified
independently by
`verify_independent_exdqlm_mcmc_rolling_state_fix_v1_promotion.R`. Article-v2,
shared validation, and Overleaf remain the responsibility of the integration
coordinator.
