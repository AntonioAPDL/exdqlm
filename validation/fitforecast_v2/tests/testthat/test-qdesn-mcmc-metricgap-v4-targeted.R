test_that("Q-DESN MCMC metric-gap v4 targeted screen is per-cell and storage-light", {
  root <- ffv2_repo_root()
  stub <- "qdesn_dynamic_fitforecast_v2_tt500_mcmc_metricgap_v4_targeted"
  promo_root <- file.path(
    root,
    "validation", "fitforecast_v2", "promotions",
    "qdesn_tt500_mcmc_metricgap_v4_targeted_prelaunch_20260727"
  )
  ...
  expect_equal(nrow(profiles), 75)
  expect_equal(nrow(assignments), 75)
  expect_equal(nrow(grid), 75)
  expect_equal(nrow(target_specs), 75)
  expect_equal(length(unique(profiles$screening_profile_id)), 75)
  expect_equal(length(unique(target_specs$spec_id)), 75)
  ...
  expect_equal(length(unique(cell_keys)), 15)
  ...
  frozen_cells <- paste(
    c("laplace", "laplace", "laplace"),
    c("0.50000000", "0.50000000", "0.25000000"),
    c("al", "exal", "exal"),
    sep = "|"
  )
  expect_false(any(frozen_cells %in% cell_keys))
  expect_true(all(assignments$launch_status == "prepared_not_launched"))
  expect_true(all(profiles$p_over_n_tt500 <= 1.60))
  expect_true(any(profiles$p_over_n_tt500 > 0.50))
  expect_true(all(profiles$target_cells != ""))
  expect_true(any(abs(profiles$rhs_tau0 - 1e-6) <= 1e-12))
  expect_true(any(abs(profiles$rhs_tau0 - 3e-6) <= 1e-12))
  ...
  expect_equal(defaults$study_contract$screening_policy$promotion_policy, "screening_selects_candidates_for_full_confirmation_only")
  expect_equal(as.numeric(defaults$study_contract$screening_policy$unresolved_threshold), 1.10)
  ...
  expect_equal(as.integer(manifest$counts$frozen_rows), 3)
  expect_equal(as.integer(manifest$counts$targeted_rows), 15)
  expect_equal(as.integer(manifest$counts$candidates_per_row), 5)
  expect_equal(as.integer(manifest$counts$target_mcmc_atomic_specs), 75)
  expect_identical(
    manifest$source_registry_hash,
